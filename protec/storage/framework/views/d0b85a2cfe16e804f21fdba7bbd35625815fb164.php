...
vars = "?tahun=" + <?php echo e(Session::get('tahun')); ?>;
$.ajax({
          type: "GET",
...
$.ajax({
    ...
    success: function(data) {
          ...
          for (i = 0; i < j; i++) {
            post = data[i];
            $('select[name="cb_unit"]').append('<option value="'+ post.id_unit +'">'+ post.nm_unit +'</option>');
          }
          }
});
...
$(document).on('click', '#btnCariSubUnit', function() {
  ...
  CariSubUnit = $('#tblCariSubUnit').DataTable( {
        ...
        dom: 'bfrtIp',
        autoWidth : false,
        "ajax": {"url": "../admin/parameter/getSubUnit/"+unit_temp},
        "columns": [
              { data: 'no_urut', sClass: "dt-center"},
              { data: 'nm_sub'}
            ],
...
function loadTblProgRenja(unit){
   prog_renja_tbl=$('#tblProgram').DataTable({
                  processing: true,
                  serverSide: true,
                  dom : 'BfRtip',
                  autoWidth : false,
                  "ajax": {"url": "./pagu/getProgramRenja/"+unit},
                  "language": {
                      "decimal": ",",
                      "thousands": "."},
                  "columns": [
                        { data: 'urut', sClass: "dt-center", width:"5px"},
                        { data: 'kd_program_ref', sClass: "dt-center", width:"5px"},
                        { data: 'uraian_program_renstra'},
                        { data: 'jml_pendapatan', sClass: "dt-right",
                          render: $.fn.dataTable.render.number( '.', ',', 2, '' )},
                        { data: 'jml_belanja', sClass: "dt-center",
                          render: $.fn.dataTable.render.number( '.', ',', 2, '' )},
                        { data: 'pembiayaan_netto', sClass: "dt-right",
                          render: $.fn.dataTable.render.number( '.', ',', 2, '' )},
...
function loadTblKegiatanRenja(id_program){
    keg_renja_tbl=$('#tblKegiatanRenja').DataTable({
                  ...
                  dom : 'BfRtip',
                  autoWidth : false,
                  "ajax": {"url": "./pagu/getKegiatanRenja/"+id_program},
                  "language": {
                      "decimal": ",",
                      "thousands": "."},
                  "columns": [
...
function loadTblAktivitas(id_forum){
    aktivitas_tbl=$('#tblAktivitas').DataTable({
                  ...
                  dom : 'BfRtip',                  
                  autoWidth : false,
                  "ajax": {"url": "./pagu/getAktivitas/"+id_forum},
                  "language": {
                      "decimal": ",",
                      "thousands": "."},
                  "columns": [
...
function loadTblPelaksana(id_aktivitas){
    pelaksana_tbl=$('#tblPelaksana').DataTable({
                  ...
                  dom : 'BfRtip',
                  autoWidth : false,
                  "ajax": {"url": "./pagu/getPelaksanaAktivitas/"+id_aktivitas},
                  "language": {
                      "decimal": ",",
                      "thousands": "."},
                  "columns": [
...
function loadTblLokasi(id_pelaksana){
    lokasi_tbl=$('#tblLokasi').DataTable({
                  ...
                  deferRender: true,
                  autoWidth : false,
                  "ajax": {"url": "./pagu/getLokasiAktivitas/"+id_pelaksana},
                  "language": {
                      "decimal": ",",
                      "thousands": "."},
                  "columns": [
...
$( "#cb_unit" ).change(function() {
  tahun_temp = $('#tahun_rkpd').val();
  unit_temp = $('#cb_unit').val();
  dokumen_temp = $('#id_dokumen_keu').val(); 

  vars = "?token="     + $('input[name=_token]').val();
  vars += "&kd_unit="     + $('#cb_unit').val(); 

  $.ajax({
      type: "GET",
      url: '../admin/parameter/getSubUnitUser'+ vars,
      dataType: "json",
      ...
      success: function(data) {
            ...
            $('select[name="cb_sub_unit"]').empty();
            $('select[name="cb_sub_unit"]').append('<option value="-1">---Pilih Sub Unit---</option>');

            for (i = 0; i < j; i++) {
              post = data[i];
              $('select[name="cb_sub_unit"]').append('<option value="'+ post.id_sub_unit +'">'+ post.nm_sub +'</option>');
            }
            }
  });
});

$( "#cb_sub_unit" ).change(function() {
  tahun_temp = $('#tahun_rkpd').val();
  unit_temp = $('#id_unit').val();
  dokumen_temp = $('#id_dokumen_keu').val();

  $('.nav-tabs a[href="#programrkpd"]').tab('show');
  loadTblProgRenja($('#cb_sub_unit').val());
});
...
$(document).on('click', '.add-ProgRenja', function() {
  ...
  $('#id_dokumen_program').val(dokumen_temp);
