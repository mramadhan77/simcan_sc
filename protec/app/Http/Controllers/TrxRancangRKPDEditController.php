...
class TrxRancangRKPDEditController extends Controller
{
    ...
    {
       $CheckProgram=DB::SELECT('SELECT status_pelaksanaan, status_data FROM trx_rkpd_rancangan WHERE id_rkpd_rancangan = '.$id_program.' LIMIT 1');
    ...
    public function getCekProgram($id_ranwal){
        ...
        $CheckProgram=DB::SELECT('SELECT a.id_rkpd_rancangan, a.id_rkpd_ranwal, a.pagu_rpjmd, a.pagu_ranwal,  
    ...
    public function getCekIndikkator($id_ranwal){
        $cek=DB::SELECT('SELECT SUM(CASE WHEN a.status_data = 0 then 1 else 0 end) as jml_target FROM trx_rkpd_rancangan_indikator a 
    ...
    $cek = DB::SELECT('SELECT tahun_rkpd, flag FROM trx_rkpd_rancangan_dokumen WHERE tahun_rkpd = '.session::get('tahun').' LIMIT 1');
    ...
    public function hapusProgramRKPD(Request $req)
      {
        $CekProgram=$this->CheckStatusProgram($req->id_rkpd_rancangan);
        if($CekProgram == true){
            TrxRkpdRancangan::where('id_rkpd_rancangan',$req->id_rkpd_rancangan)->delete ();
    ...
    {
        TrxRkpdRancanganIndikator::where('id_indikator_rkpd',$req->id_indikator_program_rkpd)->delete ();
    ...
    public function postPelaksanaRKPD(Request $req){ 
        ...
        $cekpagu = DB::SELECT('SELECT x.id_pelaksana_rkpd, 
                ...
                FROM trx_rkpd_rancangan_pelaksana x 
                LEFT OUTER JOIN (SELECT a.id_rkpd_rancangan, SUM(a.pagu_forum) AS pagu_program FROM trx_rkpd_rancangan_program_pd a 
                ...
                (SELECT p.id_rkpd_rancangan, a.pagu_forum FROM trx_rkpd_rancangan_kegiatan_pd a 
                INNER JOIN trx_rkpd_rancangan_program_pd p ON a.id_program_pd = p.id_program_pd
                ...
                (SELECT p.id_rkpd_rancangan, y.pagu_aktivitas_forum  FROM trx_rkpd_rancangan_aktivitas_pd y 
                INNER JOIN trx_rkpd_rancangan_pelaksana_pd z ON y.id_pelaksana_pd = z.id_pelaksana_pd
                INNER JOIN trx_rkpd_rancangan_kegiatan_pd x ON z.id_kegiatan_pd = x.id_kegiatan_pd 
                INNER JOIN trx_rkpd_rancangan_program_pd p ON x.id_program_pd = p.id_program_pd
                ...
                (SELECT p.id_rkpd_rancangan, w.jml_belanja_forum  FROM trx_rkpd_rancangan_belanja_pd w
                INNER JOIN trx_rkpd_rancangan_aktivitas_pd y ON w.id_aktivitas_pd = y.id_aktivitas_pd
                INNER JOIN trx_rkpd_rancangan_pelaksana_pd z ON y.id_pelaksana_pd = z.id_pelaksana_pd
                INNER JOIN trx_rkpd_rancangan_kegiatan_pd x ON z.id_kegiatan_pd = x.id_kegiatan_pd 
                INNER JOIN trx_rkpd_rancangan_program_pd p ON x.id_program_pd = p.id_program_pd
    ...
    {
        $cek = DB::SELECT('SELECT COUNT(*) AS jml_data FROM trx_rkpd_rancangan_program_pd WHERE id_rkpd_rancangan='.$req->id_pelaksana_rkpd);
    ...
    public function getLoop(){
        ...
        $bidangs=DB::select('SELECT id_bidang, kd_urusan, "'.Fungsi::checkSumData(DB::raw('Convert(ref_bidang.id_bidang)')).'" AS checkSum, 
            kd_bidang, nm_bidang, kd_fungsi FROM ref_bidang ');
