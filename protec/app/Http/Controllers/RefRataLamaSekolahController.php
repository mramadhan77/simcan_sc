...
class RefRataLamaSekolahController extends Controller
{
	...
	public function index(){
        ...
        return view('dasar.ref_ratalamasekolah');
    }
    public function getListratalamasekolah()
    {
        $getListratalamasekolah=DB::select('SELECT (@id:=@id+1) as no_urut,
...
, (SELECT @id:=0) x
where a.id_tabel_dasar=4 and b.level>0
');

        return DataTables::of($getListratalamasekolah)
            ->addColumn('action',function($getListratalamasekolah){
                    return '
                        <button id="btnEditratalamasekolah" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Edit ratalamasekolah" title="Edit Rata-rata Lama Sekolah" class="btn btn-primary btn-sm" data-id_isian_tabel_dasar="'.$getListratalamasekolah->id_isian_tabel_dasar.'"><i class="fa fa-pencil fa-fw"></i></button>
                        <button id="btnHapusratalamasekolah" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Hapus ratalamasekolah" title="Hapus Rata-rata Lama Sekolah" class="btn btn-danger btn-sm" data-id_isian_tabel_dasar="'.$getListratalamasekolah->id_isian_tabel_dasar.'"><i class="fa fa-trash fa-fw"></i></button>
                    ' ;
            })
            
        ->make(true);
    }
    public function getTahunratalamasekolah()
    {
        $getTahun=DB::select('
...
');
    ...
    public function getKecamatanratalamasekolah()
    {
...
');
    ...
    public function getSektorratalamasekolah($tahun,$kecamatan)
    {
        $getSektor=DB::select('
...
where id_kolom_tabel_dasar not in (select id_kolom_tabel_dasar from trx_isian_data_dasar where tahun='.$tahun.' and id_kecamatan='.$kecamatan.')
 and  id_tabel_dasar=4 and level<>0
');
        return json_encode($getSektor);
    }
    public function addratalamasekolah (Request $req)
    {
    ...
    public function getEditratalamasekolah($id)
    {
        $getListratalamasekolah=DB::select('SELECT c.id_isian_tabel_dasar,
c.tahun,
...
LEFT OUTER JOIN trx_isian_data_dasar AS c ON c.id_kolom_tabel_dasar = b.id_kolom_tabel_dasar
LEFT OUTER JOIN ref_kecamatan AS d ON c.id_kecamatan = d.id_kecamatan
where a.id_tabel_dasar=4 and b.level>0 and id_isian_tabel_dasar='.$id);
        
        return DataTables::of($getListratalamasekolah)
        ->make(true);
    }
    public function editratalamasekolah (Request $req)
    {
    ...
    public function hapusratalamasekolah (Request $req)
    {
