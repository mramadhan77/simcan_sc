...
class CetakUkurTriwEs3Controller extends Controller
{
    ...
    public function UkurTriwEs3Lamp(Request $request)
    {
        $rules = [
            'tahun'=>'required',
            'triwulan'=>'required',
        ...
        $html .= '<thead>
            <tr nobr="true" height=19>                
                <td width="40%"  style="padding: 50px; text-align: center; font-weight: bold;" >SASARAN PROGRAM</td>
        ...
        $sasaran=DB::select('SELECT COALESCE(a.uraian_sasaran_program,"tidak ada data") AS uraian_sasaran_program,
            ...
            INNER JOIN kin_trx_real_es3_program_indikator g ON b.id_indikator_program_renstra = g.id_indikator_program_renstra
            INNER JOIN kin_trx_real_es3_program e ON g.id_real_program = e.id_real_program
            INNER JOIN kin_trx_real_es3_dok f ON e.id_dokumen_real = f.id_dokumen_real
            WHERE f.id_sotk_es3='.$request->unit.' AND f.tahun='.$request->tahun.' AND f.triwulan='.$request->triwulan);
        $html .= '<tbody>';
        foreach($sasaran as $row)
        {
            $html .='<tr nobr="true">';
            $html .='<td width="40%"  style="padding: 50px; text-align: left; font-weight: normal;" >'.$row->uraian_sasaran_program.'</td>';
            $html .='<td width="35%"  style="padding: 50px; text-align: left; font-weight: normal;" >'.$row->uraian_indikator_program_renstra.'</td>';
        ...
        $program=DB::SELECT('SELECT COALESCE(a.uraian_kegiatan_renstra,"tidak ada data") AS uraian_kegiatan_renstra,
            ...
            INNER JOIN ref_sotk_level_3 g ON f.id_sotk_es4 = g.id_sotk_es4
            WHERE g.id_sotk_es3='.$request->unit.' AND f.tahun='.$request->tahun.' AND f.triwulan='.$request->triwulan);
        $html .= '<tbody>';
