...
class TrxRanwalRKPDDokController extends Controller
{
    ...
    {
        $cek = DB::SELECT('SELECT b.id_rkpd_ranwal, COALESCE(COUNT(a.id_rkpd_ranwal),0) as jml FROM trx_renja_ranwal_program a
                LEFT OUTER JOIN trx_rkpd_ranwal b ON a.id_rkpd_ranwal = b.id_rkpd_ranwal
                INNER JOIN trx_rkpd_ranwal_dokumen c ON b.id_dokumen = c.id_dokumen_ranwal
                WHERE c.id_dokumen_ranwal = '.$req->id_dokumen_rkpd.' GROUP BY b.id_rkpd_ranwal');
        ...
        if($cek == null){
            ...
            return response ()->json (['pesan'=>'Data Gagal Proses, Dokumen sudah dipakai di - Ranwal Renja (0cdrPD)','status_pesan'=>'0']);
        }
    }
