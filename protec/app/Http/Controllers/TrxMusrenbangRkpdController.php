    ...
    public function getDataDokumen()
    {
         $getDataDokumen = DB::SELECT('SELECT (@id:=@id+1) as no_urut, a.id_dokumen_rkpd,a.nomor_rkpd,a.tanggal_rkpd,a.tahun_rkpd,
                ...
                CASE a.flag
                    ...
                    WHEN 1 THEN "green"
                END AS warna
                FROM trx_musrenkab_dokumen AS a
...
public function getDataRekap(){
        $data = DB::SELECT('SELECT (@id:=@id+1) as no_urut, x.id_musrenkab, x.tahun_rkpd, x.uraian_program_rpjmd, x.pagu_ranwal, x.status_data, x.sumber_data, 
                (SELECT COUNT(a.id_unit) FROM trx_musrenkab_pelaksana a
                    INNER JOIN  trx_musrenkab_urusan b ON a.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_musrenkab = x.id_musrenkab GROUP BY b.id_musrenkab) AS jml_unit,
                (SELECT COUNT(d.id_program_pd) FROM trx_musrenkab_program_pd d
                    INNER JOIN trx_musrenkab_pelaksana c ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                    INNER JOIN trx_musrenkab_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_musrenkab = x.id_musrenkab GROUP BY b.id_musrenkab) AS jml_prog_renja,
                (SELECT SUM(d.pagu_forum) FROM trx_musrenkab_program_pd d
                    INNER JOIN trx_musrenkab_pelaksana c ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                    INNER JOIN trx_musrenkab_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_musrenkab = x.id_musrenkab GROUP BY b.id_musrenkab) AS pagu_prog_renja,
                (SELECT COUNT(e.id_kegiatan_pd) FROM trx_musrenkab_kegiatan_pd AS e 
                    INNER JOIN trx_musrenkab_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_musrenkab_pelaksana c ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                    INNER JOIN trx_musrenkab_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_musrenkab = x.id_musrenkab GROUP BY b.id_musrenkab)  AS jml_kegiatan,
                (SELECT SUM(e.pagu_forum) FROM trx_musrenkab_kegiatan_pd AS e 
                    INNER JOIN trx_musrenkab_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_musrenkab_pelaksana c ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                    INNER JOIN trx_musrenkab_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_musrenkab = x.id_musrenkab GROUP BY b.id_musrenkab)  AS pagu_kegiatan,
                (SELECT COUNT(g.id_aktivitas_pd) FROM trx_musrenkab_aktivitas_pd AS g
                    INNER JOIN trx_musrenkab_pelaksana_pd AS f ON f.id_pelaksana_pd = g.id_pelaksana_pd
                    INNER JOIN trx_musrenkab_kegiatan_pd AS e ON e.id_kegiatan_pd = f.id_kegiatan_pd
                    INNER JOIN trx_musrenkab_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_musrenkab_pelaksana c ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                    INNER JOIN trx_musrenkab_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_musrenkab = x.id_musrenkab GROUP BY b.id_musrenkab)  AS jml_aktivitas, 
                (SELECT SUM(g.pagu_aktivitas_forum) FROM trx_musrenkab_aktivitas_pd AS g
                    INNER JOIN trx_musrenkab_pelaksana_pd AS f ON f.id_pelaksana_pd = g.id_pelaksana_pd
                    INNER JOIN trx_musrenkab_kegiatan_pd AS e ON e.id_kegiatan_pd = f.id_kegiatan_pd
                    INNER JOIN trx_musrenkab_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_musrenkab_pelaksana c ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                    INNER JOIN trx_musrenkab_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_musrenkab = x.id_musrenkab GROUP BY b.id_musrenkab)  AS pagu_aktivitas
                FROM trx_musrenkab AS x,
                (SELECT @id:=0) z
...
public function importData(Request $req){
    $dataProgrkpd=DB::INSERT('INSERT INTO trx_musrenkab (id_rkpd_ranwal,id_rkpd_rancangan, no_urut, tahun_rkpd, jenis_belanja, id_rkpd_rpjmd, thn_id_rpjmd, id_visi_rpjmd, 
        ...
        FROM trx_rkpd_rancangan AS a WHERE (a.status_pelaksanaan <> 2 AND a.status_pelaksanaan <> 3)  AND a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
    if($dataProgrkpd == 0){
        ...
        $dataIndikatorrkpd=DB::INSERT('INSERT INTO trx_musrenkab_indikator (tahun_rkpd, no_urut, id_musrenkab, id_indikator_program_rkpd, id_perubahan, kd_indikator, uraian_indikator_program_rkpd, tolok_ukur_indikator, target_rpjmd, target_rkpd, indikator_input, target_input, id_satuan_input, indikator_output, id_satuan_ouput, status_data, sumber_data)
            ...
            INNER JOIN trx_musrenkab AS a ON a.id_rkpd_rancangan = b.id_rkpd_rancangan 
        ...
        if($dataIndikatorrkpd == 0){
            ...
            $dataUrusanrkpd=DB::INSERT('INSERT INTO trx_musrenkab_urusan (tahun_rkpd, no_urut, id_musrenkab, id_urusan_rkpd, id_bidang, sumber_data)
                SELECT DISTINCT a.tahun_rkpd, b.no_urut, a.id_musrenkab, b.id_urusan_rkpd, b.id_bidang, 0 FROM trx_musrenkab AS a
                INNER JOIN trx_rkpd_rancangan_urusan AS b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan WHERE a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
            if($dataUrusanrkpd == 0){
                return response ()->json (['pesan'=>'Data Urusan Program Rancangan RKPD gagal di-Load','status_pesan'=>'0']);
            } else {
                $dataPelaksanarkpd=DB::INSERT('INSERT INTO trx_musrenkab_pelaksana (tahun_rkpd, no_urut, id_musrenkab, id_pelaksana_rpjmd, 
                id_urusan_rkpd, id_unit, pagu_rpjmd, pagu_rkpd, hak_akses, sumber_data, status_pelaksanaan, ket_pelaksanaan, status_data)
                    SELECT DISTINCT a.tahun_rkpd, c.no_urut, a.id_musrenkab, c.id_pelaksana_rkpd, c.id_urusan_rkpd, c.id_unit, c.pagu_rpjmd, c.pagu_rkpd, 
                ...
                if($dataPelaksanarkpd == 0){
                    ...
                    $dataProgpd=DB::INSERT('INSERT INTO trx_musrenkab_program_pd (id_pelaksana_rkpd, tahun_forum, jenis_belanja, no_urut, id_unit, id_rkpd_rancangan,
                    id_renja_program, id_program_renstra, uraian_program_renstra, id_program_ref, pagu_tahun_renstra, pagu_forum, sumber_data, 
                    status_pelaksanaan, ket_usulan, status_data, id_dokumen)
                        SELECT c.id_pelaksana_rkpd, z.tahun_forum, z.jenis_belanja, z.no_urut, z.id_unit, z.id_program_pd, z.id_forum_program, 
                        z.id_program_renstra, z.uraian_program_renstra, z.id_program_ref, z.pagu_forum, z.pagu_forum, 0,
                        0, z.ket_usulan, 0, 0
                        FROM trx_musrenkab AS a
                        INNER JOIN trx_musrenkab_urusan AS b ON a.id_musrenkab = b.id_musrenkab
                        INNER JOIN trx_musrenkab_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                        INNER JOIN trx_rkpd_rancangan_program_pd AS z ON c.id_pelaksana_rpjmd = z.id_rkpd_rancangan
                        WHERE  (z.status_pelaksanaan <> 2 AND z.status_pelaksanaan <> 3) AND a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                    if($dataProgpd == 0){
                        return response ()->json (['pesan'=>'Data Program Perangkat Daerah gagal di-Load','status_pesan'=>'0']);
                    } else {
                        $dataInprogpd=DB::INSERT('INSERT INTO trx_musrenkab_prog_indikator_pd (tahun_renja, no_urut, id_program_pd, id_program_forum,
                        ...
                        target_renstra, target_renja, indikator_output, id_satuan_ouput, indikator_input, target_input, id_satuan_input, status_data, sumber_data)    
                            ...
                            INNER JOIN trx_musrenkab_urusan AS b ON a.id_musrenkab = b.id_musrenkab
                            INNER JOIN trx_musrenkab_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                            INNER JOIN trx_musrenkab_program_pd AS d ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                            INNER JOIN trx_rkpd_rancangan_prog_indikator_pd AS e ON d.id_rkpd_rancangan = e.id_program_pd
                            WHERE a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                        if($dataInprogpd == 0){
                            return response ()->json (['pesan'=>'Data Indikator Program Perangkat Daerah gagal di-Load','status_pesan'=>'0']);
                        } else {
                            $dataKegpd=DB::INSERT('INSERT INTO trx_musrenkab_kegiatan_pd (id_forum_skpd, id_program_pd, id_unit, tahun_forum, 
                            no_urut, id_renja, id_rkpd_renstra, id_program_renstra, 
                                ...
                                INNER JOIN trx_musrenkab_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                                INNER JOIN trx_musrenkab_program_pd AS d ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                                INNER JOIN trx_rkpd_rancangan_kegiatan_pd AS e ON d.id_rkpd_rancangan = e.id_program_pd
                                WHERE  (e.status_pelaksanaan <> 2 AND e.status_pelaksanaan <> 3) AND a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                            if($dataKegpd == 0){
                                return response ()->json (['pesan'=>'Data Kegiatan Perangkat Daerah RKPD gagal di-Load','status_pesan'=>'0']);
                            } else {
                                $dataInkegpd=DB::INSERT('INSERT INTO trx_musrenkab_keg_indikator_pd (tahun_renja, no_urut, id_kegiatan_pd, id_program_renstra, id_perubahan,kd_indikator, 
                                    ...
                                    INNER JOIN trx_musrenkab_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                                    INNER JOIN trx_musrenkab_program_pd AS d ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                                    INNER JOIN trx_musrenkab_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                    INNER JOIN trx_rkpd_rancangan_keg_indikator_pd AS f ON e.id_forum_skpd = f.id_kegiatan_pd
                                    WHERE a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                                if($dataInkegpd == 0){
                                    ...
                                    $dataPelSubUnit=DB::INSERT('INSERT INTO trx_musrenkab_pelaksana_pd (tahun_forum, no_urut, id_kegiatan_pd, id_pelaksana_forum, id_sub_unit, id_pelaksana_renja, 
                                        ...
                                        INNER JOIN trx_musrenkab_program_pd AS d ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                                        INNER JOIN trx_musrenkab_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                        INNER JOIN trx_rkpd_rancangan_pelaksana_pd AS f ON e.id_forum_skpd = f.id_kegiatan_pd
                                        WHERE f.status_pelaksanaan <> 1 AND a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                                    if($dataPelSubUnit == 0){
                                        return response ()->json (['pesan'=>'Data Sub Unit Pelaksana Kegiatan gagal di-Load','status_pesan'=>'0']);
                                    } else {
                                        $dataAktivitas=DB::INSERT('INSERT INTO trx_musrenkab_aktivitas_pd 
                                            ...
                                            INNER JOIN trx_musrenkab_program_pd AS d ON c.id_pelaksana_rkpd = d.id_pelaksana_rkpd
                                            INNER JOIN trx_musrenkab_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                            INNER JOIN trx_musrenkab_pelaksana_pd AS f ON e.id_kegiatan_pd = f.id_kegiatan_pd
                                            INNER JOIN trx_rkpd_rancangan_aktivitas_pd AS g ON f.id_pelaksana_forum = g.id_pelaksana_pd
                                                WHERE  g.status_pelaksanaan <> 1 AND a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                                        if($dataAktivitas == 0){
                                            ...
                                            $dataLokasi=DB::INSERT('INSERT INTO trx_musrenkab_lokasi_pd (id_lokasi_forum, tahun_forum, no_urut, id_aktivitas_pd, id_lokasi, id_lokasi_renja, id_lokasi_teknis, 
                                                    ...
                                                    INNER JOIN trx_musrenkab_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                                    INNER JOIN trx_musrenkab_pelaksana_pd AS f ON e.id_kegiatan_pd = f.id_kegiatan_pd
                                                    INNER JOIN trx_musrenkab_aktivitas_pd AS g ON f.id_pelaksana_pd = g.id_pelaksana_pd
                                                    INNER JOIN trx_rkpd_rancangan_lokasi_pd AS h ON g.id_aktivitas_forum = h.id_aktivitas_pd
                                                    WHERE h.status_pelaksanaan NOT IN (2,3,4)  AND a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                                            if($dataLokasi == 0){
                                                ...
                                                $dataBelanja=DB::INSERT('INSERT INTO trx_musrenkab_belanja_pd (tahun_forum, no_urut, id_aktivitas_pd, id_belanja_forum, id_zona_ssh, id_belanja_renja, 
                                                        ...
                                                        INNER JOIN trx_musrenkab_pelaksana_pd AS f ON e.id_kegiatan_pd = f.id_kegiatan_pd
                                                        INNER JOIN trx_musrenkab_aktivitas_pd AS g ON f.id_pelaksana_pd = g.id_pelaksana_pd
                                                        INNER JOIN trx_rkpd_rancangan_belanja_pd AS h ON g.id_aktivitas_forum = h.id_aktivitas_pd
                                                        WHERE a.id_rkpd_rancangan='.$req->id_rkpd_ranwal);
                                                if($dataBelanja == 0){
    ...
    public function sesuai(Request $request, Builder $htmlBuilder)
    {
        // if(Auth::check()){
            if(Session::has('tahun')){
                return view('musrenrkpd.blangsung');  
            } else {
                return redirect('home');
            }            
        // } else {
    ...
    public function getSelectProgram($tahun_rkpd){
        ...
        LEFT OUTER JOIN trx_musrenkab AS b ON a.tahun_rkpd = b.tahun_rkpd AND a.id_rkpd_rancangan = b.id_rkpd_rancangan,
                (SELECT @id:=0) z
        WHERE b.id_rkpd_rancangan is null AND a.status_data = 2 AND (a.status_pelaksanaan <> 2 AND a.status_pelaksanaan <> 3) AND a.tahun_rkpd ='.$tahun_rkpd);
    ...
    {
        try{
            $data = TrxMusrenkabDokumen::find($req->id_dokumen_rkpd);
            $data->nomor_rkpd = $req->nomor_rkpd ;
            $data->tanggal_rkpd = $req->tanggal_rkpd ;
            $data->tahun_rkpd = $req->tahun_rkpd ;
            $data->uraian_perkada = $req->uraian_perkada ;
    ...
    {
        $result = TrxMusrenkabDokumen::destroy($req->id_dokumen_rkpd);
    ...
    {
        $result = DB::SELECT('SELECT id_musrenkab FROM trx_musrenkab WHERE status_data=0 AND id_musrenkab = '.$req->id_musrenkab);
    ...
    {
        $cek = DB::SELECT('SELECT x.tahun_rkpd, COALESCE(COUNT(x.id_musrenkab),0) AS jml, 
            COALESCE((SELECT COUNT(a.id_rkpd_rancangan) AS jml FROM trx_musrenkab AS a 
            LEFT OUTER JOIN trx_rkpd_ranhir AS b ON a.tahun_rkpd = b.tahun_rkpd AND a.id_rkpd_rancangan = b.id_forum_rkpdprog
            WHERE b.id_forum_rkpdprog is null AND a.status_data = 2 AND (a.status_pelaksanaan <> 2 AND a.status_pelaksanaan <> 3) 
        ...
        if($req->flag == 0){
            ...
            if(($cek[0]->jml - $cek[0]->jml_load) <> $cek[0]->jml ){
                $data = DB::UPDATE('UPDATE trx_musrenkab_dokumen SET flag ='.$req->flag.' WHERE tahun_rkpd='.$req->tahun_rkpd.' AND id_dokumen_rkpd='.$req->id_dokumen_rkpd);       
                if($data != 0){
                    $dataProg=DB::UPDATE('UPDATE trx_musrenkab SET status_data ='.$req->status.', id_dokumen='.$req->id_dokumen_rkpd.' WHERE tahun_rkpd='.$req->tahun_rkpd.' AND status_data='.$req->status_awal);
                    if($dataProg != 0){
                        return response ()->json (['pesan'=>'Data Berhasil Unposting (1)','status_pesan'=>'1']);
                    } else {
                ...
                if($data != 0){
                    $dataProg=DB::UPDATE('UPDATE trx_musrenkab SET status_data ='.$req->status.', id_dokumen='.$req->id_dokumen_rkpd.' WHERE tahun_rkpd='.$req->tahun_rkpd.' AND status_data='.$req->status_awal);
                    if($dataProg != 0){
                        return response ()->json (['pesan'=>'Data Berhasil Unposting (0)','status_pesan'=>'1']);
                    } else {
                        return response ()->json (['pesan'=>'Data Gagal Unposting (1cprPD)','status_pesan'=>'0']);
                    }
                } else {
