...
class TrxRkpdRanhirController extends Controller
{
...
public function getDataDokumen()
    {
         $getDataDokumen = DB::SELECT('SELECT (@id:=@id+1) as no_urut, a.id_dokumen_rkpd,a.nomor_rkpd,a.tanggal_rkpd,a.tahun_rkpd,
                ...
                CASE a.flag
                    ...
                    WHEN 1 THEN "green"
                END AS warna
                FROM trx_rkpd_ranhir_dokumen AS a
...
public function getDataRekap(){
        $data = DB::SELECT('SELECT (@id:=@id+1) as no_urut, x.id_rkpd_rancangan, x.tahun_rkpd, 
                ...
                (SELECT COUNT(a.id_unit) FROM trx_rkpd_ranhir_pelaksana a
                    INNER JOIN  trx_rkpd_ranhir_urusan b ON a.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_rkpd_rancangan = x.id_rkpd_rancangan GROUP BY b.id_rkpd_rancangan) AS jml_unit,
                (SELECT COUNT(d.id_program_pd) FROM trx_rkpd_ranhir_program_pd d
                    INNER JOIN trx_rkpd_ranhir_pelaksana c ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                    INNER JOIN trx_rkpd_ranhir_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_rkpd_rancangan = x.id_rkpd_rancangan GROUP BY b.id_rkpd_rancangan) AS jml_prog_renja,
                (SELECT SUM(d.pagu_forum) FROM trx_rkpd_ranhir_program_pd d
                    INNER JOIN trx_rkpd_ranhir_pelaksana c ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                    INNER JOIN trx_rkpd_ranhir_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_rkpd_rancangan = x.id_rkpd_rancangan GROUP BY b.id_rkpd_rancangan) AS pagu_prog_renja,
                (SELECT COUNT(e.id_kegiatan_pd) FROM trx_rkpd_ranhir_kegiatan_pd AS e 
                    INNER JOIN trx_rkpd_ranhir_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_rkpd_ranhir_pelaksana c ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                    INNER JOIN trx_rkpd_ranhir_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_rkpd_rancangan = x.id_rkpd_rancangan GROUP BY b.id_rkpd_rancangan)  AS jml_kegiatan,
                (SELECT SUM(e.pagu_forum) FROM trx_rkpd_ranhir_kegiatan_pd AS e 
                    INNER JOIN trx_rkpd_ranhir_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_rkpd_ranhir_pelaksana c ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                    INNER JOIN trx_rkpd_ranhir_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_rkpd_rancangan = x.id_rkpd_rancangan GROUP BY b.id_rkpd_rancangan)  AS pagu_kegiatan,
                (SELECT COUNT(g.id_aktivitas_pd) FROM trx_rkpd_ranhir_aktivitas_pd AS g
                    INNER JOIN trx_rkpd_ranhir_pelaksana_pd AS f ON f.id_pelaksana_pd = g.id_pelaksana_pd
                    INNER JOIN trx_rkpd_ranhir_kegiatan_pd AS e ON e.id_kegiatan_pd = f.id_kegiatan_pd
                    INNER JOIN trx_rkpd_ranhir_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_rkpd_ranhir_pelaksana c ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                    INNER JOIN trx_rkpd_ranhir_urusan b ON c.id_urusan_rkpd = b.id_urusan_rkpd
                    WHERE b.id_rkpd_rancangan = x.id_rkpd_rancangan GROUP BY b.id_rkpd_rancangan)  AS jml_aktivitas, 
                (SELECT SUM(g.pagu_aktivitas_forum) FROM trx_rkpd_ranhir_aktivitas_pd AS g
                    INNER JOIN trx_rkpd_ranhir_pelaksana_pd AS f ON f.id_pelaksana_pd = g.id_pelaksana_pd
                    INNER JOIN trx_rkpd_ranhir_kegiatan_pd AS e ON e.id_kegiatan_pd = f.id_kegiatan_pd
                    INNER JOIN trx_rkpd_ranhir_program_pd d ON d.id_program_pd = e.id_program_pd
                    INNER JOIN trx_rkpd_ranhir_pelaksana c ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
...
public function importData(Request $req){
    $dataProgrkpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir (id_rkpd_ranwal,id_forum_rkpdprog, no_urut, tahun_rkpd, jenis_belanja, id_rkpd_rpjmd, thn_id_rpjmd, id_visi_rpjmd, 
        ...
        SELECT DISTINCT a.id_rkpd_ranwal,a.id_musrenkab, a.no_urut, a.tahun_rkpd, a.jenis_belanja, a.id_rkpd_rpjmd, a.thn_id_rpjmd,
        ...
        FROM trx_musrenkab AS a WHERE (a.status_pelaksanaan <> 2 AND a.status_pelaksanaan <> 3)  AND a.id_musrenkab='.$req->id_rkpd_ranwal);
    if($dataProgrkpd == 0){
        return response ()->json (['pesan'=>'Data Program Rancangan RKPD gagal di-Load','status_pesan'=>'0']);
    } else {
        $dataIndikatorrkpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir_indikator (tahun_rkpd, no_urut, id_rkpd_rancangan, id_indikator_program_rkpd, id_perubahan, kd_indikator, uraian_indikator_program_rkpd, tolok_ukur_indikator, target_rpjmd, target_rkpd, indikator_input, target_input, id_satuan_input, indikator_output, id_satuan_ouput, status_data, sumber_data)
            ...
            0, 0 FROM trx_musrenkab_indikator AS b 
            INNER JOIN trx_rkpd_ranhir AS a ON a.id_forum_rkpdprog = b.id_musrenkab 
        ...
        if($dataIndikatorrkpd == 0){
            ...
            $dataUrusanrkpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir_urusan (tahun_rkpd, no_urut, id_rkpd_rancangan, id_urusan_rkpd, id_bidang, sumber_data)
                SELECT DISTINCT a.tahun_rkpd, b.no_urut, a.id_rkpd_rancangan, b.id_urusan_rkpd, b.id_bidang, 0 FROM trx_rkpd_ranhir AS a
                INNER JOIN trx_musrenkab_urusan AS b ON a.id_forum_rkpdprog = b.id_musrenkab WHERE a.id_forum_rkpdprog='.$req->id_rkpd_ranwal);
            if($dataUrusanrkpd == 0){
                return response ()->json (['pesan'=>'Data Urusan Program Rancangan RKPD gagal di-Load','status_pesan'=>'0']);
            } else {
                $dataPelaksanarkpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir_pelaksana (tahun_rkpd, no_urut, id_rkpd_rancangan, id_pelaksana_rpjmd, 
                id_urusan_rkpd, id_unit, pagu_rpjmd, pagu_rkpd, hak_akses, sumber_data, status_pelaksanaan, ket_pelaksanaan, status_data)
                    ...
                    INNER JOIN trx_musrenkab_urusan AS b ON a.id_forum_rkpdprog = b.id_musrenkab
                ...
                if($dataPelaksanarkpd == 0){
                    ...
                    $dataProgpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir_program_pd (id_rkpd_rancangan, tahun_forum, jenis_belanja, no_urut, id_unit, id_forum_program, id_renja_program, id_program_renstra, uraian_program_renstra, id_program_ref, pagu_tahun_renstra, pagu_forum, sumber_data, status_pelaksanaan, ket_usulan, status_data, id_dokumen)
                        ...
                        0, z.ket_usulan, 0, 0
                        FROM trx_rkpd_ranhir AS a
                        INNER JOIN trx_rkpd_ranhir_urusan AS b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
                        INNER JOIN trx_rkpd_ranhir_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                        INNER JOIN trx_musrenkab_program_pd AS z ON c.id_pelaksana_rpjmd = z.id_pelaksana_rkpd
                    ...
                    if($dataProgpd == 0){
                        ...
                        $dataInprogpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir_prog_indikator_pd (tahun_renja, no_urut, id_program_pd, id_program_forum,
                        ...
                        target_renstra, target_renja, indikator_output, id_satuan_ouput, indikator_input, target_input, id_satuan_input, status_data, sumber_data)    
                            ...
                            INNER JOIN trx_rkpd_ranhir_urusan AS b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
                            INNER JOIN trx_rkpd_ranhir_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                            INNER JOIN trx_rkpd_ranhir_program_pd AS d ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                            INNER JOIN trx_musrenkab_prog_indikator_pd AS e ON d.id_forum_program = e.id_program_pd
                            WHERE a.id_forum_rkpdprog='.$req->id_rkpd_ranwal);
                        if($dataInprogpd == 0){
                            return response ()->json (['pesan'=>'Data Indikator Program Perangkat Daerah gagal di-Load','status_pesan'=>'0']);
                        } else {
                            $dataKegpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir_kegiatan_pd (id_forum_skpd, id_program_pd, id_unit, tahun_forum, 
                            no_urut, id_renja, id_rkpd_renstra, id_program_renstra, 
                                ...
                                INNER JOIN trx_rkpd_ranhir_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                                INNER JOIN trx_rkpd_ranhir_program_pd AS d ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                                INNER JOIN trx_musrenkab_kegiatan_pd AS e ON d.id_forum_program = e.id_program_pd
                            ...
                            if($dataKegpd == 0){
                                ...
                                $dataInkegpd=DB::INSERT('INSERT INTO trx_rkpd_ranhir_keg_indikator_pd (tahun_renja, no_urut, id_kegiatan_pd, id_program_renstra, id_perubahan,kd_indikator, 
                                    ...
                                    INNER JOIN trx_rkpd_ranhir_pelaksana AS c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                                    INNER JOIN trx_rkpd_ranhir_program_pd AS d ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                                    INNER JOIN trx_rkpd_ranhir_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                    INNER JOIN trx_musrenkab_keg_indikator_pd AS f ON e.id_forum_skpd = f.id_kegiatan_pd
                                    WHERE a.id_forum_rkpdprog='.$req->id_rkpd_ranwal);
                                if($dataInkegpd == 0){
                                    return response ()->json (['pesan'=>'Data Indikator Kegiatan Perangkat Daerah gagal di-Load','status_pesan'=>'0']);
                                } else {
                                    $dataPelSubUnit=DB::INSERT('INSERT INTO trx_rkpd_ranhir_pelaksana_pd (tahun_forum, no_urut, id_kegiatan_pd, id_pelaksana_forum, id_sub_unit, id_pelaksana_renja, 
                                        ...
                                        INNER JOIN trx_rkpd_ranhir_program_pd AS d ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                                        INNER JOIN trx_rkpd_ranhir_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                        INNER JOIN trx_musrenkab_pelaksana_pd AS f ON e.id_forum_skpd = f.id_kegiatan_pd
                                    ...
                                    if($dataPelSubUnit == 0){
                                        ...
                                        $dataAktivitas=DB::INSERT('INSERT INTO trx_rkpd_ranhir_aktivitas_pd 
                                            ...
                                            INNER JOIN trx_rkpd_ranhir_program_pd AS d ON c.id_pelaksana_rkpd = d.id_rkpd_rancangan
                                            INNER JOIN trx_rkpd_ranhir_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                            INNER JOIN trx_rkpd_ranhir_pelaksana_pd AS f ON e.id_kegiatan_pd = f.id_kegiatan_pd
                                            INNER JOIN trx_musrenkab_aktivitas_pd AS g ON f.id_pelaksana_forum = g.id_pelaksana_pd
                                        ...
                                        if($dataAktivitas == 0){
                                            ...
                                            $dataLokasi=DB::INSERT('INSERT INTO trx_rkpd_ranhir_lokasi_pd (id_lokasi_forum, tahun_forum, no_urut, id_aktivitas_pd, id_lokasi, id_lokasi_renja, id_lokasi_teknis, 
                                                    ...
                                                    INNER JOIN trx_rkpd_ranhir_kegiatan_pd AS e ON d.id_program_pd = e.id_program_pd
                                                    INNER JOIN trx_rkpd_ranhir_pelaksana_pd AS f ON e.id_kegiatan_pd = f.id_kegiatan_pd
                                                    INNER JOIN trx_rkpd_ranhir_aktivitas_pd AS g ON f.id_pelaksana_pd = g.id_pelaksana_pd
                                                    INNER JOIN trx_musrenkab_lokasi_pd AS h ON g.id_aktivitas_forum = h.id_aktivitas_pd
                                            ...
                                            if($dataLokasi == 0){
                                                ...
                                                $dataBelanja=DB::INSERT('INSERT INTO trx_rkpd_ranhir_belanja_pd (tahun_forum, no_urut, id_aktivitas_pd, id_belanja_forum, id_zona_ssh, id_belanja_renja, 
                                                        ...
                                                        INNER JOIN trx_rkpd_ranhir_pelaksana_pd AS f ON e.id_kegiatan_pd = f.id_kegiatan_pd
                                                        INNER JOIN trx_rkpd_ranhir_aktivitas_pd AS g ON f.id_pelaksana_pd = g.id_pelaksana_pd
                                                        INNER JOIN trx_musrenkab_belanja_pd AS h ON g.id_aktivitas_forum = h.id_aktivitas_pd
                                                        WHERE a.id_forum_rkpdprog='.$req->id_rkpd_ranwal);
                                                if($dataBelanja == 0){
    ...
    public function getSelectProgram($tahun_rkpd){
        $getSelect=DB::SELECT('SELECT (@id:=@id+1) as urut, a.id_rkpd_ranwal,a.id_musrenkab, a.tahun_rkpd, a.jenis_belanja, a.id_rkpd_rpjmd, a.thn_id_rpjmd,
        a.id_visi_rpjmd, a.id_misi_rpjmd, a.id_tujuan_rpjmd, a.id_sasaran_rpjmd, a.id_program_rpjmd, a.uraian_program_rpjmd
        FROM trx_musrenkab AS a 
        LEFT OUTER JOIN trx_rkpd_ranhir AS b ON a.tahun_rkpd = b.tahun_rkpd AND a.id_musrenkab = b.id_forum_rkpdprog,
                (SELECT @id:=0) z
    ...
    {
        try{
            $data = TrxRkpdRanhirDokumen::find($req->id_dokumen_rkpd);
            $data->nomor_rkpd = $req->nomor_rkpd ;
            $data->tanggal_rkpd = $req->tanggal_rkpd ;
            $data->tahun_rkpd = $req->tahun_rkpd ;
            $data->uraian_perkada = $req->uraian_perkada ;
    ...
    {
        $result = TrxRkpdRanhirDokumen::destroy($req->id_dokumen_rkpd);
    ...
    {
        $result = DB::SELECT('SELECT id_rkpd_rancangan FROM trx_rkpd_ranhir WHERE status_data=0 AND id_rkpd_rancangan = '.$req->id_rkpd_rancangan);

        if($result != Null){
            try{
                $result=DB::DELETE('DELETE FROM trx_rkpd_ranhir WHERE id_rkpd_rancangan = '.$req->id_rkpd_rancangan);
    ...
    public function postDokumen(Request $req)
    {
        $cek = DB::SELECT('SELECT x.tahun_rkpd, COALESCE(COUNT(x.id_rkpd_rancangan),0) AS jml, 
            COALESCE((SELECT COUNT(a.id_rkpd_rancangan) AS jml FROM trx_rkpd_ranhir AS a 
        ...
        if($req->flag == 0){
            if($cek != null) { 
                if(($cek[0]->jml - $cek[0]->jml_load) <> $cek[0]->jml ){
                    $data = DB::UPDATE('UPDATE trx_rkpd_ranhir_dokumen SET flag ='.$req->flag.' WHERE tahun_rkpd='.$req->tahun_rkpd.' AND id_dokumen_rkpd='.$req->id_dokumen_rkpd);       
                    if($data != 0){
                        $dataProg=DB::UPDATE('UPDATE trx_rkpd_ranhir SET status_data ='.$req->status.', id_dokumen='.$req->id_dokumen_rkpd.' WHERE tahun_rkpd='.$req->tahun_rkpd.' AND status_data='.$req->status_awal);
                        if($dataProg != 0){
