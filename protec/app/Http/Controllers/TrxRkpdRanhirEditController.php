...
class TrxRkpdRanhirEditController extends Controller
{
    ...
    {
       $CheckProgram=DB::SELECT('SELECT status_pelaksanaan, status_data FROM trx_rkpd_ranhir WHERE id_rkpd_rancangan = '.$id_program.' LIMIT 1');
    ...
    public function getCekProgram($id_ranwal){
        ...
        $CheckProgram=DB::SELECT('SELECT a.id_rkpd_rancangan, a.pagu_rpjmd, a.pagu_ranwal,  
            COALESCE((SELECT COUNT(*) AS jml_indi FROM trx_rkpd_ranhir_indikator b
            ...
            COALESCE((SELECT COUNT(*) AS jml_indi FROM trx_rkpd_ranhir_pelaksana b
                WHERE b.tahun_rkpd=a.tahun_rkpd AND b.id_rkpd_rancangan=a.id_rkpd_rancangan
                GROUP BY b.tahun_rkpd, b.id_rkpd_rancangan),0) as jml_unit, 
            COALESCE((SELECT COUNT(*) AS jml_indi FROM trx_rkpd_ranhir_pelaksana b
                WHERE b.tahun_rkpd=a.tahun_rkpd AND b.id_rkpd_rancangan=a.id_rkpd_rancangan AND b.status_data=1
                GROUP BY b.tahun_rkpd, b.id_rkpd_rancangan),0) as unit_0,
            COALESCE((SELECT SUM(d.pagu_forum) FROM trx_rkpd_ranhir_program_pd d
    ...
    public function getCekIndikkator($id_ranwal){
        $cek=DB::SELECT('SELECT SUM(CASE WHEN a.status_data = 0 then 1 else 0 end) as jml_target FROM trx_rkpd_ranhir_indikator a 
    ...
    public function getIndikatorRKPD($id_rkpd)
    {
      $indikatorRKPD=DB::select('SELECT a.tahun_rkpd,(@id:=@id+1) as urut, a.no_urut, a.id_rkpd_rancangan,a.id_indikator_program_rkpd,a.id_perubahan,a.kd_indikator,a.uraian_indikator_program_rkpd,a.id_indikator_rkpd,
          ...
          CASE a.status_data
              ...
              WHEN 1 THEN "green"
          END AS warna  
            FROM trx_rkpd_ranhir_indikator a
            INNER JOIN trx_rkpd_ranhir b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan AND a.tahun_rkpd = b.tahun_rkpd,(SELECT @id:=0) x where a.id_rkpd_rancangan='.$id_rkpd);
    ...
    public function getPelaksanaRKPD($id_rkpd,$id_urusan)
    {
      $pelaksanaRKPD=DB::select('SELECT (@id:=@id+1) as urut, a.no_urut, a.id_pelaksana_rkpd, a.id_unit,a.tahun_rkpd,a.id_rkpd_rancangan,
            ...
            FROM trx_rkpd_ranhir_pelaksana AS a
            INNER JOIN trx_rkpd_ranhir c ON a.id_rkpd_rancangan = c.id_rkpd_rancangan AND a.tahun_rkpd = c.tahun_rkpd 
    ...
    $cek = DB::SELECT('SELECT tahun_rkpd, flag FROM trx_rkpd_ranhir_dokumen WHERE tahun_rkpd = '.session::get('tahun').' LIMIT 1');
    
           $data = new TrxRkpdRanhir();
    ...
    public function editProgramRKPD(Request $req)
    {
        ...
        if($CekProgram == true){
            try{    
                $data = TrxRkpdRanhir::find($req->id_rkpd_rancangan);
                $data->no_urut = $req->no_urut ;
                $data->uraian_program_rpjmd = $req->uraian_program_rpjmd ;
                $data->jenis_belanja = $req->jenis_belanja ;
                $data->thn_id_rpjmd = $req->thn_id_rpjmd ;
    ...
    public function postProgram(Request $req)
    {
        ...
        $data = TrxRkpdRanhir::find($req->id_rkpd_ranwal);
        $data->status_data = $req->status_data ;
    ...
    public function hapusProgramRKPD(Request $req)
      {
        $CekProgram=$this->CheckStatusProgram($req->id_rkpd_rancangan);
        if($CekProgram == true){
            TrxRkpdRanhir::where('id_rkpd_rancangan',$req->id_rkpd_rancangan)->delete ();
    ...
    public function addIndikatorRKPD(Request $req){
        ...
        if($CekProgram == true){
            $data= new TrxRkpdRanhirIndikator();
            $data->tahun_rkpd=session::get('tahun') ; 
            $data->no_urut=$req->no_urut; 
    ...
    public function editIndikatorRKPD(Request $req)
    {
        $CekProgram=$this->CheckStatusProgram($req->id_rkpd_rancangan);
            ...
            $data= TrxRkpdRanhirIndikator::find($req->id_indikator_rkpd);
            $data->no_urut=$req->no_urut; 
            $data->kd_indikator=$req->kd_indikator; 
            $data->uraian_indikator_program_rkpd=$req->uraian_indikator; 
            $data->tolok_ukur_indikator=$req->tolok_ukur_indikator;
    ...
    public function postIndikatorRKPD(Request $req){
        try{     
            $data= TrxRkpdRanhirIndikator::find($req->id_indikator_program_rkpd);        
    ...
    {
        TrxRkpdRanhirIndikator::where('id_indikator_program_rkpd',$req->id_indikator_program_rkpd)->delete ();
    ...
    public function hapusUrusanRKPD(Request $req){
      ...
      if($cek == NULL || $cek[0]->jml_pelaksana == 0){
        TrxRkpdRanhirUrusan::where('id_urusan_rkpd',$req->id_urusan_rkpd)->delete ();
        return response ()->json (['pesan'=>'Data Berhasil Dihapus','status_pesan'=>'1']);
      } else {
    ...
    try{
           $data = new TrxRkpdRanhirPelaksana();
           $data->no_urut = $req->no_urut ;
           $data->tahun_rkpd = session::get('tahun') ;
    ...
    public function editPelaksanaRKPD(Request $req){
        ...
        $data = TrxRkpdRanhirPelaksana::find($req->id_pelaksana_rkpd);
        $data->no_urut = $req->no_urut ;
        $data->id_unit = $req->id_unit ;
        $data->hak_akses=$req->hak_akses;
        $data->status_data=$req->status_data;
    ...
    public function postPelaksanaRKPD(Request $req){ 
        ...
        $cekpagu = DB::SELECT('SELECT x.id_pelaksana_rkpd, 
                ...
                FROM trx_rkpd_ranhir_pelaksana x 
                LEFT OUTER JOIN (SELECT a.id_rkpd_rancangan, SUM(a.pagu_forum) AS pagu_program FROM trx_rkpd_ranhir_program_pd a 
                ...
                (SELECT p.id_rkpd_rancangan, a.pagu_forum FROM trx_rkpd_ranhir_kegiatan_pd a 
                INNER JOIN trx_rkpd_ranhir_program_pd p ON a.id_program_pd = p.id_program_pd
                ...
                (SELECT p.id_rkpd_rancangan, y.pagu_aktivitas_forum  FROM trx_rkpd_ranhir_aktivitas_pd y 
                INNER JOIN trx_rkpd_ranhir_pelaksana_pd z ON y.id_pelaksana_pd = z.id_pelaksana_pd
                INNER JOIN trx_rkpd_ranhir_kegiatan_pd x ON z.id_kegiatan_pd = x.id_kegiatan_pd 
                INNER JOIN trx_rkpd_ranhir_program_pd p ON x.id_program_pd = p.id_program_pd
                ...
                (SELECT p.id_rkpd_rancangan, w.jml_belanja_forum  FROM trx_rkpd_ranhir_belanja_pd w
                INNER JOIN trx_rkpd_ranhir_aktivitas_pd y ON w.id_aktivitas_pd = y.id_aktivitas_pd
                INNER JOIN trx_rkpd_ranhir_pelaksana_pd z ON y.id_pelaksana_pd = z.id_pelaksana_pd
                INNER JOIN trx_rkpd_ranhir_kegiatan_pd x ON z.id_kegiatan_pd = x.id_kegiatan_pd 
                INNER JOIN trx_rkpd_ranhir_program_pd p ON x.id_program_pd = p.id_program_pd
    ...
    {
        $cek = DB::SELECT('SELECT COUNT(*) AS jml_data FROM trx_rkpd_ranhir_program_pd WHERE id_rkpd_rancangan='.$req->id_pelaksana_rkpd);
        if($cek == NULL || $cek[0]->jml_data == 0){
            TrxRkpdRanhirPelaksana::where('id_pelaksana_rkpd',$req->id_pelaksana_rkpd)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus','status_pesan'=>'1'] );
        } else {
