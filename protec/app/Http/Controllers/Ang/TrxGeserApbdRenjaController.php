...
class TrxGeserApbdRenjaController extends Controller
{
        ...
        $getRenja=DB::SELECT('SELECT (@id:=@id+1) as no_urut, a.id_anggaran_pemda, a.id_rkpd_ranwal, a.tahun_anggaran, a.id_program_rpjmd, 
          ...
          return DataTables::of($getRenja)
            ...
            ->addColumn('details_url', function($getRenja) {
                    return url('GeserApbd/getChildBidang/'.$getRenja->id_unit.'/'. $getRenja->id_anggaran_pemda);
                })
            ->make(true);
    }
    ...
    {
      $getProgramRenja=DB::select('SELECT (@id:=@id+1) as urut, a.* FROM (SELECT a.id_program_pd, a.id_program_pd_rkpd_final, a.id_pelaksana_anggaran, a.tahun_anggaran, a.no_urut, a.id_unit, b.nm_unit,  
            a.id_program_renstra, a.uraian_program_renstra, a.id_program_ref, d.uraian_program, a.pagu_rkpd_final, a.pagu_anggaran, 
            a.sumber_data, a.status_pelaksanaan, a.jenis_belanja,g.uraian_program_renstra as program_renstra,
            a.ket_usulan, a.status_data,a.kd_dokumen_keu,a.jns_dokumen_keu,a.id_perubahan,a.id_dokumen_keu,
            CASE a.status_data
                WHEN 0 THEN "fa fa-question"
                WHEN 1 THEN "fa fa-check-square-o"
            END AS status_icon,
            ...
            COALESCE((SELECT COUNT(x.id_kegiatan_pd) AS jml_kegiatan
                        FROM trx_anggaran_kegiatan_pd AS x
            ...
            WHERE a.id_unit ='.$id_unit.' AND a.id_pelaksana_anggaran = '.$id_pelaksana_rkpd.' AND a.tahun_anggaran='.Session::get('tahun'));

      return DataTables::of($getProgramRenja)
        ->addColumn('details_url', function($getProgramRenja) {
                    return url('GeserApbd/getIndikatorRenja/'.$getProgramRenja->id_program_pd);
                })
    ...
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_anggaran, a.no_urut, a.id_program_pd, a.id_program_renstra,
                ...
                a.id_satuan_output, a.indikator_input, a.target_input, a.id_satuan_input, a.status_data, a.sumber_data,
                        CASE a.status_data
                            WHEN 0 THEN "fa fa-question"
                            WHEN 1 THEN "fa fa-check-square-o"
                        END AS status_icon,
                ...
                INNER JOIN trx_anggaran_program_pd AS b ON a.id_program_pd=b.id_program_pd
    ...
    public function getKegiatanRenja($id_program)
    {
      ...
      ->addColumn('details_url', function($getKegiatanRenja) {
                    return url('Apbd/getIndikatorKegiatan/'.$getKegiatanRenja->id_kegiatan_pd);
                })
    ...
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_anggaran, a.no_urut, a.id_kegiatan_pd,
            a.id_indikator_rkpd_final, a.id_indikator_kegiatan, a.id_perubahan,
            a.kd_indikator, a.uraian_indikator_kegiatan, a.tolok_ukur_indikator,
            a.target_renstra, a.target_renja, a.indikator_output, a.id_satuan_output,
...
public function getAktivitas($id_forum_skpd)
{
   $getAktivitas=DB::SELECT('SELECT (@id:=@id+1) as urut, a.id_aktivitas_pd, a.id_pelaksana_pd, a.tahun_anggaran, a.no_urut, 
        ...
        COALESCE(a.id_satuan_1,-1) as id_satuan_1, a.volume_aktivitas_2, a.volume_rkpd_1, a.volume_rkpd_2, a.group_keu,
...
public function getPelaksanaAktivitas($id_aktivitas){
   $getPelaksana=DB::SELECT('SELECT (@id:=@id+1) as urut,a.* FROM (SELECT a.id_pelaksana_pd, a.tahun_anggaran, a.no_urut, a.id_kegiatan_pd, a.id_sub_unit, 
                ...
                CASE a.status_data
                    ...
                    WHEN 1 THEN "green"
                END AS warna 
                FROM trx_anggaran_pelaksana_pd a
                INNER JOIN trx_anggaran_kegiatan_pd f ON a.id_kegiatan_pd = f.id_kegiatan_pd
...
public function postProgRenja(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT a.id_program_pd, a.pagu_anggaran, (COALESCE(a.pagu_anggaran,0) - COALESCE(b.pagu_kegiatan,0)) AS selisih  
            FROM trx_anggaran_program_pd a
        ...
        if($cekRkpd != null && $cekRkpd[0]->status_rkpd == 0){
            ...
            return response ()->json (['pesan'=>'Status Pelaksana APBD telah di Posting','status_pesan'=>'0']);
        }
    }
...
public function postKegRenja(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT c.id_kegiatan_pd, c.status_data, c.pagu_forum, COALESCE(b.jml_pagu_aktivitas,0) as jml_pagu_aktivitas, 
            ...
            FROM (SELECT a.tahun_anggaran, b.id_kegiatan_pd,
                SUM(a.pagu_anggaran) AS jml_pagu_aktivitas
                FROM trx_anggaran_aktivitas_pd a 
...
public function postAktivitas(Request $req)
    {
        ...
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, a.status_data AS status_kegiatan
            FROM trx_anggaran_aktivitas_pd e
            INNER JOIN trx_anggaran_pelaksana_pd d ON e.id_pelaksana_pd = d.id_pelaksana_pd
            INNER JOIN trx_anggaran_kegiatan_pd a ON d.id_kegiatan_pd = a.id_kegiatan_pd
            INNER JOIN trx_anggaran_program_pd b ON a.id_program_pd = b.id_program_pd
        ...
        $cek = DB::SELECT('SELECT a.tahun_anggaran,a.id_aktivitas_pd, SUM(a.pagu_anggaran) as pagu_aktivitas, COALESCE(sum(e.jml_belanja),0) as pagu_belanja, 
            SUM(a.pagu_anggaran)-COALESCE(SUM(e.jml_belanja),0) as selisih, COALESCE(SUM(b.jml_lokasi),0) AS jml_lokasi
            FROM trx_anggaran_aktivitas_pd a
            ...
            LEFT OUTER JOIN (SELECT a.id_aktivitas_pd, Sum(a.jml_belanja) as jml_belanja
            FROM trx_anggaran_belanja_pd AS a GROUP BY a.id_aktivitas_pd) e ON a.id_aktivitas_pd = e.id_aktivitas_pd
            WHERE a.id_aktivitas_pd='.$req->id_aktivitas_pd.' GROUP BY a.tahun_anggaran,a.id_aktivitas_pd');
...
public function addIndikatorProg(Request $req)
    {
            ...
            $data->tahun_anggaran = $req->tahun_renja;
            $data->no_urut = $req->no_urut ;
            $data->id_program_pd = $req->id_program_pd ;
            $data->id_perubahan = 0 ;
