...
class RefPDRBHBController extends Controller
{
	...
	public function index(){
        ...
        return view('dasar.ref_pdrbhb');
    }
    public function getListpdrbhb()
    {
        $getListpdrbhb=DB::select('SELECT (@id:=@id+1) as no_urut,
                ...
                where a.id_tabel_dasar=2 and b.level>0');

        return DataTables::of($getListpdrbhb)
            ->addColumn('action',function($getListpdrbhb){
                    return '
                        <button id="btnEditpdrbhb" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Edit pdrbhb" title="Edit pdrbhb" class="btn btn-primary btn-sm" data-id_isian_tabel_dasar="'.$getListpdrbhb->id_isian_tabel_dasar.'"><i class="fa fa-pencil fa-fw"></i></button>
                        <button id="btnHapuspdrbhb" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Hapus pdrbhb" title="Hapus pdrbhb" class="btn btn-danger btn-sm" data-id_isian_tabel_dasar="'.$getListpdrbhb->id_isian_tabel_dasar.'"><i class="fa fa-trash fa-fw"></i></button>
                    ' ;
            })
            
        ->make(true);
    }
    public function getTahunpdrbhb()
    {
        $getTahun=DB::select('
    ...
    public function getKecamatanpdrbhb()
    {
...
');
    ...
    public function getSektorpdrbhb($tahun,$kecamatan)
    {
        $getSektor=DB::select('
...
where id_kolom_tabel_dasar not in (select id_kolom_tabel_dasar from trx_isian_data_dasar where tahun='.$tahun.' and id_kecamatan='.$kecamatan.')
 and  id_tabel_dasar=2 and level<>0
');
        return json_encode($getSektor);
    }
    public function addpdrbhb (Request $req)
    {
    ...
    public function getEditpdrbhb($id)
    {
        $getListpdrbhb=DB::select('SELECT c.id_isian_tabel_dasar,
c.tahun,
...
where a.id_tabel_dasar=2 and b.level>0 and id_isian_tabel_dasar='.$id);
        
        return DataTables::of($getListpdrbhb)
        ->make(true);
    }
    public function editpdrbhb (Request $req)
    {
    ...
    public function hapuspdrbhb (Request $req)
    {
