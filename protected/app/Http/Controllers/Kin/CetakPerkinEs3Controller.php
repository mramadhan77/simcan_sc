...
class CetakPerkinEs3Controller extends Controller
{
    ...
    public function PerkinEs3BA(Request $request)
    {
        $rules = [
            'tahun'=>'required',
            'kota'=>'required',
        ...
        $messages =[
            ...
            'unit.required'=>'SOTK Level 2 Pelaporan Kosong',
        ];
        ...
        $kaban=DB::select('SELECT a.id_dokumen_perkin, a.id_sotk_es3, a.tahun, a.no_dokumen, a.tgl_dokumen, a.tanggal_mulai, a.id_pegawai, 
                ...
                FROM kin_trx_perkin_es3_dok a
                INNER JOIN ref_sotk_level_2 b ON a.id_sotk_es3=b.id_sotk_es3
                INNER JOIN kin_trx_perkin_opd_dok c ON b.id_sotk_es2 = c.id_sotk_es2 AND a.tahun = c.tahun
                WHERE a.id_sotk_es3='.$request->unit.' AND a.tahun='.$request->tahun.' LIMIT 1');
    ...
    public function PerkinEs3Lamp(Request $request)
    {
        $rules = [
            'tahun'=>'required',
            'kota'=>'required',
        ...
        $html .= '<thead>
            <tr nobr="true" height=19>                
                <td width="50%"  style="padding: 50px; text-align: center; font-weight: bold;" >SASARAN PROGRAM</td>
        ...
        // $sasaran=DB::select('SELECT COALESCE(a.uraian_sasaran_program,"tidak ada data") AS uraian_sasaran_program,
        //     COALESCE(c.nm_indikator,"tidak ada data") AS uraian_indikator_program_renstra,
        ...
        //     LEFT OUTER JOIN trx_renstra_program_indikator b ON a.id_program_renstra=b.id_program_renstra
        ...
        //     INNER JOIN kin_trx_perkin_es3_program e ON a.id_program_renstra = e.id_program_renstra
        //     INNER JOIN kin_trx_perkin_es3_dok f ON e.id_dokumen_perkin = f.id_dokumen_perkin
        //     WHERE f.id_sotk_es3='.$request->unit.' AND f.tahun='.$request->tahun);
        $sasaran=DB::select('SELECT COALESCE(a.uraian_hasil_program,"tidak ada data") AS uraian_sasaran_program,
            COALESCE(c.nm_indikator,"tidak ada data") AS uraian_indikator_program_renstra,
            COALESCE(CASE (SELECT tahun_5 FROM ref_tahun)-'.$request->tahun.'
            WHEN 0 THEN b.angka_tahun5
            WHEN 1 THEN b.angka_tahun4
            ...
            ELSE b.angka_tahun1 end,"tidak ada data") AS angka_tahun,
            COALESCE(d.uraian_satuan,"tidak ada data") AS uraian_satuan
            FROM kin_trx_cascading_program_opd a
            INNER JOIN kin_trx_cascading_indikator_program_pd p ON a.id_hasil_program = p.id_hasil_program
            LEFT OUTER JOIN trx_renstra_program_indikator b ON p.id_renstra_program_indikator=b.id_indikator_program_renstra
            LEFT OUTER JOIN ref_indikator c ON b.kd_indikator=c.id_indikator
            LEFT OUTER JOIN ref_satuan d ON c.id_satuan_output=d.id_satuan
            INNER JOIN kin_trx_perkin_es3_program e ON a.id_renstra_program = e.id_program_renstra
            INNER JOIN kin_trx_perkin_es3_dok f ON e.id_dokumen_perkin = f.id_dokumen_perkin
            WHERE f.id_sotk_es3='.$request->unit.' AND f.tahun='.$request->tahun);
        $html .= '<tbody>';
        ...
        {
            $html.='<tr nobr="true">
                <td width="50%"  style="padding: 50px; text-align: left; font-weight: normal;" >'.$row->uraian_sasaran_program.'</td>
                <td width="40%"  style="padding: 50px; text-align: left; font-weight: normal;" >'.$row->uraian_indikator_program_renstra.'</td>';
        ...
        $program=DB::SELECT('SELECT a.uraian_kegiatan_renstra,
            ...
            INNER JOIN kin_trx_perkin_es3_kegiatan b ON a.id_kegiatan_renstra=b.id_kegiatan_renstra
            INNER JOIN kin_trx_perkin_es3_program c ON b.id_perkin_program = c.id_perkin_program
            INNER JOIN kin_trx_perkin_es3_dok d ON c.id_dokumen_perkin = d.id_dokumen_perkin
            WHERE d.id_sotk_es3='.$request->unit.' AND d.tahun='.$request->tahun);
        $html .= '<tbody>';
        ...
        Template::setFooter();
        PDF::writeHTML($html, true, false, true, false, '');
        PDF::Output('LampPerkinEs3.pdf', 'I');
    }
