...
class CetakUkurTriwEs2Controller extends Controller
{
    ...
    public function UkurTriwEs2Lamp(Request $request)
    {
        $rules = [
            'tahun'=>'required',
            'triwulan'=>'required',
        ...
        $messages =[
            ...
            'unit.required'=>'SOTK Level 1 Pelaporan Kosong',
        ];
        ...
        $sasaran=DB::select('SELECT COALESCE(a.uraian_sasaran_renstra,"tidak ada data") AS uraian_sasaran_renstra,
            COALESCE(c.nm_indikator,"tidak ada data") AS uraian_indikator_sasaran_renstra,
            ...
            INNER JOIN kin_trx_real_es2_sasaran_indikator g ON b.id_indikator_sasaran_renstra = g.id_indikator_sasaran_renstra
            INNER JOIN kin_trx_real_es2_sasaran e ON g.id_real_sasaran = e.id_real_sasaran
            INNER JOIN kin_trx_real_es2_dok f ON e.id_dokumen_real = f.id_dokumen_real
            WHERE f.id_sotk_es2='.$request->unit.' AND f.tahun='.$request->tahun.' AND f.triwulan='.$request->triwulan);
        $html .= '<tbody>';
        foreach($sasaran as $row)
        {
            $html .='<tr nobr="true">';
            $html .='<td width="40%"  style="padding: 50px; text-align: left; font-weight: normal;" >'.$row->uraian_sasaran_renstra.'</td>';
            $html .='<td width="35%"  style="padding: 50px; text-align: left; font-weight: normal;" >'.$row->uraian_indikator_sasaran_renstra.'</td>';
        ...
        $html .= '<thead>
            <tr nobr="true" height=19>            
            <td width="45%"  style="padding: 50px; text-align: center; font-weight: bold;" >PROGRAM</td>
            <td width="25%"  style="padding: 50px; text-align: center; font-weight: bold;" >ANGGARAN (Rp.)</td>
            <td width="25%"  style="padding: 50px; text-align: center; font-weight: bold;" >REALISASI (Rp.) </td>
            <td width="5%"  style="padding: 50px; text-align: center; font-weight: bold;" >%</td>
            </tr></thead>';
        $program=DB::SELECT('SELECT COALESCE(a.uraian_program_renstra,"tidak ada data") AS uraian_program_renstra,
            COALESCE(CASE 4 - '.$request->triwulan.'
            ...
            INNER JOIN kin_trx_real_es3_program e ON a.id_program_renstra = e.id_program_renstra
            INNER JOIN kin_trx_real_es3_dok f ON e.id_dokumen_real = f.id_dokumen_real
            INNER JOIN ref_sotk_level_2 g ON f.id_sotk_es3 = g.id_sotk_es3
            WHERE g.id_sotk_es2='.$request->unit.' AND f.tahun='.$request->tahun.' AND f.triwulan='.$request->triwulan);
        $html .= '<tbody>';
        foreach($program as $row)
        {
            $html.='<tr nobr="true">';
            $html.='<td width="45%"  style="padding: 50px; text-align: left; font-weight: normal;" >'.$row->uraian_program_renstra.'</td>';
        ...
        $kaban=DB::select('SELECT a.id_dokumen_perkin, a.id_sotk_es2, a.tahun, a.no_dokumen, 
                ...
                WHERE a.id_sotk_es2='.$request->unit.' AND a.tahun='.$request->tahun.' LIMIT 1');
        ...
        foreach ($kaban as $row)
        {
            ...
            $html .= '<tr><td width="40%" style="text-align: center;"></td><td width="10%"></td><td width="40%" style="text-align: center;">Pihak Pertama,</td></tr>';
            $html .= '<tr><td width="40%" height="80"></td><td width="10%" height="80"></td><td width="40%" height="80"></td></tr>';
            $html .= '<tr><td width="40%" style="text-align: center;"></td><td width="10%"></td><td width="40%" style="text-align: center;">'.$row->nama_penandatangan.'</td></tr>';
            $html .= '</table>'; 
        }
        $html .= '</body></html>';
