...
class CetakMatrikRenstraController extends Controller
{
  ...
  public function printRenstra($unit, $id_renstra)
  {
    ...
    $data = DB::SELECT('SELECT a.no_urut, a.id_visi_renstra, 
            ...
            INNER JOIN trx_renstra_tujuan b ON e.id_tujuan_renstra = b.id_tujuan_renstra
            INNER JOIN trx_renstra_misi c ON b.id_misi_renstra = c.id_misi_renstra 
            WHERE f.id_program_renstra = r.id_program_renstra) AS level_3,
    ...
    foreach ($data as $row){
        ...
        if ($jum_level_2 <= 1){
            $html.='<td rowspan="'.$row->level_1.'" style="text-align: justify;"></td>';
        ...
        if ($jum_level_5 <= 1){
            $html.='<td rowspan="'.$row->level_2.'" style="text-align: justify;"></td>';
