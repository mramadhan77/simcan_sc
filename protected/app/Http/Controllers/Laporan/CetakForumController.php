...
WHERE h.id_sub_unit=' . $sub_unit . ' and d.id_forum_program=' . $row->id_forum_program . ' and d.id_forum_skpd=' . $row2->id_forum_skpd);
    ...
    public function PrakiraanMajuHtml(Request $request)  
    {
      Template::settingPageLandscape();
      Template::headerLandscape();
      $pemda=Session::get('xPemda');
      ...
      $html .= '<br>';
      $nm_unit = DB::SELECT('SELECT CONCAT(b.kd_urusan,".",b.kd_bidang,".",a.kd_unit) AS kode_bidang, nm_unit from ref_unit AS a
            INNER JOIN ref_bidang b ON a.id_bidang=b.id_bidang WHERE a.id_unit=' . $unit);

      foreach ($nm_unit as $nm_units) {
          PDF::SetFont('helvetica', '', 6);
          $html .= '<div style="text-align: left; font-size:8px; font-weight: bold;"> Nama Perangkat Daerah : '.$nm_units->kode_bidang. '  '. $nm_units->nm_unit . '</div>';
          $html .= '<br>';
          $html .= '<table border="0.5" cellpadding="4" cellspacing="0">
            <thead>
                <tr height=19>
          ...
          $sumunit = DB::SELECT('SELECT  SUM(c.pagu_aktivitas) AS pagu_tahun_current, SUM(b.pagu_plus1_forum) AS pagu_tahun_next
                FROM (SELECT * FROM trx_forum_skpd_program WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS a
                INNER JOIN (SELECT * FROM trx_forum_skpd WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS b 
                ON a.id_forum_program = b.id_forum_program
                INNER JOIN (SELECT c.id_aktivitas_forum AS id_forum_skpd, SUM(d.pagu_aktivitas_forum) AS pagu_aktivitas
                FROM (SELECT * FROM trx_forum_skpd_pelaksana WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS c 
                INNER JOIN (SELECT * FROM trx_forum_skpd_aktivitas WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS d 
                ON c.id_pelaksana_forum=d.id_forum_skpd
                GROUP BY c.id_aktivitas_forum) AS c  ON b.id_forum_skpd = c.id_forum_skpd
                WHERE  a.tahun_forum='.$tahun.' AND a.id_unit='.$unit.' AND a.jenis_belanja=0 
                GROUP BY a.tahun_forum, a.id_unit, a.jenis_belanja');

          foreach ($sumunit as $units) {
              PDF::SetFont('helvetica', 'B', 10);
              $html .= '<tr nobr="true">';
              $html .= '<td width="5%"  style="padding: 50px; text-align: justify;  font-weight: bold;"><div></div></td>';
              $html .= '<td width="50%"  style="padding: 50px; text-align: justify;  font-weight: bold;"><div>TOTAL BELANJA LANGSUNG</div></td>';
              $html .= '<td width="11%"  style="padding: 50px; text-align: right;  font-weight: bold;"><div>' . number_format($units->pagu_tahun_current, 2, ',', '.') . '</div></td>';
              $html .= '<td width="22%"  style="padding: 50px; text-align: justify;  font-weight: bold;"><div></div></td>';
              $html .= '<td width="12%"  style="padding: 50px; text-align: right;  font-weight: bold;"><div>' . number_format($units->pagu_tahun_next, 2, ',', '.') . '</div></td>';
              $html .= '</tr>';

          $urusan = DB::SELECT('SELECT g.kd_urusan,g.nm_urusan 
            FROM (SELECT * FROM trx_forum_skpd_program WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) a
            INNER JOIN ref_program e ON a.id_program_ref=e.id_program
            INNER JOIN ref_bidang f ON e.id_bidang = f.id_bidang
            INNER JOIN ref_urusan g ON f.kd_urusan=g.kd_urusan
            WHERE a.tahun_forum='.$tahun.' AND a.id_unit='.$unit.'  AND a.jenis_belanja=0 
            GROUP BY g.kd_urusan,g.nm_urusan, a.tahun_forum, a.id_unit, a.jenis_belanja');
          ...
          foreach ($urusan as $urusans) {
              ...
              $bidang = DB::SELECT('SELECT f.kd_bidang,f.nm_bidang, SUM(c.pagu_aktivitas) AS pagu_tahun_current, 
                SUM(b.pagu_plus1_forum) AS pagu_tahun_next
                FROM (SELECT * FROM trx_forum_skpd_program WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS a
                INNER JOIN (SELECT * FROM trx_forum_skpd WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS b 
                ON a.id_forum_program = b.id_forum_program
                INNER JOIN (SELECT c.id_aktivitas_forum AS id_forum_skpd, SUM(d.pagu_aktivitas_forum) AS pagu_aktivitas
                ...
                INNER JOIN ref_bidang f ON e.id_bidang = f.id_bidang
                INNER JOIN ref_urusan g ON f.kd_urusan=g.kd_urusan
                WHERE g.kd_urusan=' . $urusans->kd_urusan . ' AND a.tahun_forum='.$tahun.' AND a.id_unit='.$unit.'  AND a.jenis_belanja=0
                GROUP BY f.kd_bidang,f.nm_bidang, g.kd_urusan,a.tahun_forum, a.id_unit,a.jenis_belanja');
              ...
              foreach ($bidang as $bidangs) {
                  ...
                  $html .= '<td width="11%"  style="padding: 50px; text-align: right;  font-weight: bold;"><div>' . number_format($bidangs->pagu_tahun_current, 2, ',', '.') . '</div></td>';
                  $html .= '<td width="22%"  style="padding: 50px; text-align: justify;  font-weight: bold;"><div></div></td>';
                  $html .= '<td width="12%"  style="padding: 50px; text-align: right;  font-weight: bold;"><div>' . number_format($bidangs->pagu_tahun_next, 2, ',', '.') . '</div></td>';
                  $html .= '</tr>';

                  $program = DB::SELECT('SELECT e.kd_program,a.id_forum_program, XML_Encode(e.uraian_program) AS uraian_program, 
                    CONCAT(g.kd_urusan,".",f.kd_bidang,".",e.kd_program) AS kode_program,
                    SUM(c.pagu_aktivitas) AS pagu_tahun_current, 
                    SUM(b.pagu_plus1_forum) AS pagu_tahun_next
                    FROM (SELECT * FROM trx_forum_skpd_program WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS a
                    INNER JOIN (SELECT * FROM trx_forum_skpd WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS b 
                    ON a.id_forum_program = b.id_forum_program
                    ...
                    INNER JOIN ref_bidang f ON e.id_bidang = f.id_bidang
                    INNER JOIN ref_urusan g ON f.kd_urusan=g.kd_urusan                   
                    WHERE g.kd_urusan=' . $urusans->kd_urusan . ' AND f.kd_bidang=' . $bidangs->kd_bidang . ' AND a.tahun_forum='.$tahun.' 
                    AND a.id_unit='.$unit.'  AND a.jenis_belanja=0
                    GROUP BY e.kd_program,a.id_forum_program,e.uraian_program,g.kd_urusan,f.kd_bidang,a.tahun_forum, a.id_unit,a.jenis_belanja');
                  ...
                  foreach ($program as $programs) {
                      $program_ind = DB::SELECT('SELECT DISTINCT e.kd_program, XML_Encode(e.uraian_program) AS uraian_program,  XML_Encode( i.nm_indikator) AS nm_indikator, j.singkatan_satuan, h.target_renja, 
                        ...
                        INNER JOIN ref_bidang f ON e.id_bidang = f.id_bidang
                        INNER JOIN ref_urusan g ON f.kd_urusan=g.kd_urusan
                        INNER JOIN (SELECT * FROM trx_forum_skpd_program_indikator WHERE target_renja > 0 ) h ON a.id_forum_program=h.id_forum_program
                        ...
                        AND a.id_forum_program=' . $programs->id_forum_program . ' AND a.tahun_forum='.$tahun.' AND a.id_unit='.$unit);                       
                      ...
                      foreach ($program_ind as $programs_ind) {
                          ...
                          {
                              $html .= '<tr nobr="true">';
                              $html .= '<td width="5%"  style="padding: 50px; text-align: justify;  font-weight: bold; "><div>' . $programs->kode_program . '</div></td>';
                              ...
                              $html .= '<td width="11%"  style="padding: 50px; text-align: right; "><div>'.number_format($programs->pagu_tahun_current, 2, ',', '.').'</div></td>';
                              ...
                              $html .= '<td width="12%"  style="padding: 50px; text-align: right; "><div>'.number_format($programs->pagu_tahun_next, 2, ',', '.').'</div></td>';
                              $html .= '</tr>';
                          }
                          else {
                              $html .= '<tr nobr="true">';
                          ...
                          $kegiatan = DB::SELECT('SELECT a.id_forum_program, d.id_forum_skpd,  h.nm_kegiatan AS uraian_kegiatan_renstra, 
                            CONCAT(g.kd_urusan,".",f.kd_bidang,".",e.kd_program,".",h.kd_kegiatan) AS kode_kegiatan,
                            h.kd_kegiatan, d.pagu_plus1_forum, p.pagu_aktivitas AS pagu_tahun_kegiatan, q.sumber_dana,  r.lokasi AS lokasi
                            FROM  (SELECT * FROM trx_forum_skpd_program WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3)  a
                            INNER JOIN (SELECT * FROM trx_forum_skpd WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) d 
                                    ON a.id_forum_program = d.id_forum_program
                            LEFT OUTER JOIN (SELECT DISTINCT c.id_aktivitas_forum AS id_forum_skpd, SUM(d.pagu_aktivitas_forum) AS pagu_aktivitas
                                    FROM (SELECT * FROM trx_forum_skpd_pelaksana WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS c 
                                    INNER JOIN (SELECT * FROM trx_forum_skpd_aktivitas WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS d 
                                    ON c.id_pelaksana_forum=d.id_forum_skpd
                                    GROUP BY c.id_aktivitas_forum ) p ON d.id_forum_skpd = p.id_forum_skpd                                                      
                            LEFT OUTER JOIN (SELECT DISTINCT GROUP_CONCAT(" ",q.uraian_sumber_dana) AS sumber_dana,p.id_aktivitas_forum AS id_forum_skpd
                                    FROM (SELECT * FROM trx_forum_skpd_pelaksana WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS p
                                    INNER JOIN (SELECT b.uraian_sumber_dana, a.id_forum_skpd AS id_pelaksana_forum FROM 
                                    (SELECT * FROM trx_forum_skpd_aktivitas WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3)  a
                                    LEFT OUTER JOIN ref_sumber_dana b ON a.sumber_dana=b.id_sumber_dana
                                    GROUP BY a.id_forum_skpd,b.uraian_sumber_dana)  AS q ON p.id_pelaksana_forum=q.id_pelaksana_forum 
                                    GROUP BY p.id_aktivitas_forum) q ON d.id_forum_skpd=q.id_forum_skpd                                                     
                            LEFT OUTER JOIN (SELECT DISTINCT p.id_aktivitas_forum AS id_forum_skpd, q.lokasi
                                    FROM (SELECT * FROM trx_forum_skpd_pelaksana WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS p
                                    INNER JOIN  (SELECT  DISTINCT a.id_forum_skpd AS id_pelaksana_forum, COALESCE(group_concat(c.nama_lokasi),"Kosong") AS lokasi 
                                    FROM (SELECT * FROM trx_forum_skpd_aktivitas WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS a
                                    LEFT OUTER JOIN (SELECT p.* FROM  (SELECT id_pelaksana_forum, id_lokasi FROM trx_forum_skpd_lokasi 
                                    WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS p
                                    GROUP BY p.id_pelaksana_forum, p.id_lokasi) AS b ON a.id_aktivitas_forum=b.id_pelaksana_forum
                                    LEFT OUTER JOIN ref_lokasi c ON b.id_lokasi=c.id_lokasi 
                                    GROUP BY a.id_forum_skpd) AS q ON p.id_pelaksana_forum=q.id_pelaksana_forum) r ON d.id_forum_skpd=r.id_forum_skpd
                            INNER JOIN ref_program e ON a.id_program_ref = e.id_program
                            INNER JOIN ref_bidang f ON e.id_bidang = f.id_bidang
                            INNER JOIN ref_urusan g ON f.kd_urusan = g.kd_urusan
                            LEFT OUTER JOIN ref_kegiatan h ON d.id_kegiatan_ref = h.id_kegiatan                            
                            WHERE g.kd_urusan=' . $urusans->kd_urusan . ' AND f.kd_bidang= ' . $bidangs->kd_bidang . '
                            AND a.tahun_forum='.$tahun.' AND a.id_unit='.$unit.' AND a.id_forum_program=' . $programs->id_forum_program . '  AND a.jenis_belanja=0 
                            ORDER BY  h.kd_kegiatan asc');

                          foreach ($kegiatan as $kegiatans) {
                              $kegiatan_ind = DB::SELECT('SELECT  DISTINCT d.id_forum_skpd, XML_Encode(b.uraian_indikator_kegiatan) AS nm_indikator,b.tolok_ukur_indikator,b.target_renja,m.singkatan_satuan,
                                  CASE b.status_data 
                                  ...
                                  CASE k.tahun_5-2019 
                                      ...
                                      ELSE j.angka_tahun2
                                  END AS target_n1
                                  FROM  (SELECT * FROM trx_forum_skpd WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) d
                                  INNER JOIN (SELECT * FROM trx_forum_skpd_kegiatan_indikator WHERE target_renja > 0) b ON d.id_forum_skpd = b.id_forum_skpd
                                  LEFT OUTER JOIN trx_renstra_kegiatan_indikator j ON d.id_kegiatan_renstra = j.id_kegiatan_renstra AND j.kd_indikator = b.kd_indikator
                                  LEFT OUTER  JOIN ref_tahun k ON d.tahun_forum = k.id_tahun
                                  ...
                                  WHERE d.id_forum_skpd=' . $kegiatans->id_forum_skpd );
                              $b=1;
                              ...
                              foreach ($kegiatan_ind as $kegiatans_ind) {
                                  ...
                                  {
                                      $html .= '<tr nobr="true">';
                                      $html .= '<td width="5%"  style="padding: 50px; text-align: justify; "><div>' . $kegiatans->kode_kegiatan . '</div></td>';
                                      ...
                                      $html .= '<td width="6%"  style="padding: 50px; text-align: justify; "><div>' . $kegiatans->lokasi . '</div></td>';
                                      $html .= '<td width="7%"  style="padding: 50px; text-align: justify; "><div>'.$kegiatans_ind->target_renja.' '.$kegiatans_ind->singkatan_satuan.'</div></td>';
                                      ...
                                      $html .= '<td width="12%"  style="padding: 50px; text-align: right; "><div>'.number_format($kegiatans->pagu_plus1_forum, 2, ',', '.').'</div></td>';
                                      $html .= '</tr>';
                                  }
                                  else {
                                      PDF::SetFont('helvetica', '', 7);
      ...
      $html .= '</table>';
      $html .= '<table border="0" cellpadding="4" cellspacing="0">';
      $pimp=DB::SELECT('SELECT DISTINCT c.nm_unit,a.nama_jabatan_pimpinan_skpd,a.nama_pimpinan_skpd,a.nip_pimpinan_skpd,a.kota_sub_unit 
        ...
        $pagu=DB::SELECT('SELECT sum(p.jml_unit) as total, sum(p.jml_langsung) as langsung, sum(p.jml_tidak_langsung) as tdk_langsung
                FROM (SELECT a.id_unit, a.jenis_belanja, c.pagu_aktivitas AS jml_unit, 
                CASE a.jenis_belanja WHEN 0 THEN c.pagu_aktivitas ELSE 0 END AS jml_langsung, 
                CASE a.jenis_belanja WHEN 1 THEN c.pagu_aktivitas ELSE 0 END AS jml_pendapatan, 
                CASE a.jenis_belanja WHEN 2 THEN c.pagu_aktivitas ELSE 0 END AS jml_tidak_langsung 
                FROM (SELECT * FROM trx_forum_skpd_program WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS a
                INNER JOIN (SELECT * FROM trx_forum_skpd WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS b ON a.id_forum_program = b.id_forum_program
                INNER JOIN (SELECT DISTINCT c.id_aktivitas_forum AS id_forum_skpd, SUM(d.pagu_aktivitas_forum) AS pagu_aktivitas
                                FROM (SELECT * FROM trx_forum_skpd_pelaksana WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS c 
                                INNER JOIN (SELECT * FROM trx_forum_skpd_aktivitas WHERE status_pelaksanaan <> 2 AND status_pelaksanaan <> 3) AS d ON c.id_pelaksana_forum=d.id_forum_skpd
                                GROUP BY c.id_aktivitas_forum) AS c ON b.id_forum_skpd = c.id_forum_skpd
                INNER JOIN ref_unit AS s ON a.id_unit = s.id_unit
                WHERE a.jenis_belanja <> 1 AND a.id_unit ='.$unit.'    AND a.tahun_forum='.$tahun.'
                GROUP BY a.tahun_forum, a.id_unit, a.jenis_belanja, c.pagu_aktivitas) AS p
                GROUP BY p.id_unit');
        foreach ($pagu as $pagus)
          {   
              $html .= '<tr>';
              ...
              $html .= '</tr>';  
              $html .= '<tr>';
              $html .= '<td width="15%"  style="padding: 50px; text-align: left; "><div>Jumlah Belanja Langsung</div></td>';
              $html .= '<td width="5%"  style="padding: 50px; text-align: center; "><div> : </div></td>';
              $html .= '<td width="15%"  style="padding: 50px; text-align: right; "><div>'.number_format($pagus->langsung, 2, ',', '.').'</div></td>';
              $html .= '<td width="35%"  style="padding: 50px; text-align: center; "><div></div></td>';
              $html .= '<td width="30%"  style="padding: 50px; text-align: center; "><div></div></td>';
              $html .= '</tr>';
              $html .= '<tr>';
              $html .= '<td width="15%"  style="padding: 50px; text-align: left; "><div>Jumlah Belanja Tidak Langsung</div></td>';
              $html .= '<td width="5%"  style="padding: 50px; text-align: center; "><div> : </div></td>';
              $html .= '<td width="15%"  style="padding: 50px; text-align: right; "><div>'.number_format($pagus->tdk_langsung, 2, ',', '.').'</div></td>';
              $html .= '<td width="35%"  style="padding: 50px; text-align: center; "><div></div></td>';
              $html .= '<td width="30%"  style="padding: 50px; text-align: center; "><div></div></td>';
              $html .= '</tr>';
              $html .= '<tr>';
              $html .= '<td width="15%"  style="padding: 50px; text-align: left; "><div>Jumlah Pagu Belanja</div></td>';
              $html .= '<td width="5%"  style="padding: 50px; text-align: center; "><div> : </div></td>';
              $html .= '<td width="15%"  style="padding: 50px; text-align: right; "><div>'.number_format($pagus->total, 2, ',', '.').'</div></td>';
              $html .= '<td width="35%"  style="padding: 50px; text-align: center; "><div></div></td>';
              $html .= '<td width="30%"  style="padding: 50px; text-align: center; "><div></div></td>';
              $html .= '</tr>';
          ...
          { 
              $html .= '<tr>';
              $html .= '<td width="70%"  style="padding: 50px; text-align: center; "><div></div></td>';
