...
use Auth;
use App\Models\TrxRenjaRancanganDokumen;


class TrxRenjaRancanganDokController extends Controller
{
    ...
    public function getDataDokumen($id_unit)
    {
        $getDataDokumen = DB::SELECT('SELECT (@id:=@id+1) as no_urut, a.id_dokumen_ranwal,a.nomor_ranwal,a.tanggal_ranwal,a.tahun_ranwal,
                ...
                CASE a.flag
                    ...
                    WHEN 1 THEN "green"
                END AS warna
                FROM trx_renja_rancangan_dokumen AS a
    ...
    {
        // if(Auth::check()){ 
            return view('rancanganrenja.doku');
        // } else {
    ...
    {
        try{
            $data = new TrxRenjaRancanganDokumen;
    ...
    {
        try{
            $data = TrxRenjaRancanganDokumen::find($req->id_dokumen_rkpd);
            $data->nomor_ranwal = $req->nomor_rkpd ;
            $data->tanggal_ranwal = $req->tanggal_rkpd ;
            $data->tahun_ranwal = $req->tahun_rkpd ;
            $data->uraian_perkada = $req->uraian_perkada ;
    ...
    {
        $cekForum=DB::SELECT('SELECT tahun_forum, id_unit, COALESCE(count(id_renja_program),0) as jml FROM trx_forum_skpd_program WHERE tahun_forum = '.$req->tahun_rkpd.' 
        ...
        if($req->flag==1){
                if($CheckProgram[0]->jml_1==0){
                    ...
                    $data = DB::UPDATE('UPDATE trx_renja_rancangan_dokumen SET flag ='.$req->flag.' WHERE tahun_ranwal='.$req->tahun_rkpd.' AND id_dokumen_ranwal='.$req->id_dokumen_rkpd);
                       
                    if($data != 0){
                        $dataProg=DB::UPDATE('UPDATE trx_renja_rancangan_program SET status_data ='.$req->status.', id_dokumen ='.$req->id_dokumen_rkpd.' WHERE tahun_renja='.$req->tahun_rkpd.' AND status_data='.$req->status_awal.' AND id_unit='.$req->id_unit);
            ...
            $data = DB::UPDATE('UPDATE trx_renja_rancangan_dokumen SET flag ='.$req->flag.' WHERE tahun_ranwal='.$req->tahun_rkpd.' AND id_dokumen_ranwal='.$req->id_dokumen_rkpd);                       
                ...
                return response ()->json (['pesan'=>'Data Gagal Proses, Dokumen sudah dipakai di - Forum OPD (0cdrPD)','status_pesan'=>'0']);
            }
        }
    }
    ...
    {
        $result = TrxRenjaRancanganDokumen::destroy($req->id_dokumen_rkpd);
