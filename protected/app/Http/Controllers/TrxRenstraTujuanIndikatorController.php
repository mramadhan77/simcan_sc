...
use App\Models\Can\TrxRenstraTujuanIndikator;

class TrxRenstraTujuanIndikatorController extends Controller
{
    ...
    public function getIndikatorTujuan($id_tujuan_renstra)
    {
      $indikator=DB::SELECT('SELECT (@id:=@id+1) AS urut,a.thn_id, a.no_urut, a.id_tujuan_renstra, a.id_indikator_tujuan_renstra, a.id_perubahan, 
      a.kd_indikator, a.uraian_indikator_sasaran_renstra, a.tolok_ukur_indikator, a.angka_awal_periode, a.angka_tahun1, a.angka_tahun2, a.angka_tahun3, 
      a.angka_tahun4, a.angka_tahun5, a.angka_akhir_periode, a.created_at, a.updated_at,
      ...
      LEFT OUTER JOIN ref_satuan AS c ON b.id_satuan_output = c.id_satuan,(SELECT @id:=0) x  WHERE a.id_tujuan_renstra='.$id_tujuan_renstra);
    ...
    public function addIndikator(Request $request)
    {
        ...
        $messages =[
            ...
            'id_tujuan_renstra.required'=> 'No Tujuan Kosong',
		...
		if($validation->fails()) {
			...
			$data = new TrxRenstraTujuanIndikator();
    ...
    public function editIndikator(Request $request)
    {
        ...
        $messages =[
            'id_indikator_tujuan_renstra.required' => 'Id Indikator Kosong',
            'thn_id.required' => 'Tahun Kosong',
            'no_urut.required' => 'No Urut Kosong',
            'id_tujuan_renstra.required'=> 'No Tujuan Kosong',
            'id_perubahan.required' => 'Id Perubahan Kosong',
		...
		if($validation->fails()) {
            ...
            $data = TrxRenstraTujuanIndikator::find($request->id_indikator_tujuan_renstra);
            $data->thn_id= $request->thn_id;
            $data->no_urut= $request->no_urut;
            $data->id_tujuan_renstra= $request->id_tujuan_renstra;
            $data->id_perubahan= $request->id_perubahan;
    ...
    public function delIndikator(Request $request){
		...
		if($validation->fails()) {
            ...
            $data = TrxRenstraTujuanIndikator::where('id_indikator_tujuan_renstra',$request->id_indikator_tujuan_renstra)->delete();
