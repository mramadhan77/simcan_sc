...
class KirimXmlRancanganController extends Controller
{
...
XML_Encode(GantiEnter(x.uraian_misi_renstra)) as ur_misi
 from trx_renstra_tujuan y 
 INNER JOIN trx_renstra_misi x on x.id_misi_renstra = y.id_misi_renstra 
  INNER JOIN trx_renstra_visi z on z.id_visi_renstra= x.id_visi_renstra 
INNER JOIN ref_unit i on z.id_unit=i.id_unit 
INNER JOIN ref_sub_unit h on h.id_unit=i.id_unit 
 INNER JOIN ref_bidang j on i.id_bidang=j.id_bidang 
 INNER JOIN ref_urusan k ON j.kd_urusan = k.kd_urusan 
 where  x.no_urut not in (98,99) 
order by k.kd_urusan, j.kd_bidang, i.kd_unit, 
 h.kd_sub, x.no_urut, y.no_urut
           
   ');
        /*$TaTujuan = DB::SELECT('SELECT DISTINCT YEAR(CURDATE()) as Tahun, k.kd_urusan, j.kd_bidang, i.kd_unit, 
 h.kd_sub, x.no_urut as no_misi, y.no_urut as no_tujuan, XML_Encode(y.uraian_tujuan_renstra) as ur_tujuan 
...
where a.tahun_renja = 2019 and x.no_urut not in (98,99) 
   ');*/
   $TaTujuan = DB::SELECT('SELECT DISTINCT YEAR(CURDATE()) as Tahun,x.id_misi_renstra, k.kd_urusan, j.kd_bidang, i.kd_unit, 
 h.kd_sub, x.no_urut as no_misi, y.no_urut as no_tujuan, XML_Encode(y.uraian_tujuan_renstra) as ur_tujuan 
 from trx_renstra_tujuan y 
 INNER JOIN trx_renstra_misi x on x.id_misi_renstra = y.id_misi_renstra 
  INNER JOIN trx_renstra_visi z on z.id_visi_renstra= x.id_visi_renstra 
...
order by k.kd_urusan, j.kd_bidang, i.kd_unit, 
 ...
 XML_Encode(z.uraian_sasaran_renstra) as ur_sasaran from trx_renstra_tujuan y 
 INNER JOIN trx_renstra_misi x on x.id_misi_renstra = y.id_misi_renstra 
  INNER JOIN trx_renstra_visi a on a.id_visi_renstra= x.id_visi_renstra 
INNER JOIN ref_unit i on a.id_unit=i.id_unit 
INNER JOIN ref_sub_unit h on h.id_unit=i.id_unit 
 INNER JOIN ref_bidang j on i.id_bidang=j.id_bidang 
 INNER JOIN ref_urusan k ON j.kd_urusan = k.kd_urusan  
 INNER JOIN trx_renstra_sasaran z on y.id_tujuan_renstra = z.id_tujuan_renstra 
 WHERE x.no_urut not in (98,99) 
   ');
...
FROM
	(
		SELECT
			...
			a.sumber_aktivitas as sumber_belanja,
			concat_WS(
				" ",
			...
			COALESCE (e.pagu_tahun_renstra) AS Pagu_Anggaran
		FROM
			trx_renja_rancangan_belanja a
		INNER JOIN trx_renja_rancangan_aktivitas b ON a.id_lokasi_renja = b.id_aktivitas_renja
		LEFT OUTER JOIN (
			SELECT
				id_pelaksana_renja,
				GROUP_CONCAT(" ", uraian_lokasi) AS uraian_lokasi
			FROM
				trx_renja_rancangan_lokasi
			GROUP BY
				id_pelaksana_renja
		) AS ab ON ab.id_pelaksana_renja = b.id_aktivitas_renja
		INNER JOIN trx_renja_rancangan_pelaksana d ON b.id_renja = d.id_pelaksana_renja
		INNER JOIN trx_renja_rancangan e ON d.id_renja = e.id_renja
		INNER JOIN (
			SELECT
				d.Kd_Urusan,
				c.Kd_Bidang,
		...
		INNER JOIN trx_renja_rancangan_program g ON e.id_renja_program = g.id_renja_program
		LEFT JOIN trx_renja_rancangan_program_indikator gx ON g.id_renja_program = gx.id_renja_program
...
GROUP BY
   ...
   ');
	...
	a.id_prog,
	a.Kd_Keg,
	min(ket_kegiatan) AS ket_kegiatan,
	min(Lokasi) AS Lokasi,
	min(Kelompok_Sasaran) AS Kelompok_Sasaran,
	max(Status_Kegiatan) AS Status_Kegiatan,
	sum(Pagu_Anggaran) AS Pagu_Anggaran,
	min(Waktu_Pelaksanaan) AS Waktu_Pelaksanaan,
	min(Kd_Sumber) AS Kd_Sumber
FROM
	(
		SELECT
			a.tahun,
		...
		WHEN a.Kd_Keg = 0 THEN
			"Non Kegiatan"
		ELSE
			min(XML_Encode(a.nm_kegiatan))
		END AS ket_kegiatan,
		-- ifnull(min(a.uraian_lokasi),"-") as Lokasi, 
		CASE
	WHEN length(min(a.uraian_lokasi)) > 775 THEN
		concat(
			substring(min(a.uraian_lokasi), 1, 775),
			" ... dst"
		)
	ELSE
		MIN(a.uraian_lokasi)
...
FROM
	(
		SELECT
			...
			COALESCE (a.jml_belanja, 0) AS jml_belanja,
			a.status_data,
		...
		LEFT OUTER JOIN (
			SELECT
				a.id_pelaksana_renja,
				min(b.id_lokasi) AS id_lokasi,
				GROUP_CONCAT(" ", b.nama_lokasi) AS uraian_lokasi
			FROM
				trx_renja_rancangan_lokasi a
...
END AS lokasi,
 "-" AS Kelompok_Sasaran,
 max(a.jenis_kegiatan) AS Status_Kegiatan,
 sum(
	COALESCE (a.Pagu_Anggaran, 0)
) AS Pagu_Anggaran,
 NULL AS Waktu_Pelaksanaan,
...
FROM
	(
		SELECT
			...
			a.angka_tahun AS Target_Angka,
			COALESCE (a.uraian_satuan, "-") AS Target_Uraian,
			XML_Encode(a.uraian_kegiatan_renstra) as uraian_kegiatan_forum,
			a.id_indikator_kegiatan_renja as id_indikator_kegiatan,
			a.no_urut,
			a.id_kegiatan
		FROM
			(
				SELECT
					ik.id_renja,
					ik.id_indikator_kegiatan_renja,
					ik.kd_indikator,
					ik.uraian_indikator_kegiatan_renja,
					SUBSTRING(
				...
				INNER JOIN trx_renja_rancangan_pelaksana d ON b.id_renja = d.id_pelaksana_renja
				INNER JOIN trx_renja_rancangan e ON d.id_renja = e.id_renja
				INNER JOIN trx_renja_rancangan_indikator AS ik ON ik.id_renja = e.id_renja
				...
				AND e.status_pelaksanaan_kegiatan = 0 -- // jenis_belanja    0=BL, 1=pdpt, 2=BTL
				-- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
			) a
		GROUP BY
			a.tahun,
...
FROM
	(
		...
		FROM
			...
			INNER JOIN trx_renja_rancangan_pelaksana d ON b.id_renja = d.id_pelaksana_renja
			INNER JOIN trx_renja_rancangan e ON d.id_aktivitas_renja = e.id_renja
			INNER JOIN trx_renja_rancangan_indikator AS ik ON ik.id_renja = e.id_renja
			INNER JOIN (
				SELECT
					d.Kd_Urusan,
			...
			AND e.status_pelaksanaan_kegiatan = 0 -- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
			) a
		GROUP BY
			a.tahun,
			a.Kd_Urusan,
		...
		FROM
			(
				SELECT
					...
					gx.id_indikator_program_renja as id_indikator_program,
					f.id_program
				FROM
					trx_renja_rancangan_belanja a
				INNER JOIN trx_renja_rancangan_aktivitas b ON a.id_lokasi_renja = b.id_aktivitas_renja
				...
				AND e.status_pelaksanaan_kegiatan = 0 --    // jenis_belanja=1 ==> pendapatan    0=BL, 1=pdpt, 2=BTL
				-- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
				ORDER BY
					k.Kd_Urusan,
					j.Kd_Bidang,
...
FROM
	(
		...
		WHERE
		 ...
		 e.status_pelaksanaan_kegiatan NOT IN (2, 3, 5)
		AND a.tahun_renja = 2019 --  +jika+
		-- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
	) a  where concat(a.Kd_Urusan,".", a.Kd_Bidang,".", a.kd_unit)="'.$unit.'"
GROUP BY
	a.tahun,
	a.Kd_Urusan,
	a.Kd_Bidang,
...
HAVING
	a.kd_rek_1 = 5
 ');
        $BelanjaHistory=DB::SELECT('SELECT
	a.tahun,
	a.Kd_Urusan,
	a.Kd_Bidang,
	a.kd_unit,
	...
	a.kd_rek_5,
	0 AS Nilai_Lalu,
	sum(pagu_tahun_selanjutnya) AS Nilai_YAD,
	0 AS Nilai_YAD_2,
	0 AS Nilai_YAD_3
FROM
	(
...
HAVING
	a.kd_rek_1 = 5');
 $BelanjaItem=DB::Select(' SELECT
	a.id_belanja_renja AS id_belanja_renja,
	b.group_keu,
	substring(
		XML_Encode (
			GantiEnter (
	...
	l.kd_rek_4,
	l.kd_rek_5,
	l.nama_kd_rek_5 AS nm_rek_5,
	b.id_aktivitas_renja as id_aktivitas_forum,
	substring(
		XML_Encode (
			GantiEnter (
				b.uraian_aktivitas_kegiatan
	...
	a.volume_1 AS Nilai_1,
	substring(dd.uraian_satuan, 1, 10) AS Sat_2,
	a.volume_2 AS Nilai_2,
	NULL AS Sat_3,
	0 AS Nilai_3,
	CASE
WHEN substring(dd.uraian_satuan, 1, 10) IS NULL THEN
	CONCAT(
		COALESCE (
			substring(cc.uraian_satuan, 1, 10),
			"-"
		),
		COALESCE (
			substring(dd.uraian_satuan, 1, 10),
			""
		)
	)
ELSE
...
END AS Satuan123,
 CASE
WHEN a.volume_2 = 0 THEN
	a.volume_1
ELSE
	(
		a.volume_1 * a.volume_2
	)
END AS Jml_Satuan,
 a.harga_satuan AS Nilai_Rp,
 a.jml_belanja AS Total,
 substring(
	XML_Encode (
		GantiEnter (
			CASE
			WHEN aa.uraian_tarif_ssh = a.uraian_belanja THEN
				aa.uraian_tarif_ssh
			WHEN a.uraian_belanja = "-" THEN
				aa.uraian_tarif_ssh
			WHEN COALESCE (a.uraian_belanja, " ") = " " THEN
				aa.uraian_tarif_ssh
			ELSE
				CONCAT(
					aa.uraian_tarif_ssh,
					" (",
					COALESCE (a.uraian_belanja, " "),
					")"
				)
			END
		)
 ...
 b.sumber_aktivitas,
 b.id_aktivitas_renja,
 a.id_aktivitas_asb,
 aa.id_tarif_ssh
FROM
	trx_renja_rancangan_belanja a
INNER JOIN ref_ssh_tarif aa ON a.id_tarif_ssh = aa.id_tarif_ssh
LEFT OUTER JOIN ref_satuan cc ON (
	a.id_satuan_1 = cc.id_satuan
)
LEFT OUTER JOIN ref_satuan dd ON (
	a.id_satuan_2 = dd.id_satuan
)
INNER JOIN trx_renja_rancangan_aktivitas b ON a.id_lokasi_renja = b.id_aktivitas_renja
INNER JOIN trx_renja_rancangan_pelaksana d ON b.id_renja = d.id_pelaksana_renja
INNER JOIN trx_renja_rancangan e ON d.id_renja = e.id_renja
INNER JOIN (
	...
	INNER JOIN ref_bidang c ON b.id_bidang = c.id_bidang
	INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan
) f ON e.id_kegiatan_ref = f.id_kegiatan
INNER JOIN trx_renja_rancangan_program g ON e.id_renja_program = g.id_renja_program
INNER JOIN ref_sub_unit h ON d.id_sub_unit = h.id_sub_unit
INNER JOIN ref_unit i ON h.id_unit = i.id_unit
INNER JOIN ref_bidang j ON i.id_bidang = j.id_bidang
...
HAVING
 ...
 $PendRinc = DB::SELECT('SELECT
	...
	a.volume_1 AS Nilai_1,
	a.Sat_2,
	COALESCE (a.volume_2, 0) AS Nilai_2,
	NULL AS Sat_3,
	1 AS Nilai_3,
	CASE
...
FROM
	(
		SELECT
			...
			a.harga_satuan,
			a.jml_belanja,
			COALESCE (a.volume_1, 0) AS volume_1,
			a.id_satuan_1,
			COALESCE (a.volume_2, 0) AS volume_2,
			a.id_satuan_2,
			...
			COALESCE (e.pagu_tahun_kegiatan, 0) AS pagu_tahun_kegiatan,
			b.sumber_dana,
			COALESCE (e.pagu_tahun_renstra, 0) AS Pagu_Anggaran,
			substring(cc.uraian_satuan, 1, 10) AS Sat_1,
