...
use App\Models\TrxRpjmdRanwalDokumen;
use App\Models\TrxRpjmdRanwalVisi;
use App\Models\TrxRpjmdRanwalMisi;
use App\Models\TrxRpjmdRanwalTujuan;
use App\Models\TrxRpjmdRanwalSasaran;
use App\Models\TrxRpjmdRanwalKebijakan;
use App\Models\TrxRpjmdRanwalStrategi;
use App\Models\TrxRpjmdRanwalProgram;
use App\Models\TrxRpjmdRanwalProgramIndikator;
use App\Models\TrxRpjmdRanwalProgramUrusan;
use App\Models\TrxRpjmdRanwalProgramPelaksana;

class TrxRpjmdRancanganController extends Controller
{
    ...
    public function getDokumen()
    {
        ...
        $dataperdarpjmd=DB::select('SELECT (@id:=@id+1) as no_urut, id_rpjmd, periode_awal, periode_akhir, no_perda, keterangan_dokumen, tgl_perda, id_revisi, id_status_dokumen, CASE id_revisi
                WHEN 0 THEN "Pra RPJMD"
                WHEN 1 THEN "Dokumen Induk"
                WHEN 2 THEN "Revisi 1"
                WHEN 3 THEN "Revisi 2"
                WHEN 4 THEN "Revisi 3"
                END AS display_revisi, 
                     CASE id_status_dokumen
                WHEN 0 THEN "Draft"
                WHEN 1 THEN "Aktif"
                END AS display_status,
                CONCAT(periode_awal," s.d ",periode_akhir) as display_periode,
                created_at, update_at
        FROM trx_rpjmd_ranwal_dokumen, (SELECT @id:=0) b');
        return DataTables::of($dataperdarpjmd)
        ->addColumn('action', function ($dataperdarpjmd) {
            return '                         
            <div class="btn-group">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                        ...
                        <li>
                            <a class="view-dokumen dropdown-item"><i class="fa fa-book fa-fw fa-lg"></i> Rincian Dokumen</a>
                        </li>
                        <li>
                            <a class="hapus-dokumen dropdown-item"><i class="fa fa-trash-o fa-fw fa-lg"></i> Hapus Dokumen</a>
                        </li>
                        <li>
                            <a class="post-dokumen dropdown-item"><i class="fa fa-check-square-o fa-fw fa-lg"></i> Posting Dokumen</a>
                        </li>                          
                    </ul>
                </div>
            ';
    ...
    public function addDokRpjmd(Request $req)
    {
        $data = new TrxRpjmdRanwalDokumen;
        $data->periode_awal= $req->periode_awal;
        $data->periode_akhir= $req->periode_akhir;
        $data->no_perda= $req->no_perda;
        $data->keterangan_dokumen= $req->keterangan_dokumen;
        $data->tgl_perda= $req->tgl_perda;
        $data->id_revisi= $req->id_revisi;
    ...
    public function editDokRpjmd(Request $req)
    {    
        $data = TrxRpjmdRanwalDokumen::find($req->id_rpjmd);
        $data->periode_awal= $req->periode_awal;
        $data->periode_akhir= $req->periode_akhir;
        $data->no_perda= $req->no_perda;
        $data->keterangan_dokumen= $req->keterangan_dokumen;
    ...
    public function postDokRpjmd(Request $req)
    {
        $data = TrxRpjmdRanwalDokumen::find($req->id_rpjmd);
        $data->id_status_dokumen= 1;
    ...
    public function delDokRpjmd(Request $req)
    {
        $result = TrxRpjmdRanwalDokumen::where('id_rpjmd',$req->id_rpjmd)->delete();
        if($result!=0){
    ...
    {
      $rpjmdtujuan = DB::select('SELECT c.thn_id_rpjmd, c.no_urut, c.id_misi_rpjmd, c.id_tujuan_rpjmd, c.id_perubahan, c.uraian_tujuan_rpjmd, b.id_visi_rpjmd, b.id_misi_rpjmd, b.no_urut as id_misi
    ...
    public function getSasaranRPJMD($id_tujuan_rpjmd)
    {
      $rpjmdsasaran = DB::select('SELECT d.thn_id_rpjmd, d.no_urut, d.id_tujuan_rpjmd, d.id_sasaran_rpjmd, d.id_perubahan, d.uraian_sasaran_rpjmd, b.id_visi_rpjmd,
                        b.id_misi_rpjmd, b.no_urut as id_misi, c.no_urut as id_tujuan
                        FROM trx_rpjmd_visi AS a
                        INNER JOIN trx_rpjmd_misi AS b ON b.id_visi_rpjmd = a.id_visi_rpjmd
                        INNER JOIN trx_rpjmd_tujuan AS c ON c.id_misi_rpjmd = b.id_misi_rpjmd
            	...
            	<button type="button" class="btn btn-info dropdown-toggle btn-labeled" aria-haspopup="true" aria-expanded="false" data-toggle="dropdown"><span class="btn-label"><i class="fa fa-wrench fa-fw fa-lg"></i></span>Aksi <span class="caret"></span></button>
            		...
            		<ul class="dropdown-menu dropdown-menu-right">
                        ...
                        <a class="edit-program dropdown-item" data-id_program_rpjmd_program="'.$rpjmdprogram->id_program_rpjmd.'" data-thn_id_program="'.$rpjmdprogram->thn_id.'" data-id_sasaran_rpjmd_program="'.$rpjmdprogram->id_sasaran_rpjmd.'" data-id_perubahan_program="'.$rpjmdprogram->id_perubahan.'"  data-uraian_program_rpjmd_program="'.$rpjmdprogram->uraian_program_rpjmd.'" data-no_urut_program="'.$rpjmdprogram->no_urut.'"  data-pagu_tahun1_program="'.$rpjmdprogram->pagu_tahun1a.'" data-pagu_tahun2_program="'.$rpjmdprogram->pagu_tahun2a.'" data-pagu_tahun3_program="'.$rpjmdprogram->pagu_tahun3a.'" data-pagu_tahun4_program="'.$rpjmdprogram->pagu_tahun4a.'" data-pagu_tahun5_program="'.$rpjmdprogram->pagu_tahun5a.'" ><i class="fa fa-pencil fa-fw fa-lg text-success"></i> Lihat Data Program</a>
							         <a class="view-rpjmdindikator dropdown-item" data-id_program="'.$rpjmdprogram->id_program_rpjmd.'" ><i class="fa fa-bullseye fa-fw fa-lg text-primary"></i> Lihat Indikator</a>
