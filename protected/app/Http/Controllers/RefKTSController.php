...
class RefKTSController extends Controller
{
    ...
    public function index(){
        ...
        return view('dasar.ref_kts');
    }
    public function getListkts()
    {
        $getListkts=DB::select('SELECT (@id:=@id+1) as no_urut,
...
, (SELECT @id:=0) x
where a.id_tabel_dasar=7 and b.level>0
');
        
        return DataTables::of($getListkts)
        ->addColumn('action',function($getListkts){
            return '
                        <button id="btnEditkts" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Edit Rasio Ketersediaan Sekolah" title="Edit Rasio Ketersediaan Sekolah" class="btn btn-primary btn-sm" data-id_isian_tabel_dasar="'.$getListkts->id_isian_tabel_dasar.'"><i class="fa fa-pencil fa-fw"></i></button>
                        <button id="btnHapuskts" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Hapus Rasio Ketersediaan Sekolah" title="Hapus Rasio Ketersediaan Sekolah" class="btn btn-danger btn-sm" data-id_isian_tabel_dasar="'.$getListkts->id_isian_tabel_dasar.'"><i class="fa fa-trash fa-fw"></i></button>
                    ' ;
        })
        
        ->make(true);
    }
    public function getTahunkts()
    {
        $getTahun=DB::select('
...
');
    ...
    public function getKecamatankts()
    {
...
where id_kolom_tabel_dasar not in (select id_kolom_tabel_dasar from trx_isian_data_dasar where tahun='.$tahun.' and id_kecamatan='.$kecamatan.')
 and  id_tabel_dasar=7 and level<>0 and parent_id='.$tingkat);
        return json_encode($getSektor);
    }
    
    public function addkts (Request $req)
    {
    ...
    public function getEditkts($id)
    {
        $getListkts=DB::select('SELECT c.id_isian_tabel_dasar,
c.tahun,
...
LEFT OUTER JOIN trx_isian_data_dasar AS c ON c.id_kolom_tabel_dasar = b.id_kolom_tabel_dasar
LEFT OUTER JOIN ref_kecamatan AS d ON c.id_kecamatan = d.id_kecamatan
where a.id_tabel_dasar=7 and b.level>0 and id_isian_tabel_dasar='.$id);
        
        return DataTables::of($getListkts)
        ->make(true);
    }
    public function editkts (Request $req)
    {
    ...
    public function hapuskts (Request $req)
    {
