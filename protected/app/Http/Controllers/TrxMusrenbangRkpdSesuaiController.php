...
use App\Models\Can\TrxMusrenkabDokumen;
use App\Models\Can\TrxMusrenkab;
use App\Models\Can\TrxMusrenkabIndikator;
use App\Models\Can\TrxMusrenkabUrusan;
use App\Models\Can\TrxMusrenkabPelaksana;
use App\Models\Can\TrxMusrenkabProgramPd;
use App\Models\Can\TrxMusrenkabProgIndikatorPd;
use App\Models\Can\TrxMusrenkabKegiatanPd;
use App\Models\Can\TrxMusrenkabKegIndikatorPd;
use App\Models\Can\TrxMusrenkabPelaksanaPd;
use App\Models\Can\TrxMusrenkabAktivitasPd;
use App\Models\Can\TrxMusrenkabLokasiPd;
use App\Models\Can\TrxMusrenkabBelanjaPd;
...
class TrxMusrenbangRkpdSesuaiController extends Controller
{
    ...
    {
        $dataForum = TrxMusrenkabKegiatanPd::paginate(15);
    }
    ...
    {
        $getRenja=DB::SELECT('SELECT (@id:=@id+1) as no_urut, a.id_musrenkab, a.id_rkpd_ranwal, a.tahun_rkpd, a.id_program_rpjmd, 
            ...
            FROM trx_musrenkab_program_pd AS x 
            WHERE x.id_pelaksana_rkpd = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            GROUP BY x.id_pelaksana_rkpd, x.tahun_forum, x.id_unit ),0)) as jml_program, 
            SUM(COALESCE((SELECT COUNT(y.id_kegiatan_pd) AS jml_kegiatan 
            FROM trx_musrenkab_program_pd AS x 
            INNER JOIN trx_musrenkab_kegiatan_pd AS y ON x.id_program_pd = y.id_program_pd
            WHERE x.id_pelaksana_rkpd = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            GROUP BY x.id_pelaksana_rkpd, x.tahun_forum, x.id_unit ),0)) as jml_kegiatan,
            SUM(COALESCE((SELECT SUM(y.pagu_forum) AS jml_pagu 
            FROM trx_musrenkab_program_pd AS x 
            INNER JOIN trx_musrenkab_kegiatan_pd AS y ON x.id_program_pd = y.id_program_pd
            WHERE x.id_pelaksana_rkpd = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            GROUP BY x.id_pelaksana_rkpd, x.tahun_forum, x.id_unit),0)) as jml_pagu,
            SUM(COALESCE((SELECT SUM(r.pagu_aktivitas_forum) AS jml_pagu_aktivitas 
            FROM trx_musrenkab_program_pd AS x 
            INNER JOIN trx_musrenkab_kegiatan_pd AS y ON x.id_program_pd = y.id_program_pd
            INNER JOIN trx_musrenkab_pelaksana_pd AS z ON y.id_kegiatan_pd = z.id_kegiatan_pd
            INNER JOIN trx_musrenkab_aktivitas_pd AS r ON z.id_pelaksana_pd = r.id_pelaksana_pd
            WHERE x.id_pelaksana_rkpd = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            GROUP BY x.id_pelaksana_rkpd, x.tahun_forum, x.id_unit),0)) as jml_pagu_aktivitas, 
            ...
            INNER JOIN trx_musrenkab_aktivitas_pd AS r ON z.id_pelaksana_pd = r.id_pelaksana_pd
            WHERE x.id_pelaksana_rkpd = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            GROUP BY x.id_pelaksana_rkpd, x.tahun_forum, x.id_unit),0)) as jml_aktivitas,
            CASE a.status_data
            ...
            INNER JOIN trx_musrenkab_urusan AS d ON a.id_musrenkab = d.id_musrenkab
            INNER JOIN trx_musrenkab_pelaksana AS b ON d.id_urusan_rkpd = b.id_urusan_rkpd, 
            (SELECT @id:=0) AS p WHERE a.tahun_rkpd = '.$tahun.' AND b.id_unit ='.$unit.'            
            GROUP BY a.id_musrenkab, a.id_rkpd_ranwal, a.tahun_rkpd, a.id_program_rpjmd, 
            b.id_unit, a.uraian_program_rpjmd, a.pagu_rpjmd, a.pagu_ranwal, a.keterangan_program, a.jenis_belanja,
            a.status_data, a.sumber_data');
          ...
          return DataTables::of($getRenja)
            ...
            ->addColumn('details_url', function($getRenja) {
                    return url('musrenrkpd/getChildBidang/'.$getRenja->id_unit.'/'. $getRenja->id_musrenkab);
                })
            ->make(true);
    }
    ...
    {
        $getRenja = DB::SELECT('SELECT DISTINCT '.Session::get('tahun').' AS tahun_forum, b.id_musrenkab,d.kd_bidang,d.nm_bidang,c.id_unit,
                ...
                FROM trx_musrenkab a
                INNER JOIN trx_musrenkab_urusan b ON a.id_musrenkab = b.id_musrenkab
                INNER JOIN trx_musrenkab_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
                INNER JOIN ref_bidang d ON b.id_bidang = d.id_bidang
                WHERE c.id_unit='. $id_unit.' AND b.id_musrenkab='.$id_rkpd_rancangan.'
                ORDER BY b.id_musrenkab,d.kd_bidang,c.id_unit, b.id_bidang');
    ...
    {
      $getProgramRenja=DB::select('SELECT (@id:=@id+1) as urut, a.* FROM (SELECT a.id_program_pd, c.id_pelaksana_rkpd, c.id_musrenkab, a.tahun_forum, a.no_urut, a.id_unit, b.nm_unit, a.id_renja_program, 
            ...
            COALESCE((SELECT COUNT(x.id_kegiatan_pd) AS jml_kegiatan
                        FROM trx_musrenkab_kegiatan_pd AS x
            ...
            FROM trx_musrenkab_program_pd a
            INNER JOIN ref_unit b ON a.id_unit = b.id_unit 
            INNER JOIN trx_musrenkab_pelaksana c ON a.id_pelaksana_rkpd = c.id_pelaksana_rkpd
      ...
      return DataTables::of($getProgramRenja)
        ->addColumn('details_url', function($getProgramRenja) {
                    return url('musrenrkpd/getIndikatorRenja/'.$getProgramRenja->id_program_pd);
                })
    ...
    public function getIndikatorRenja($id_rkpd)
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_renja, a.no_urut, a.id_program_pd, a.id_program_renstra,
                ...
                FROM trx_musrenkab_prog_indikator_pd AS a  
                INNER JOIN trx_musrenkab_program_pd AS b ON a.id_program_pd=b.id_program_pd
    ...
    public function getKegiatanRenja($id_program)
    {
      $getKegiatanRenja=DB::select('SELECT (@id:=@id+1) as urut, a.* FROM (SELECT a.id_kegiatan_pd, a.id_program_pd, a.id_unit, 
            ...
            WHEN 1 THEN "green"
            END AS warna,
            COALESCE((SELECT SUM(x.pagu_forum) FROM trx_musrenkab_kegiatan_pd x WHERE x.id_kegiatan_pd=a.id_kegiatan_pd 
                GROUP BY x.id_kegiatan_pd),0) AS jml_pagu,
            COALESCE((SELECT COUNT(x.id_aktivitas_forum) as jml_aktivitas
            FROM trx_musrenkab_aktivitas_pd x 
            INNER JOIN trx_musrenkab_pelaksana_pd y ON x.id_pelaksana_pd = y.id_pelaksana_pd
            ...
            FROM trx_musrenkab_kegiatan_pd a
            INNER JOIN trx_musrenkab_program_pd AS x ON a.id_program_pd=x.id_program_pd
      ...
      ->addColumn('details_url', function($getKegiatanRenja) {
                    return url('musrenrkpd/getIndikatorKegiatan/'.$getKegiatanRenja->id_kegiatan_pd);
                })
    ...
    public function getIndikatorKegiatan($id_rkpd)
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_renja, a.no_urut, a.id_kegiatan_pd,
                ...
                FROM trx_musrenkab_keg_indikator_pd AS a 
                INNER JOIN trx_musrenkab_kegiatan_pd AS b ON a.id_kegiatan_pd=b.id_kegiatan_pd 
                INNER JOIN trx_musrenkab_program_pd AS c ON b.id_program_pd=c.id_program_pd 
...
public function getAktivitas($id_forum_skpd)
{
   $getAktivitas=DB::SELECT('SELECT (@id:=@id+1) as urut, a.id_aktivitas_pd, a.id_pelaksana_pd, a.tahun_forum, a.no_urut, 
            ...
            FROM trx_musrenkab_aktivitas_pd a
            ...
            LEFT OUTER JOIN (SELECT a.tahun_forum, a.id_aktivitas_pd,
                ...
                FROM trx_musrenkab_lokasi_pd AS a
                INNER JOIN trx_musrenkab_aktivitas_pd AS b ON a.id_aktivitas_pd = b.id_aktivitas_pd
            ...
            LEFT OUTER JOIN (SELECT a.tahun_forum, a.id_aktivitas_pd, Sum(a.jml_belanja_forum) as jml_belanja
                FROM trx_musrenkab_belanja_pd AS a
...
public function getPelaksanaAktivitas($id_aktivitas){
   $getPelaksana=DB::SELECT('SELECT (@id:=@id+1) as urut,a.* FROM (SELECT a.id_pelaksana_pd, a.tahun_forum, a.no_urut, 
            ...
            FROM trx_musrenkab_aktivitas_pd AS X
            INNER JOIN trx_musrenkab_lokasi_pd AS Y ON X.id_aktivitas_pd = Y.id_aktivitas_pd
            ...
            FROM trx_musrenkab_belanja_pd AS X
            INNER JOIN trx_musrenkab_aktivitas_pd AS Y ON X.id_aktivitas_pd = Y.id_aktivitas_pd
            ...
            CASE a.status_data
                ...
                WHEN 1 THEN "green"
            END AS warna 
            FROM trx_musrenkab_pelaksana_pd a
            INNER JOIN trx_musrenkab_kegiatan_pd f ON a.id_kegiatan_pd = f.id_kegiatan_pd
...
public function getLokasiAktivitas($id_aktivitas_pd)
{
   $LokAktiv=DB::SELECT('SELECT (@id:=@id+1) as no_urut,a.* FROM (SELECT a.tahun_forum, a.no_urut, a.id_aktivitas_pd, a.id_lokasi_pd,
            ...
            FROM trx_musrenkab_lokasi_pd a
            INNER JOIN ref_lokasi b on a.id_lokasi = b.id_lokasi
            INNER JOIN trx_musrenkab_aktivitas_pd c ON a.id_aktivitas_pd = c.id_aktivitas_pd) a,
   ...
   $getBelanja=DB::SELECT('SELECT (@id:=@id+1) as no_urut,a.* FROM (SELECT a.tahun_forum,a.no_urut as urut,a.id_belanja_pd,a.id_aktivitas_pd, p.status_data AS status_aktivitas,
        ...
        h.uraian_satuan as satuan_2_forum, e.kd_rekening, e.nm_rekening, f.nm_aktivitas_asb,
                    ...
                    CASE a.status_data
                        ...
                        WHEN 1 THEN "green"
                    END AS warna
        FROM trx_musrenkab_belanja_pd a
        ...
        INNER JOIN trx_musrenkab_aktivitas_pd p on a.id_aktivitas_pd = p.id_aktivitas_pd) a,
        (SELECT @id:=0) x WHERE a.id_aktivitas_pd='.$id_aktivitas_pd);
...
public function getLokasiCopy($id_unit){
   $getBelanja=DB::SELECT('SELECT (@id:=@id+1) as urut, e.*, c.id_unit
        FROM trx_musrenkab_aktivitas_pd e
        INNER JOIN trx_musrenkab_pelaksana_pd d ON e.id_pelaksana_pd = d.id_pelaksana_pd
        INNER JOIN trx_musrenkab_kegiatan_pd a ON d.id_kegiatan_pd = a.id_kegiatan_pd
        INNER JOIN trx_musrenkab_program_pd b ON a.id_program_pd = b.id_program_pd
...
public function getBelanjaCopy(Request $req){
    ...
    if($cekProgramRkpd != NULL && $cekProgramRkpd[0]->status_rkpd == 0 && $cekProgramRkpd[0]->status_program == 0 && $cekProgramRkpd[0]->status_kegiatan == 0 && $cekProgramRkpd[0]->status_aktivitas == 0) {
        $getBelanja=DB::INSERT('INSERT INTO trx_musrenkab_belanja_pd (tahun_forum, no_urut, id_aktivitas_pd, id_belanja_forum, id_zona_ssh, id_belanja_renja, sumber_belanja, id_aktivitas_asb, id_item_ssh, id_rekening_ssh, uraian_belanja, volume_1, id_satuan_1, volume_2, id_satuan_2, harga_satuan, jml_belanja, volume_1_forum, id_satuan_1_forum, volume_2_forum, id_satuan_2_forum, harga_satuan_forum, jml_belanja_forum, status_data, sumber_data) 
        SELECT tahun_forum, no_urut, '.$req->id_aktivitas_new.', id_belanja_forum, id_zona_ssh, id_belanja_renja, sumber_belanja, id_aktivitas_asb, id_item_ssh, id_rekening_ssh, uraian_belanja, volume_1, id_satuan_1, volume_2, id_satuan_2, harga_satuan, jml_belanja, volume_1_forum, id_satuan_1_forum, volume_2_forum, id_satuan_2_forum, harga_satuan_forum, jml_belanja_forum, 0, 4 FROM trx_musrenkab_belanja_pd where id_aktivitas_pd ='.$req->id_aktivitas_pd);
...
public function unloadASB(Request $req){
    ...
    if($cekProgramRkpd != NULL && $cekProgramRkpd[0]->status_rkpd == 0 && $cekProgramRkpd[0]->status_program == 0 && $cekProgramRkpd[0]->status_kegiatan == 0 && $cekProgramRkpd[0]->status_aktivitas == 0) {
        ...
        $getHitung=DB::DELETE('DELETE FROM trx_musrenkab_belanja_pd 
...
// $cekProgramRkpd = DB::SELECT('SELECT a.status_data FROM trx_musrenkab a
//     INNER JOIN trx_musrenkab_urusan b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
//     INNER JOIN trx_musrenkab_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
//     WHERE c.id_pelaksana_rkpd='.$req->id_rkpd_rancangan);

$cek = DB::SELECT('SELECT c.hak_akses, a.status_data FROM trx_musrenkab a
    INNER JOIN trx_musrenkab_urusan b ON a.id_musrenkab = b.id_musrenkab
    INNER JOIN trx_musrenkab_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
    WHERE c.id_pelaksana_rkpd='.$req->id_pelaksana_rkpd);

        $data = new TrxMusrenkabProgramPd();
        $data->id_pelaksana_rkpd= $req->id_pelaksana_rkpd;
        $data->tahun_forum= Session::get('tahun');
        $data->jenis_belanja= $req->jenis_belanja;
        $data->no_urut= $req->no_urut;
        $data->id_unit= $req->id_unit; 
        $data->id_rkpd_rancangan= 0;
...
$data = TrxMusrenkabProgramPd::find($req->id_program_pd);
        $data->jenis_belanja= $req->jenis_belanja;
        $data->no_urut= $req->no_urut;
        $data->id_unit= $req->id_unit;
...
public function hapusProgRenja(Request $req){
    TrxMusrenkabProgramPd::where('id_program_pd',$req->id_program_pd)->delete();
...
public function postProgRenja(Request $req)
    {
        ...
        $cekRkpd = DB::SELECT('SELECT a.status_data AS status_rkpd FROM trx_musrenkab_pelaksana AS a
            INNER JOIN trx_musrenkab_program_pd AS b ON a.id_pelaksana_rkpd = b.id_pelaksana_rkpd
            WHERE b.id_program_pd='.$req->id_program_pd);

        $cek = DB::SELECT('SELECT a.id_program_pd, a.pagu_forum, (COALESCE(a.pagu_forum,0) - COALESCE(b.pagu_kegiatan,0)) AS selisih, c.jml_indikator  
            FROM trx_musrenkab_program_pd a
            LEFT OUTER JOIN (SELECT id_program_pd, COALESCE(SUM(pagu_forum),0) AS pagu_kegiatan FROM trx_musrenkab_kegiatan_pd
            WHERE status_data = 1 GROUP BY id_program_pd, status_data) b ON a.id_program_pd=b.id_program_pd
            LEFT OUTER JOIN (SELECT id_program_pd, (COALESCE(COUNT(id_indikator_program),0) - COALESCE(SUM(status_data),0)) as jml_indikator FROM trx_musrenkab_prog_indikator_pd GROUP BY id_program_pd) c
            ON a.id_program_pd=c.id_program_pd
            WHERE a.id_program_pd='.$req->id_program_pd);

        if($cekRkpd != null && $cekRkpd[0]->status_rkpd == 0){
            if($cek != null && $cek[0]->selisih == 0 && $cek[0]->jml_indikator == 0){
                try{
                    $data->save (['timestamps' => false]);
                ...
                return response ()->json (['pesan'=>'Cek Kembali Pagu Program, Pagu Kegiatan, Indikator Program','status_pesan'=>'0']);
            }
        } else {
...
$cek = DB::SELECT('SELECT a.hak_akses FROM trx_musrenkab_pelaksana a
        INNER JOIN trx_musrenkab_program_pd b ON a.id_pelaksana_rkpd = b.id_pelaksana_rkpd
        WHERE b.id_program_pd = '.$req->id_program_pd.' GROUP BY b.id_program_pd,a.hak_akses');

        $data = new TrxMusrenkabKegiatanPd();
...
$cek = DB::SELECT('SELECT a.hak_akses FROM trx_musrenkab_pelaksana a
        ...
        $data = TrxMusrenkabKegiatanPd::find($req->id_kegiatan_pd);
...
public function postKegRenja(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT c.id_kegiatan_pd, c.status_data, c.pagu_forum, COALESCE(b.jml_pagu_aktivitas,0) as jml_pagu_aktivitas, 
            COALESCE(c.pagu_forum,0) - COALESCE(b.jml_pagu_aktivitas,0) AS selisih, jml_indikator 
            FROM (SELECT a.tahun_forum, b.id_kegiatan_pd,
                SUM(a.pagu_aktivitas_forum) AS jml_pagu_aktivitas
                FROM trx_musrenkab_aktivitas_pd a 
                INNER JOIN trx_musrenkab_pelaksana_pd b ON a.id_pelaksana_pd = b.id_pelaksana_pd
                WHERE a.status_data = 1
                GROUP BY a.tahun_forum, b.id_kegiatan_pd, a.status_data) b
            INNER JOIN trx_musrenkab_kegiatan_pd c ON b.id_kegiatan_pd = c.id_kegiatan_pd
            INNER JOIN (SELECT id_kegiatan_pd, (COALESCE(COUNT(id_indikator_kegiatan),0) - COALESCE(SUM(status_data),0)) as jml_indikator 
			FROM trx_musrenkab_keg_indikator_pd GROUP BY id_kegiatan_pd) d ON c.id_kegiatan_pd = d.id_kegiatan_pd
        ...
        if($cek != null && $cek[0]->selisih == 0 && $cek[0]->jml_indikator == 0){
            try{
             ...
             return response ()->json (['pesan'=>'Cek Kembali Pagu Kegiatan, Pagu Aktivitas, dan Indikator Kegiatan','status_pesan'=>'0']);
        }
    }

public function hapusKegRenja(Request $req){
    $result = TrxMusrenkabKegiatanPd::where('id_kegiatan_pd',$req->id_kegiatan_pd)->delete ();
...
public function addAktivitas(Request $req)
    {
            $data = new TrxMusrenkabAktivitasPd;
            ...
            FROM trx_musrenkab_pelaksana_pd b
            INNER JOIN trx_musrenkab_kegiatan_pd c ON b.id_kegiatan_pd = c.id_kegiatan_pd
            INNER JOIN trx_musrenkab_program_pd d ON c.id_program_pd = d.id_program_pd
...
public function editAktivitas(Request $req)
    {
        $data = TrxMusrenkabAktivitasPd::find($req->id_aktivitas_pd);
            $data->no_urut= $req->no_urut;
            $data->sumber_aktivitas= $req->sumber_aktivitas;
            $data->id_aktivitas_asb= $req->id_aktivitas_asb;
            $data->id_aktivitas_renja= $req->id_aktivitas_renja;
...
public function postAktivitas(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT a.tahun_forum,a.id_aktivitas_pd, SUM(a.pagu_aktivitas_forum) as pagu_aktivitas, COALESCE(sum(e.jml_belanja),0) as pagu_belanja, 
            ...
            FROM trx_musrenkab_aktivitas_pd a
            LEFT OUTER JOIN (SELECT id_aktivitas_pd, COUNT(*) AS jml_lokasi FROM trx_musrenkab_lokasi_pd
            ...
            FROM trx_musrenkab_belanja_pd AS a GROUP BY a.id_aktivitas_pd) e ON a.id_aktivitas_pd = e.id_aktivitas_pd
...
public function hapusAktivitas(Request $req)
      {
        ...
        if($cek[0]->status_data==0){
            TrxMusrenkabAktivitasPd::where('id_aktivitas_pd',$req->id_aktivitas_pd)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
        } else {
...
public function addPelaksana(Request $req)
    {
            ...
            $data = new TrxMusrenkabPelaksanaPd;
            $data->tahun_forum= Session::get('tahun');
            $data->no_urut= $req->no_urut;
        ...
        $cek = DB::SELECT('SELECT c.status_data AS status_kegiatan, d.status_data AS status_program,
            ...
            FROM trx_musrenkab_kegiatan_pd c
            INNER JOIN trx_musrenkab_program_pd d ON c.id_program_pd = d.id_program_pd
            WHERE c.id_kegiatan_pd='.$req->id_kegiatan_pd);
...
public function editPelaksana(Request $req)
    {
            ...
            $data = TrxMusrenkabPelaksanaPd::find($req->id_pelaksana_pd);
            $data->no_urut= $req->no_urut;
            $data->id_sub_unit= $req->id_sub_unit;
            $data->id_lokasi= $req->id_lokasi;
            $data->ket_pelaksana= $req->ket_pelaksana;
...
public function hapusPelaksana(Request $req)
      {
        ...
        if($cek[0]->status_data==0){
            TrxMusrenkabPelaksanaPd::where('id_pelaksana_pd',$req->id_pelaksana_pd)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
        } else {
            return response ()->json (['pesan'=>'Data Kegiatan telah diposting, Data tidak dapat dihapus','status_pesan'=>'0']);
        }
...
public function editLokasi(Request $req)
    {
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan, e.status_data AS status_aktivitas
            ...
            $data = TrxMusrenkabLokasiPd::find($req->id_lokasi_pd);
            $data->no_urut = $req->no_urut ;
            $data->jenis_lokasi = $req->jenis_lokasi ;
            $data->id_lokasi = $req->id_lokasi ;
            $data->id_lokasi_teknis = $req->id_lokasi_teknis ;            
...
public function hapusLokasi(Request $req)
      {
        TrxMusrenkabLokasiPd::where('id_lokasi_pd',$req->id_lokasi_pd)->delete ();
        return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
      }
...
public function addUsulan(Request $req)
    {
        try{
            $data = new TrxMusrenkabKegiatanPdUsulan;
...
public function editUsulan(Request $req)
    {
        try{
            $data = TrxMusrenkabKegiatanPdUsulan::find($req->id_sumber_usulan);
            $data->sumber_usulan= $req->sumber_usulan;
            $data->id_lokasi_forum= $req->id_lokasi_forum;
            $data->id_ref_usulan= $req->id_ref_usulan;
            $data->volume_1_usulan= $req->volume_1_usulan;
...
public function hapusUsulan(Request $req)
      {
        TrxMusrenkabKegiatanPdUsulan::where('id_sumber_usulan',$req->id_sumber_usulan)->delete ();
        return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
      }
    ...
    public function addBelanja(Request $req)
    {
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan, e.status_data AS status_aktivitas
            ...
            $data = new TrxMusrenkabBelanjaPd;
            $data->tahun_forum= Session::get('tahun');
            $data->no_urut= $req->no_urut;
            $data->id_aktivitas_pd= $req->id_aktivitas_pd;
...
public function editBelanja(Request $req)
    {
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan, e.status_data AS status_aktivitas
            ...
            $data = TrxMusrenkabBelanjaPd::find($req->id_belanja_pd);
            $data->no_urut= $req->no_urut;
            $data->id_zona_ssh= $req->id_zona_ssh;
            $data->id_aktivitas_asb= $req->id_aktivitas_asb;
            $data->id_item_ssh= $req->id_item_ssh;
...
public function hapusBelanja(Request $req)
      {
        TrxMusrenkabBelanjaPd::where('id_belanja_pd',$req->id_belanja_pd)->delete ();
        return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
      }

public function addIndikatorProg(Request $req)
    {
            $data = new TrxMusrenkabProgIndikatorPd;
        ...
        $cek = DB::SELECT('SELECT * from trx_musrenkab_program_pd WHERE id_program_pd='.$req->id_program_pd);
        if($cek[0]->status_data == 0){            
            try{
                $data->save (['timestamps' => false]);
                return response ()->json (['pesan'=>'Data Berhasil Disimpan','status_pesan'=>'1']);
...
public function editIndikatorProg(Request $req)
    {
            $data = TrxMusrenkabProgIndikatorPd::find($req->id_indikator_program);
            $data->no_urut = $req->no_urut ;
            // $data->id_program_pd = $req->id_program_pd ;
            $data->kd_indikator = $req->kd_indikator ;
            $data->uraian_indikator_program =  $req->uraian_indikator_program ;
...
public function postIndikatorProg(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT a.status_data from trx_musrenkab_program_pd a
            INNER JOIN trx_musrenkab_prog_indikator_pd b ON a.id_program_pd = b.id_program_pd
...
public function delIndikatorProg(Request $req)
    {
        ...
        if($cek[0]->status_data == 0){            
            TrxMusrenkabProgIndikatorPd::where('id_indikator_program',$req->id_indikator_program)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus','status_pesan'=>'1'] );
        } else {
            return response ()->json (['pesan'=>'Data Gagal Status Program telah diposting','status_pesan'=>'0']);
        }
...
public function addIndikatorKeg(Request $req)
    {
            $data = new TrxMusrenkabKegIndikatorPd;
            $data->tahun_renja = $req->tahun_renja;
            $data->no_urut = $req->no_urut ;
            $data->id_kegiatan_pd = $req->id_kegiatan_pd ;
            $data->id_perubahan = 0 ;
        ...
        $cek = DB::SELECT('SELECT * FROM trx_musrenkab_kegiatan_pd WHERE id_kegiatan_pd = '.$req->id_kegiatan_pd);
        if($cek[0]->status_data == 0){
            try{
                $data->save (['timestamps' => false]);
                return response ()->json (['pesan'=>'Data Berhasil Disimpan','status_pesan'=>'1']);
...
public function editIndikatorKeg(Request $req)
    {
        $cek = DB::SELECT('SELECT * FROM trx_musrenkab_kegiatan_pd WHERE id_kegiatan_pd = '.$req->id_kegiatan_pd);
            ...
            $data = TrxMusrenkabKegIndikatorPd::find($req->id_indikator_kegiatan);
            $data->no_urut = $req->no_urut ;
            $data->kd_indikator = $req->kd_indikator ;
            $data->uraian_indikator_kegiatan =  $req->uraian_indikator_kegiatan ;
            $data->tolok_ukur_indikator = $req->tolok_ukur_indikator ;
...
public function postIndikatorKeg(Request $req)
    {
        $cek = DB::SELECT('SELECT a.status_data FROM trx_musrenkab_kegiatan_pd a
            INNER JOIN trx_musrenkab_keg_indikator_pd b ON a.id_kegiatan_pd = b.id_kegiatan_pd
...
public function delIndikatorKeg(Request $req)
    {
        ...
        if($cek[0]->status_data == 0){
            TrxMusrenkabKegIndikatorPd::where('id_indikator_kegiatan',$req->id_indikator_kegiatan)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus','status_pesan'=>'1'] );
        } else {
            return response ()->json (['pesan'=>'Data Gagal Status Kegiatan telah diposting','status_pesan'=>'0']);
        }
