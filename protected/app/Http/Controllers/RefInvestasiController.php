...
class RefInvestasiController extends Controller
{
	...
	public function index(){
        ...
        return view('dasar.ref_investasi');
    }
    public function getListinvestasi()
    {
        $getListinvestasi=DB::select('SELECT (@id:=@id+1) as no_urut,
...
, (SELECT @id:=0) x
where a.id_tabel_dasar=10 and b.level>0
');

        return DataTables::of($getListinvestasi)
            ->addColumn('action',function($getListinvestasi){
                    return '
                        <button id="btnEditinvestasi" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Edit Jumlah Investasi" title="Edit Jumlah Investasi" class="btn btn-primary btn-sm" data-id_isian_tabel_dasar="'.$getListinvestasi->id_isian_tabel_dasar.'"><i class="fa fa-pencil fa-fw"></i></button>
                        <button id="btnHapusinvestasi" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Hapus Jumlah Investasi" title="Hapus Jumlah Investasi" class="btn btn-danger btn-sm" data-id_isian_tabel_dasar="'.$getListinvestasi->id_isian_tabel_dasar.'"><i class="fa fa-trash fa-fw"></i></button>
                    ' ;
            })
            
        ->make(true);
    }
    public function getTahuninvestasi()
    {
        $getTahun=DB::select('
...
');
    ...
    public function getKecamataninvestasi()
    {
...
');
    ...
    public function getSektorinvestasi($tahun,$kecamatan)
    {
        $getSektor=DB::select('
...
where id_kolom_tabel_dasar not in (select id_kolom_tabel_dasar from trx_isian_data_dasar where tahun='.$tahun.' and id_kecamatan='.$kecamatan.')
 and  id_tabel_dasar=10 and level<>0
');
        return json_encode($getSektor);
    }
    public function addinvestasi (Request $req)
    {
    ...
    public function getEditinvestasi($id)
    {
        $getListinvestasi=DB::select('SELECT c.id_isian_tabel_dasar,
c.tahun,
...
LEFT OUTER JOIN trx_isian_data_dasar AS c ON c.id_kolom_tabel_dasar = b.id_kolom_tabel_dasar
LEFT OUTER JOIN ref_kecamatan AS d ON c.id_kecamatan = d.id_kecamatan
where a.id_tabel_dasar=10 and b.level>0 and id_isian_tabel_dasar='.$id);
        
        return DataTables::of($getListinvestasi)
        ->make(true);
    }
    public function editinvestasi (Request $req)
    {
    ...
    public function hapusinvestasi (Request $req)
    {
