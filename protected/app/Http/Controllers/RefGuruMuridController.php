...
class RefGuruMuridController extends Controller
{
    ...
    public function index(){
        // if(Auth::check()){ 
            return view('dasar.ref_guru_murid');
        // } else {
            // return view ( 'errors.401' );
        // } 
        
    }
    public function getListgurumurid()
    {
        $getListgurumurid=DB::select('SELECT (@id:=@id+1) as no_urut,
...
, (SELECT @id:=0) x
where a.id_tabel_dasar=8 and b.level>0
');
        
        return DataTables::of($getListgurumurid)
        ->addColumn('action',function($getListgurumurid){
            return '
                        <button id="btnEditgurumurid" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Edit Rasio Guru dan Murid" title="Edit Rasio Guru dan Murid" class="btn btn-primary btn-sm" data-id_isian_tabel_dasar="'.$getListgurumurid->id_isian_tabel_dasar.'"><i class="fa fa-pencil fa-fw"></i></button>
                        <button id="btnHapusgurumurid" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Hapus Rasio Guru dan Murid" title="Hapus Rasio Guru dan Murid" class="btn btn-danger btn-sm" data-id_isian_tabel_dasar="'.$getListgurumurid->id_isian_tabel_dasar.'"><i class="fa fa-trash fa-fw"></i></button>
                    ' ;
        })
        
        ->make(true);
    }
    public function getTahungurumurid()
    {
        $getTahun=DB::select('
...
');
    ...
    public function getKecamatangurumurid()
    {
...
where id_kolom_tabel_dasar not in (select id_kolom_tabel_dasar from trx_isian_data_dasar where tahun='.$tahun.' and id_kecamatan='.$kecamatan.')
 and  id_tabel_dasar=8 and level<>0 and parent_id='.$tingkat);
        return json_encode($getSektor);
    }
    
    public function addgurumurid (Request $req)
    {
    ...
    public function getEditgurumurid($id)
    {
        $getListgurumurid=DB::select('SELECT c.id_isian_tabel_dasar,
c.tahun,
...
LEFT OUTER JOIN trx_isian_data_dasar AS c ON c.id_kolom_tabel_dasar = b.id_kolom_tabel_dasar
LEFT OUTER JOIN ref_kecamatan AS d ON c.id_kecamatan = d.id_kecamatan
where a.id_tabel_dasar=8 and b.level>0 and id_isian_tabel_dasar='.$id);
        
        return DataTables::of($getListgurumurid)
        ->make(true);
    }
    public function editgurumurid (Request $req)
    {
    ...
    public function hapusgurumurid (Request $req)
    {
