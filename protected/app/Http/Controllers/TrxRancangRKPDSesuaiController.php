...
class TrxRancangRKPDSesuaiController extends Controller
{
    ...
    public function getProgramRkpd($tahun,$unit)
    {
        $getRenja=DB::select('SELECT (@id:=@id+1) as no_urut, a.id_rkpd_rancangan, a.id_rkpd_ranwal, a.tahun_rkpd, a.id_program_rpjmd, 
            ...
            FROM trx_rkpd_rancangan_program_pd AS x 
            ...
            FROM trx_rkpd_rancangan_program_pd AS x 
            INNER JOIN trx_rkpd_rancangan_kegiatan_pd AS y ON x.id_program_pd = y.id_program_pd
            WHERE x.id_rkpd_rancangan = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            ...
            FROM trx_rkpd_rancangan_program_pd AS x 
            INNER JOIN trx_rkpd_rancangan_kegiatan_pd AS y ON x.id_program_pd = y.id_program_pd
            INNER JOIN trx_rkpd_rancangan_pelaksana_pd AS z ON y.id_kegiatan_pd = z.id_kegiatan_pd
            INNER JOIN trx_rkpd_rancangan_aktivitas_pd AS r ON z.id_pelaksana_pd = r.id_pelaksana_pd
            WHERE x.id_rkpd_rancangan = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            ...
            INNER JOIN trx_rkpd_rancangan_pelaksana AS b ON d.id_urusan_rkpd = b.id_urusan_rkpd, 
    ...
    public function getProgramRenja($id_unit,$id_pelaksana_rkpd)
    {
      $getProgramRenja=DB::select('SELECT (@id:=@id+1) as urut, a.* FROM (SELECT a.id_program_pd, c.id_pelaksana_rkpd, c.id_rkpd_rancangan, a.tahun_forum, a.no_urut, a.id_unit, b.nm_unit, a.id_renja_program, 
            ...
            COALESCE((SELECT COUNT(x.id_kegiatan_pd) AS jml_kegiatan
                        FROM trx_rkpd_rancangan_kegiatan_pd AS x
    ...
    public function getIndikatorRenja($id_rkpd)
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_renja, a.no_urut, a.id_program_pd, a.id_program_renstra,
                ...
                INNER JOIN trx_rkpd_rancangan_program_pd AS b ON a.id_program_pd=b.id_program_pd
    ...
    public function getKegiatanRenja($id_program)
    {
      $getKegiatanRenja=DB::select('SELECT (@id:=@id+1) as urut, a.* FROM (SELECT a.id_kegiatan_pd, a.id_program_pd, a.id_unit, 
            ...
            WHEN 1 THEN "green"
            END AS warna,
            COALESCE((SELECT SUM(x.pagu_forum) FROM trx_rkpd_rancangan_kegiatan_pd x WHERE x.id_kegiatan_pd=a.id_kegiatan_pd 
                GROUP BY x.id_kegiatan_pd),0) AS jml_pagu,
            COALESCE((SELECT COUNT(x.id_aktivitas_forum) as jml_aktivitas
            FROM trx_rkpd_rancangan_aktivitas_pd x 
            INNER JOIN trx_rkpd_rancangan_pelaksana_pd y ON x.id_pelaksana_pd = y.id_pelaksana_pd
            ...
            INNER JOIN trx_rkpd_rancangan_program_pd AS x ON a.id_program_pd=x.id_program_pd
    ...
    public function getIndikatorKegiatan($id_rkpd)
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_renja, a.no_urut, a.id_kegiatan_pd,
                ...
                INNER JOIN trx_rkpd_rancangan_kegiatan_pd AS b ON a.id_kegiatan_pd=b.id_kegiatan_pd 
                INNER JOIN trx_rkpd_rancangan_program_pd AS c ON b.id_program_pd=c.id_program_pd 
...
public function getPelaksanaAktivitas($id_aktivitas){
   $getPelaksana=DB::SELECT('SELECT (@id:=@id+1) as urut,a.* FROM (SELECT a.id_pelaksana_pd, a.tahun_forum, a.no_urut, 
            ...
            FROM trx_rkpd_rancangan_aktivitas_pd AS X
            INNER JOIN trx_rkpd_rancangan_lokasi_pd AS Y ON X.id_aktivitas_pd = Y.id_aktivitas_pd
            ...
            FROM trx_rkpd_rancangan_belanja_pd AS X
            INNER JOIN trx_rkpd_rancangan_aktivitas_pd AS Y ON X.id_aktivitas_pd = Y.id_aktivitas_pd
            ...
            INNER JOIN trx_rkpd_rancangan_kegiatan_pd f ON a.id_kegiatan_pd = f.id_kegiatan_pd
...
public function getBelanja($id_aktivitas_pd){
   $getBelanja=DB::SELECT('SELECT (@id:=@id+1) as no_urut,a.* FROM (SELECT a.tahun_forum,a.no_urut as urut,a.id_belanja_pd,a.id_aktivitas_pd, p.status_data AS status_aktivitas,
            ...
            INNER JOIN trx_rkpd_rancangan_aktivitas_pd p on a.id_aktivitas_pd = p.id_aktivitas_pd) a,
            (SELECT @id:=0) x WHERE a.id_aktivitas_pd='.$id_aktivitas_pd);
...
public function getLokasiCopy($id_unit){
   $getBelanja=DB::SELECT('SELECT (@id:=@id+1) as urut, e.*, c.id_unit
            FROM trx_rkpd_rancangan_aktivitas_pd e
            INNER JOIN trx_rkpd_rancangan_pelaksana_pd d ON e.id_pelaksana_pd = d.id_pelaksana_pd
            ...
            INNER JOIN trx_rkpd_rancangan_pelaksana c ON b.id_rkpd_rancangan = c.id_pelaksana_rkpd,
            (SELECT @id:=0) x
...
public function getBelanjaCopy(Request $req){
    ...
    if($cekProgramRkpd != NULL && $cekProgramRkpd[0]->status_rkpd == 0 && $cekProgramRkpd[0]->status_program == 0 && $cekProgramRkpd[0]->status_kegiatan == 0 && $cekProgramRkpd[0]->status_aktivitas == 0) {
        $getBelanja=DB::INSERT('INSERT INTO trx_rkpd_rancangan_belanja_pd (tahun_forum, no_urut, id_aktivitas_pd, id_belanja_forum, id_zona_ssh, id_belanja_renja, sumber_belanja, id_aktivitas_asb, id_item_ssh, id_rekening_ssh, uraian_belanja, volume_1, id_satuan_1, volume_2, id_satuan_2, harga_satuan, jml_belanja, volume_1_forum, id_satuan_1_forum, volume_2_forum, id_satuan_2_forum, harga_satuan_forum, jml_belanja_forum, status_data, sumber_data) 
        SELECT tahun_forum, no_urut, '.$req->id_aktivitas_new.', id_belanja_forum, id_zona_ssh, id_belanja_renja, sumber_belanja, id_aktivitas_asb, id_item_ssh, id_rekening_ssh, uraian_belanja, volume_1, id_satuan_1, volume_2, id_satuan_2, harga_satuan, jml_belanja, volume_1_forum, id_satuan_1_forum, volume_2_forum, id_satuan_2_forum, harga_satuan_forum, jml_belanja_forum, 0, 4 FROM trx_rkpd_rancangan_belanja_pd where id_aktivitas_pd ='.$req->id_aktivitas_pd);
...
// $cekProgramRkpd = DB::SELECT('SELECT a.status_data FROM trx_rkpd_rancangan a
//     INNER JOIN trx_rkpd_rancangan_urusan b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
//     INNER JOIN trx_rkpd_rancangan_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
//     WHERE c.id_pelaksana_rkpd='.$req->id_rkpd_rancangan);

$cek = DB::SELECT('SELECT c.hak_akses, a.status_data FROM trx_rkpd_rancangan a
    INNER JOIN trx_rkpd_rancangan_urusan b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
    INNER JOIN trx_rkpd_rancangan_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
    WHERE c.id_pelaksana_rkpd='.$req->id_rkpd_rancangan);
...
public function postProgRenja(Request $req)
    {
        ...
        $cekRkpd = DB::SELECT('SELECT a.status_data AS status_rkpd FROM trx_rkpd_rancangan_pelaksana AS a
            INNER JOIN trx_rkpd_rancangan_program_pd AS b ON a.id_pelaksana_rkpd = b.id_rkpd_rancangan
        ...
        $cek = DB::SELECT('SELECT a.id_program_pd, a.pagu_forum, (COALESCE(a.pagu_forum,0) - COALESCE(b.pagu_kegiatan,0)) AS selisih, c.jml_indikator  
            FROM trx_rkpd_rancangan_program_pd a
            LEFT OUTER JOIN (SELECT id_program_pd, COALESCE(SUM(pagu_forum),0) AS pagu_kegiatan FROM trx_rkpd_rancangan_kegiatan_pd
            WHERE status_data = 1 GROUP BY id_program_pd, status_data) b
            ON a.id_program_pd=b.id_program_pd            
            LEFT OUTER JOIN (SELECT id_program_pd, (COALESCE(COUNT(id_indikator_program),0) - COALESCE(SUM(status_data),0)) as jml_indikator FROM trx_rkpd_rancangan_prog_indikator_pd GROUP BY id_program_pd) c
...
$cek = DB::SELECT('SELECT a.hak_akses FROM trx_rkpd_rancangan_pelaksana a
        INNER JOIN trx_rkpd_rancangan_program_pd b ON a.id_pelaksana_rkpd = b.id_rkpd_rancangan
...
public function postKegRenja(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT c.id_kegiatan_pd, c.status_data, c.pagu_forum, COALESCE(b.jml_pagu_aktivitas,0) as jml_pagu_aktivitas, 
            COALESCE(c.pagu_forum,0) - COALESCE(b.jml_pagu_aktivitas,0) AS selisih, d.jml_indikator 
			...
			FROM trx_rkpd_rancangan_keg_indikator_pd GROUP BY id_kegiatan_pd) d ON c.id_kegiatan_pd = d.id_kegiatan_pd
...
public function postAktivitas(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT a.tahun_forum,a.id_aktivitas_pd, SUM(a.pagu_aktivitas_forum) as pagu_aktivitas, COALESCE(sum(e.jml_belanja),0) as pagu_belanja, 
            ...
            FROM trx_rkpd_rancangan_aktivitas_pd a
            LEFT OUTER JOIN (SELECT id_aktivitas_pd, COUNT(*) AS jml_lokasi FROM trx_rkpd_rancangan_lokasi_pd
            ...
            FROM trx_rkpd_rancangan_belanja_pd AS a GROUP BY a.id_aktivitas_pd) e ON a.id_aktivitas_pd = e.id_aktivitas_pd
...
public function hapusUsulan(Request $req)
    ...
    public function addBelanja(Request $request)
    {
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan, e.status_data AS status_aktivitas
            ...
            INNER JOIN trx_rkpd_rancangan_program_pd b ON a.id_program_pd = b.id_program_pd
            INNER JOIN trx_rkpd_rancangan_pelaksana c ON b.id_rkpd_rancangan = c.id_pelaksana_rkpd
            WHERE e.id_aktivitas_pd='.$request->id_aktivitas_pd);

        $rules = [
            'no_urut' => 'required',
            'id_aktivitas_pd' => 'required',
            'id_zona_ssh'=> 'required',
            'sumber_belanja' => 'required',
            'id_item_ssh' => 'required',
            'id_rekening_ssh' => 'required',
            'uraian_belanja' => 'required',
            'volume_1_forum' => 'required',
            'id_satuan_1_forum' => 'required',
            'volume_2_forum' => 'required',
            'id_satuan_2_forum' => 'required',
            'harga_satuan_forum' => 'required',
            'jml_belanja_forum' => 'required',
        ];
        $messages =[
            'no_urut.required' => 'No Urut Kosong',
            'id_aktivitas_pd.required' => 'Id Aktivitas Kosong',
            'id_zona_ssh.required'=> 'Id Zona Kosong',
            'sumber_belanja.required' => 'Sumber Belanja Kosong',
            'id_item_ssh.required' => 'Item SSH Kosong',
            'id_rekening_ssh.required' => 'Kode Rekening Kosong',
            'uraian_belanja.required' => 'Uraian Belanja Kosong',
            'volume_1_forum.required' => 'Volumen 1 Kosong',
            'id_satuan_1_forum.required' => 'Satuan 1  Kosong',
            'volume_2_forum.required' => 'Volume 2 Kosong',
            'id_satuan_2_forum.required' => 'Satuan 2 Kosong',
            'harga_satuan_forum.required' => 'Harga Satuan Kosong',
            'jml_belanja_forum.required' => 'Jumlah Belanja Kosong',
        ];
        ...
        if($validation->fails()) {
            ...
            return response ()->json (['pesan'=>$errors,'status_pesan'=>'0']);			
                ...
                $data->id_aktivitas_pd= $request->id_aktivitas_pd;
                $data->id_zona_ssh= $request->id_zona_ssh;
                $data->sumber_belanja= $request->sumber_belanja;
                $data->id_aktivitas_asb= 0;
                $data->id_item_ssh= $request->id_item_ssh;
                $data->id_rekening_ssh= $request->id_rekening_ssh;
                $data->uraian_belanja= $request->uraian_belanja;
                $data->volume_1= 0;
                $data->id_satuan_1= 0;
                $data->volume_2= 0;
                $data->id_satuan_2= 0;
                $data->harga_satuan= 0;
                $data->jml_belanja= 0;
                $data->volume_1_forum= $request->volume_1_forum;
                $data->id_satuan_1_forum= $request->id_satuan_1_forum;
                $data->volume_2_forum= $request->volume_2_forum;
                $data->id_satuan_2_forum= $request->id_satuan_2_forum;
                $data->harga_satuan_forum= $request->harga_satuan_forum;
                $data->jml_belanja_forum= $request->jml_belanja_forum;
                $data->sumber_data= 4;
                $data->status_data= 0;
