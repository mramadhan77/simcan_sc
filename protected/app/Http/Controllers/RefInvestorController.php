...
class RefInvestorController extends Controller
{
	...
	public function index(){
        ...
        return view('dasar.ref_investor');
    }
    public function getListinvestor()
    {
        $getListinvestor=DB::select('SELECT (@id:=@id+1) as no_urut,
...
, (SELECT @id:=0) x
where a.id_tabel_dasar=9 and b.level>0
');

        return DataTables::of($getListinvestor)
            ->addColumn('action',function($getListinvestor){
                    return '
                        <button id="btnEditinvestor" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Edit Data Jumlah Investor" title="Edit Data Jumlah Investor" class="btn btn-primary btn-sm" data-id_isian_tabel_dasar="'.$getListinvestor->id_isian_tabel_dasar.'"><i class="fa fa-pencil fa-fw"></i></button>
                        <button id="btnHapusinvestor" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Hapus Data Jumlah Investor" title="Hapus Data Jumlah Investor" class="btn btn-danger btn-sm" data-id_isian_tabel_dasar="'.$getListinvestor->id_isian_tabel_dasar.'"><i class="fa fa-trash fa-fw"></i></button>
                    ' ;
            })
            
        ->make(true);
    }
    public function getTahuninvestor()
    {
        $getTahun=DB::select('
...
');
    ...
    public function getKecamataninvestor()
    {
...
');
    ...
    public function getSektorinvestor($tahun,$kecamatan)
    {
        $getSektor=DB::select('
...
where id_kolom_tabel_dasar not in (select id_kolom_tabel_dasar from trx_isian_data_dasar where tahun='.$tahun.' and id_kecamatan='.$kecamatan.')
 and  id_tabel_dasar=9 and level<>0
');
        return json_encode($getSektor);
    }
    public function addinvestor (Request $req)
    {
    ...
    public function getEditinvestor($id)
    {
        $getListinvestor=DB::select('SELECT c.id_isian_tabel_dasar,
c.tahun,
...
LEFT OUTER JOIN trx_isian_data_dasar AS c ON c.id_kolom_tabel_dasar = b.id_kolom_tabel_dasar
LEFT OUTER JOIN ref_kecamatan AS d ON c.id_kecamatan = d.id_kecamatan
where a.id_tabel_dasar=9 and b.level>0 and id_isian_tabel_dasar='.$id);
        
        return DataTables::of($getListinvestor)
        ->make(true);
    }
    public function editinvestor (Request $req)
    {
    ...
    public function hapusinvestor (Request $req)
    {
