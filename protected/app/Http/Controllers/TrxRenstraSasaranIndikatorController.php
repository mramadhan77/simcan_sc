...
use App\Models\Can\TrxRenstraSasaranIndikator;

class TrxRenstraSasaranIndikatorController extends Controller
{
    ...
    public function getIndikatorSasaran($id_sasaran_rpjmd)
    {
        ...
        LEFT OUTER JOIN ref_satuan AS c ON b.id_satuan_output = c.id_satuan,(SELECT @id:=0) x  WHERE a.id_sasaran_renstra='.$id_sasaran_rpjmd);
    ...
    public function getIndikatorSasaranRpjmd($id_sasaran){
        $refindikator=DB::SELECT('SELECT (@id:=@id+1) as no_urut, a.id_indikator_sasaran_rpjmd, d.uraian_sasaran_rpjmd, 
            a.kd_indikator, b.nm_indikator, b.id_satuan_output,c.uraian_satuan,
            a.angka_tahun1,a.angka_tahun2,a.angka_tahun3,a.angka_tahun4,a.angka_tahun5,a.angka_akhir_periode
            ...
            INNER JOIN trx_rpjmd_sasaran AS d ON a.id_sasaran_rpjmd = d.id_sasaran_rpjmd, (SELECT @id:=0) x
    ...
    public function addIndikator(Request $request)
    {
        ...
        $messages =[
            ...
            'id_sasaran_renstra.required'=> 'No Sasaran Kosong',
		...
		if($validation->fails()) {
			...
			$data = new TrxRenstraSasaranIndikator();
            ...
            $data->uraian_indikator_sasaran_renstra= $request->uraian_indikator_sasaran_renstra;
    ...
    public function editIndikator(Request $request)
    {
        ...
        $messages =[
            'id_indikator_sasaran_renstra.required' => 'ID Indikator Sasaran Renstra Kosong',
            'thn_id.required' => 'Tahun Kosong',
            'no_urut.required' => 'No Urut Kosong',
            'id_sasaran_renstra.required'=> 'No Sasaran Kosong',
            'id_perubahan.required' => 'ID Perubahan Kosong',
		...
		if($validation->fails()) {
            ...
            $data = TrxRenstraSasaranIndikator::find($request->id_indikator_sasaran_renstra);
            $data->thn_id= $request->thn_id;
            $data->no_urut= $request->no_urut;
            $data->id_sasaran_renstra= $request->id_sasaran_renstra;
            // $data->id_indikator_sasaran_renstra=$request->id_indikator_sasaran_renstra;
    ...
    public function delIndikator(Request $request){
		...
		if($validation->fails()) {
            ...
            $data = TrxRenstraSasaranIndikator::where('id_indikator_sasaran_renstra',$request->id_indikator_sasaran_renstra)->delete();
