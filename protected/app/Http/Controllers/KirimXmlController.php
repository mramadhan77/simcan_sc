...
class KirimXmlController extends Controller
{
...
ref_ssh_tarif.id_tarif_ssh,
ref_ssh_tarif.no_urut,
ref_ssh_tarif.id_sub_kelompok_ssh,
XML_Encode(ref_ssh_tarif.uraian_tarif_ssh) as uraian_tarif_ssh ,
XML_Encode(ref_ssh_tarif.keterangan_tarif_ssh) as keterangan_tarif_ssh,
ref_ssh_tarif.id_satuan
FROM
	ref_ssh_tarif ');
        $xml = new SimpleXMLElement('<?xml version="1.0" encoding="utf-8"?><mydoc></mydoc>');
    ...
    public function getRefRek(Request $request)
    {
        ...
        $rek5 = DB::SELECT('SELECT  kd_rek_1,kd_rek_2, kd_rek_3,kd_rek_4,kd_rek_5, min(XML_Encode(nama_kd_rek_5)) as nama_kd_rek_5, min(Peraturan) as peraturan
		FROM Ref_Rek_5
    ...
    {
        $TaSub = DB::SELECT('SELECT DISTINCT YEAR(CURDATE())+1 as Tahun, f.kd_urusan, e.kd_bidang, d.kd_unit, c.kd_sub, 
 ...
 INNER JOIN ref_bidang e on e.id_bidang=d.id_bidang inner join ref_urusan f on f.kd_urusan=e.kd_urusan 
   ...
   ');
        $TaMisi = DB::SELECT('SELECT distinct YEAR(CURDATE())+1 as Tahun, k.kd_urusan, j.kd_bidang, i.kd_unit, h.kd_sub, x.no_urut as no_misi, 
...
ORDER BY Tahun, k.kd_urusan, j.kd_bidang, i.kd_unit, h.kd_sub, x.no_urut
   ...
   ');
        $TaTujuan = DB::SELECT('SELECT DISTINCT YEAR(CURDATE())+1 as Tahun, k.kd_urusan, j.kd_bidang, i.kd_unit, 
...
where a.tahun_renja = 2019 and x.no_urut not in (98,99) 
   ');
        $TaSasaran = DB::SELECT('SELECT DISTINCT YEAR(CURDATE())+1 as Tahun, k.kd_urusan, j.kd_bidang, i.kd_unit, h.kd_sub, 
 ...
 WHERE a.tahun_renja = 2019 and x.no_urut not in (98,99) 
   ');
        $TaProgram = DB::SELECT('SELECT a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog,	a.id_prog, case when a.Kd_Prog = 0  
 then "Non Program" else min(a.ket_program) end as ket_program, coalesce(min(XML_Encode(a.Tolak_Ukur)),"-") as Tolak_Ukur, COALESCE(max(a.Target_Angka),0) AS Target_Angka, 
 ...
 concat_WS(" ", b.uraian_aktivitas_kegiatan, ab.uraian_lokasi) as AKTIVITAS_LOKASI, 
...
FROM
	(
		SELECT
			...
			a.volume_2_forum,
			a.id_satuan_2_forum,
			a.harga_satuan_forum,
			COALESCE (a.jml_belanja_forum, 0) AS jml_belanja_forum,
			a.status_data,
			a.sumber_data,
		...
		LEFT OUTER JOIN (
			SELECT
				a.id_pelaksana_forum,
				b.id_lokasi,
				GROUP_CONCAT(" ", b.nama_lokasi) AS uraian_lokasi
			FROM
				trx_forum_skpd_lokasi a
			INNER JOIN ref_lokasi b ON a.id_lokasi = b.id_lokasi
			GROUP BY
				id_pelaksana_forum,b.id_lokasi
		) AS ab ON ab.id_pelaksana_forum = b.id_aktivitas_forum
		INNER JOIN trx_forum_skpd_pelaksana d ON b.id_forum_skpd = d.id_pelaksana_forum
		INNER JOIN trx_forum_skpd e ON d.id_aktivitas_forum = e.id_forum_skpd
		INNER JOIN (
			SELECT
...
GROUP BY
   ...
   ');
        $indikator=DB::select(' SELECT  @row_num := IF(@prev_value=a.id_kegiatan,@row_num+1,1) AS No_ID, @prev_value := a.id_kegiatan as id_kegiatanq, a.* FROM ( 
 SELECT  a. id_forum_skpd, a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.Kd_Keg, 3 as Kd_Indikator,  
  XML_Encode(a.tolok_ukur_indikator) as Tolak_Ukur, a.target_renja as Target_Angka, COALESCE(a.uraian_satuan,"-") as Target_Uraian,  
  a.uraian_kegiatan_forum, a.id_indikator_kegiatan, a.no_urut, a.id_kegiatan   
 FROM (SELECT ik.id_forum_skpd, ik.id_indikator_kegiatan, ik.kd_indikator, ik.uraian_indikator_kegiatan, SUBSTRING(ik.tolok_ukur_indikator,1,255)  
 as tolok_ukur_indikator, ik.target_renja, ik.indikator_output, ik.id_satuan_ouput, ik.indikator_input, ik.target_input,  
 ik.id_satuan_input, rs.uraian_satuan, b.tahun_forum as tahun, k.Kd_Urusan, j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog,  
 f.id_prog, f.Kd_Keg, f.nm_kegiatan, e.uraian_kegiatan_forum, ik.no_urut, f.id_kegiatan   
 FROM trx_forum_skpd_belanja a 
 INNER JOIN trx_forum_skpd_aktivitas b ON a.id_lokasi_forum = b.id_aktivitas_forum 
 LEFT OUTER JOIN (select id_pelaksana_forum, GROUP_CONCAT(" ",uraian_lokasi) as uraian_lokasi from trx_forum_skpd_lokasi  
 GROUP BY id_pelaksana_forum) AS ab ON ab.id_pelaksana_forum = b.id_aktivitas_forum  
 INNER JOIN trx_forum_skpd_pelaksana d ON b.id_forum_skpd = d.id_pelaksana_forum  
 INNER JOIN trx_forum_skpd e ON d.id_aktivitas_forum = e.id_forum_skpd  
  INNER JOIN trx_forum_skpd_kegiatan_indikator AS ik ON ik.id_forum_skpd = e.id_forum_skpd  
  LEFT JOIN ref_satuan rs on ik.id_satuan_ouput = rs.id_satuan  
...
INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan) as f   
  ...
  INNER JOIN trx_forum_skpd_program g ON e.id_forum_program = g.id_forum_program  
  LEFT JOIN trx_forum_skpd_program_indikator gx on g.id_forum_program = gx.id_forum_program  
  INNER JOIN ref_sub_unit h on d.id_sub_unit = h.id_sub_unit INNER JOIN ref_unit i on h.id_unit=i.id_unit   
  INNER JOIN ref_bidang j on i.id_bidang=j.id_bidang INNER JOIN ref_urusan k ON j.kd_urusan = k.kd_urusan   
               where b.tahun_forum = 2019 AND  g.jenis_belanja = 0  and b.status_pelaksanaan=0 and e.status_pelaksanaan=0           -- // jenis_belanja    0=BL, 1=pdpt, 2=BTL
 -- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
  ) a  
  GROUP BY a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.kd_keg, a.id_forum_skpd, a.id_indikator_kegiatan  
  order by a.id_kegiatan, a.no_urut 
 ) a, (SELECT @row_num := 0, @prev_value := 0) yy ');
        $indikatorhasil=DB::select('SELECT  @row_num := IF(@prev_value=a.id_kegiatan,@row_num+1,1) AS No_ID, @prev_value := a.id_kegiatan as id_kegiatanq, a.* FROM (
 SELECT a.id_forum_skpd, a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.Kd_Keg, 4 as Kd_Indikator, XML_Encode(a.Tolak_Ukur) as Tolak_Ukur,
 a.Target_Angka, /*COALESCE(a.uraian_indikator_sasaran_renstra,"-")*/ " " as Target_Uraian, a.id_indikator_sasaran_renstra AS id_indikator_program, a.no_urut, a.id_kegiatan
  FROM ( 
  SELECT b.tahun_forum as tahun, b.id_forum_skpd, k.Kd_Urusan, j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog, f.id_prog,  f.Kd_Keg,
  gd.no_urut, gd.tolok_ukur_indikator AS Tolak_Ukur, 
  case gd.thn_id 
  ...
  gd.uraian_indikator_sasaran_renstra, gd.id_indikator_sasaran_renstra, f.id_kegiatan
  FROM trx_forum_skpd_belanja a 
 INNER JOIN trx_forum_skpd_aktivitas b ON a.id_lokasi_forum = b.id_aktivitas_forum 
 LEFT OUTER JOIN (select id_pelaksana_forum, GROUP_CONCAT(" ",uraian_lokasi) as uraian_lokasi from trx_forum_skpd_lokasi
  GROUP BY id_pelaksana_forum) AS ab ON ab.id_pelaksana_forum = b.id_aktivitas_forum
...
INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan) as f
 ...
 INNER JOIN trx_renstra_sasaran_indikator AS gd ON gc.id_sasaran_renstra = gd.id_sasaran_renstra
  ...
  INNER JOIN ref_sub_unit h on d.id_sub_unit = h.id_sub_unit INNER JOIN ref_unit i on h.id_unit=i.id_unit
  INNER JOIN ref_bidang j on i.id_bidang=j.id_bidang INNER JOIN ref_urusan k ON j.kd_urusan = k.kd_urusan
 where b.tahun_forum = 2019 AND g.jenis_belanja = 0 AND f.Kd_Prog <> 0  and b.status_pelaksanaan=0 and e.status_pelaksanaan=0    
-- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
 ) a  
  GROUP BY a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.kd_keg, a.id_forum_skpd, a.id_indikator_sasaran_renstra 
  order by a.id_kegiatan, a.no_urut
  ) a, (SELECT @row_num := 0, @prev_value := 0) yy 
');
        $capaianprogram=DB::select('SELECT  @row_num := IF(@prev_value=a.id_programq,@row_num+1,1) AS No_ID, @prev_value := a.id_programq as id_programq, a.* FROM ( 
 SELECT a.id_forum_skpd, a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, XML_Encode(a.Tolak_Ukur) as Tolak_Ukur,   
 a.Target_Angka, COALESCE(a.Target_Uraian,"-") as Target_Uraian, a.id_indikator_program , a.no_urut, 
  concat(a.id_sub_unit,".",a.id_program) as id_programq    
 FROM (  
 SELECT g.id_forum_program, h.id_sub_unit, b.tahun_forum as tahun, b.id_forum_skpd, k.Kd_Urusan, j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog, f.id_prog,   
 gx.no_urut , gx.tolok_ukur_indikator AS Tolak_Ukur, gx.target_renja as Target_Angka, rs.uraian_satuan AS Target_Uraian, gx.id_indikator_program, f.id_program 
 FROM trx_forum_skpd_belanja a 
 INNER JOIN trx_forum_skpd_aktivitas b ON a.id_lokasi_forum = b.id_aktivitas_forum 
 LEFT OUTER JOIN (select id_pelaksana_forum, GROUP_CONCAT(" ",uraian_lokasi) as uraian_lokasi from trx_forum_skpd_lokasi  
 GROUP BY id_pelaksana_forum) AS ab ON ab.id_pelaksana_forum = b.id_aktivitas_forum  
...
INNER JOIN ref_bidang c ON b.id_bidang = c.id_bidang   
INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan) as f   
 ON e.id_kegiatan_ref = f.id_kegiatan  INNER JOIN trx_forum_skpd_program g ON e.id_forum_program = g.id_forum_program  
 LEFT JOIN trx_forum_skpd_program_indikator gx on g.id_forum_program = gx.id_forum_program  
 LEFT JOIN ref_satuan rs on gx.id_satuan_ouput = rs.id_satuan  
 INNER JOIN ref_sub_unit h on d.id_sub_unit = h.id_sub_unit INNER JOIN ref_unit i on h.id_unit=i.id_unit   
 INNER JOIN ref_bidang j on i.id_bidang=j.id_bidang INNER JOIN ref_urusan k ON j.kd_urusan = k.kd_urusan   
 where b.tahun_forum = 2019 AND g.jenis_belanja = 0   and b.status_pelaksanaan=0 and e.status_pelaksanaan=0        --    // jenis_belanja=1 ==> pendapatan    0=BL, 1=pdpt, 2=BTL
 -- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
 order by k.Kd_Urusan, j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog, f.id_prog ) a  
 GROUP BY a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.id_indikator_program     
  order by a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog  
 ) a, (SELECT @row_num := 0, @prev_value := 0) yy   
');
    ...
    {
        $unit = DB::SELECT('SELECT DISTINCT YEAR(CURDATE())+1 as Tahun, f.kd_urusan, e.kd_bidang, d.kd_unit, c.kd_sub, 
 COALESCE(b.nama_pimpinan_skpd,"-") as nm_pimpinan, COALESCE(b.nip_pimpinan_skpd,"-") as nip_pimpinan, 
 COALESCE(b.nama_jabatan_pimpinan_skpd,"-") as jbt_pimpinan, COALESCE(b.alamat_sub_unit,"-") as Alamat, 
 case when COALESCE(g.uraian_visi_renstra,"-") = "" then "-" else COALESCE(g.uraian_visi_renstra,"-") end as Ur_Visi 
 FROM ref_sub_unit c LEFT OUTER JOIN ref_data_sub_unit b on b.id_sub_unit=c.id_sub_unit 
 ...
 INNER JOIN ref_bidang e on e.id_bidang=d.id_bidang inner join ref_urusan f on f.kd_urusan=e.kd_urusan 
   ...
   ');
    ...
    {
        $unit = DB::SELECT('SELECT distinct YEAR(CURDATE())+1 as Tahun, i.kd_urusan, i.kd_bidang, h.kd_unit, g.kd_sub, a.no_urut as no_misi, 
XML_Encode(GantiEnter(a.uraian_misi_renstra)) as ur_misi
 ...
 WHERE NOT a.no_urut IN (98,99) 
   ');
    ...
    {
        $unit = DB::SELECT('SELECT DISTINCT YEAR(CURDATE())+1 as Tahun, k.kd_urusan, j.kd_bidang, i.kd_unit, 
 h.kd_sub, x.no_urut as no_misi, y.no_urut as no_tujuan, y.uraian_tujuan_renstra as ur_tujuan 
 from Trx_renja_rancangan a INNER JOIN trx_renja_rancangan_pelaksana g on a.id_renja=g.id_renja 
 INNER JOIN ref_sub_unit h on g.id_sub_unit=h.id_sub_unit 
 INNER JOIN ref_unit i on h.id_unit=i.id_unit 
...
where a.tahun_renja = 2019 and x.no_urut not in (98,99) 
   ');
    ...
    {
        $unit = DB::SELECT('SELECT DISTINCT YEAR(CURDATE())+1 as Tahun, k.kd_urusan, j.kd_bidang, i.kd_unit, h.kd_sub, 
 x.no_urut as no_misi, y.no_urut as no_tujuan, z.no_urut as no_sasaran, 
 XML_Encode(z.uraian_sasaran_renstra) as ur_sasaran from Trx_renja_rancangan a 
 INNER JOIN trx_renja_rancangan_pelaksana g on a.id_renja=g.id_renja 
 INNER JOIN ref_sub_unit h on g.id_sub_unit=h.id_sub_unit 
...
INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan) as f 
 ...
 l.kd_rek_1, l.kd_rek_2, l.kd_rek_3, l.kd_rek_4, l.kd_rek_5, 
 a.id_belanja_forum, a.id_lokasi_forum, a.id_zona_ssh, a.id_belanja_renja, a.sumber_belanja, 
 ab.uraian_lokasi, concat_WS(" ", b.uraian_aktivitas_kegiatan, ab.uraian_lokasi) as AKTIVITAS_LOKASI, 
 a.id_aktivitas_asb, a.id_item_ssh, a.id_rekening_ssh, l.nama_kd_rek_5, 
 a.uraian_belanja, a.volume_1, a.id_satuan_1, a.volume_2, a.id_satuan_2, 
 a.harga_satuan, a.jml_belanja, a.volume_1_forum, a.id_satuan_1_forum, a.volume_2_forum, a.id_satuan_2_forum, a.harga_satuan_forum, 
 COALESCE(a.jml_belanja_forum,0) as jml_belanja_forum, a.status_data, a.sumber_data, 
...
INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan) as f  
 ...
 GROUP BY a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.kd_keg, a.jenis_kegiatan 
   ...
   ');
    ...
    {
  //      $unit='1.1.1';
        $Belanja = DB::SELECT('SELECT a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.Kd_Keg, 
 a.kd_rek_1, a.kd_rek_2, a.kd_rek_3, a.kd_rek_4, a.kd_rek_5, 2 AS Kd_Ap_Pub, case max(sumber_dana) when 0 then 1 else max(sumber_dana) end AS Kd_Sumber 
 FROM ( 
 SELECT a.tahun_forum as tahun, k.Kd_Urusan, j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog, f.id_prog, f.Kd_Keg, f.nm_kegiatan,  
 l.kd_rek_1, l.kd_rek_2, l.kd_rek_3, l.kd_rek_4, l.kd_rek_5,  
 a.id_belanja_forum, a.id_lokasi_forum, a.id_zona_ssh, a.id_belanja_renja, a.sumber_belanja, 
 ...
 FROM trx_forum_skpd_belanja a 
 INNER JOIN trx_forum_skpd_aktivitas b ON a.id_lokasi_forum = b.id_aktivitas_forum 
 LEFT OUTER JOIN (select a.id_pelaksana_forum, GROUP_CONCAT(" ",b.nama_lokasi) as uraian_lokasi from trx_forum_skpd_lokasi a inner join ref_lokasi b on a.id_lokasi=b.id_lokasi 
 GROUP BY id_pelaksana_forum) AS ab ON ab.id_pelaksana_forum = b.id_aktivitas_forum 
 INNER JOIN trx_forum_skpd_pelaksana d ON b.id_forum_skpd = d.id_pelaksana_forum 
 INNER JOIN trx_forum_skpd e ON d.id_aktivitas_forum = e.id_forum_skpd 
 INNER JOIN (  
...
INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan) as f  
 ...
 INNER JOIN ref_urusan k ON j.kd_urusan = k.kd_urusan  
 INNER JOIN ref_rek_5 l on l.id_rekening = a.id_rekening_ssh 
 WHERE b.status_pelaksanaan = 0 and e.status_pelaksanaan not in (2,3,5) and a.tahun_forum = 2019 --  +jika+
 -- and k.Kd_Urusan like "kode_ur" and j.Kd_Bidang like "kode_bid" and i.Kd_Unit like "kode_unit" and h.Kd_Sub like "kode_sub" 
 ) a 
   where concat(a.Kd_Urusan,".", a.Kd_Bidang,".", a.kd_unit)="'.$unit.'"
 GROUP BY a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.kd_keg, 
 a.kd_rek_1, a.kd_rek_2, a.kd_rek_3, a.kd_rek_4, a.kd_rek_5  HAVING a.kd_rek_1=5
 ');
        $BelanjaHistory=DB::SELECT('SELECT a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.Kd_Keg, 
 a.kd_rek_1, a.kd_rek_2, a.kd_rek_3, a.kd_rek_4, a.kd_rek_5, 0 AS Nilai_Lalu, sum(pagu_plus1_forum) as Nilai_YAD, 0 AS Nilai_YAD_2, 0 AS Nilai_YAD_3 
 FROM ( 
SELECT a.tahun_forum as tahun, k.Kd_Urusan, j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog, f.id_prog, f.Kd_Keg, f.nm_kegiatan,  
 l.kd_rek_1, l.kd_rek_2, l.kd_rek_3, l.kd_rek_4, l.kd_rek_5, e.pagu_plus1_forum, 
 a.id_belanja_forum, a.id_lokasi_forum, a.id_zona_ssh, a.id_belanja_renja, a.sumber_belanja, 
 ab.uraian_lokasi, concat_WS(" ", b.uraian_aktivitas_kegiatan, ab.uraian_lokasi) as AKTIVITAS_LOKASI, 
 a.id_aktivitas_asb, a.id_item_ssh, a.id_rekening_ssh, l.nama_kd_rek_5,  
 a.uraian_belanja, a.volume_1, a.id_satuan_1, a.volume_2, a.id_satuan_2, 
...
substring(XML_Encode(GantiEnter( b.uraian_aktivitas_kegiatan )),1,255) as Keterangan_rinc,              
...
b.sumber_aktivitas,b.id_aktivitas_renja,a.id_aktivitas_asb,aa.id_tarif_ssh        
 ...
 WHERE
			...
			AND l.kd_rek_1 = 5
            AND concat(k.Kd_Urusan,".", j.Kd_Bidang,".", i.kd_unit)="'.$unit.'"
...
k.Kd_Urusan,j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog, f.id_prog, f.Kd_Keg,l.kd_rek_1, l.kd_rek_2, l.kd_rek_3, l.kd_rek_4, l.kd_rek_5,l.nama_kd_rek_5 as nm_rek_5,
b.id_aktivitas_forum, 
substring(XML_Encode(GantiEnter(case when ab.uraian_lokasi <> " " then concat_WS("",b.uraian_aktivitas_kegiatan, " (",ab.uraian_lokasi,")") else b.uraian_aktivitas_kegiatan end)),1,255) as Keterangan_rinc,              
b.sumber_dana as Kd_Sumber,
substring(cc.uraian_satuan,1,10) as Sat_1, a.volume_1_forum as Nilai_1,  substring(dd.uraian_satuan,1,10) as Sat_2, a.volume_2_forum as Nilai_2,            
  Null as Sat_3, 0 as Nilai_3,            
  case when substring(dd.uraian_satuan,1,10) is NULL then CONCAT(COALESCE(substring(cc.uraian_satuan,1,10),"-"),COALESCE(substring(dd.uraian_satuan,1,10),""))            
...
b.sumber_aktivitas,b.id_aktivitas_renja,a.id_aktivitas_asb,aa.id_tarif_ssh        
 ...
 WHERE
  ...
  l.kd_rek_1, l.kd_rek_2, l.kd_rek_3, l.kd_rek_4, l.kd_rek_5, id_aktivitas_forum   ');
    ...
    {
        $unit = DB::SELECT('SELECT a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.Kd_Keg, 
 a.kd_rek_1, a.kd_rek_2, a.kd_rek_3, a.kd_rek_4, a.kd_rek_5, a.kd_rek_2 AS Kd_Pendapatan, case max(sumber_dana) when 0 then 1 else max(sumber_dana) end AS Kd_Sumber 
 FROM ( 
 SELECT a.tahun_forum as tahun, k.Kd_Urusan, j.Kd_Bidang, i.kd_unit, h.kd_sub, f.Kd_Prog, f.id_prog, f.Kd_Keg, f.nm_kegiatan,  
 l.kd_rek_1, l.kd_rek_2, l.kd_rek_3, l.kd_rek_4, l.kd_rek_5,  
 a.id_belanja_forum, a.id_lokasi_forum, a.id_zona_ssh, a.id_belanja_renja, a.sumber_belanja, 
...
INNER JOIN ref_urusan d ON c.kd_urusan = d.kd_urusan) as f  
 ...
 GROUP BY a.tahun, a.Kd_Urusan, a.Kd_Bidang, a.kd_unit, a.kd_sub, a.Kd_Prog, a.id_prog, a.kd_keg, 
 a.kd_rek_1, a.kd_rek_2, a.kd_rek_3, a.kd_rek_4, a.kd_rek_5  HAVING a.kd_rek_1=4  
 ');
