...
class RefSeniORController extends Controller
{
    ...
    public function index(){
        ...
        return view('dasar.ref_senior');
    }
    public function getListsenior()
    {
        $getListsenior=DB::select('SELECT (@id:=@id+1) as no_urut,
...
, (SELECT @id:=0) x
where a.id_tabel_dasar=5 and b.level>0
');
        
        return DataTables::of($getListsenior)
        ->addColumn('action',function($getListsenior){
            return '
                        <button id="btnEditsenior" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Edit Perkembangan Kesenian, Budaya dan Olahraga" title="Edit Perkembangan Kesenian, Budaya dan Olahraga" class="btn btn-primary btn-sm" data-id_isian_tabel_dasar="'.$getListsenior->id_isian_tabel_dasar.'"><i class="fa fa-pencil fa-fw"></i></button>
                        <button id="btnHapussenior" type="button" data-toggle="popover" data-trigger="hover" data-container="body" data-html="true" data-content="Hapus Perkembangan Kesenian, Budaya dan Olahraga" title="Hapus Perkembangan Kesenian, Budaya dan Olahraga" class="btn btn-danger btn-sm" data-id_isian_tabel_dasar="'.$getListsenior->id_isian_tabel_dasar.'"><i class="fa fa-trash fa-fw"></i></button>
                    ' ;
        })
        
        ->make(true);
    }
    public function getTahunsenior()
    {
        $getTahun=DB::select('
...
');
    ...
    public function getKecamatansenior()
    {
...
');
    ...
    public function getSektorsenior($tahun,$kecamatan)
    {
        $getSektor=DB::select('
...
where id_kolom_tabel_dasar not in (select id_kolom_tabel_dasar from trx_isian_data_dasar where tahun='.$tahun.' and id_kecamatan='.$kecamatan.')
 and  id_tabel_dasar=5 and level<>0
');
        return json_encode($getSektor);
    }
    public function addsenior (Request $req)
    {
    ...
    public function getEditsenior($id)
    {
        $getListsenior=DB::select('SELECT c.id_isian_tabel_dasar,
c.tahun,
...
LEFT OUTER JOIN trx_isian_data_dasar AS c ON c.id_kolom_tabel_dasar = b.id_kolom_tabel_dasar
LEFT OUTER JOIN ref_kecamatan AS d ON c.id_kecamatan = d.id_kecamatan
where a.id_tabel_dasar=5 and b.level>0 and id_isian_tabel_dasar='.$id);
        
        return DataTables::of($getListsenior)
        ->make(true);
    }
    public function editsenior (Request $req)
    {
...
//         $data->nmin5_persen= $req->nmin5_persen;
    ...
    public function hapussenior (Request $req)
    {
