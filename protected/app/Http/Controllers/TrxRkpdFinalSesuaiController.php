...
class TrxRkpdFinalSesuaiController extends Controller
{
    ...
    {
        $dataForum = TrxRkpdFinalKegiatanPd::paginate(15);
    }
    ...
    public function getProgramRkpd($tahun,$unit)
    {
        $getRenja=DB::SELECT('SELECT (@id:=@id+1) as no_urut, a.id_rkpd_rancangan, a.id_rkpd_ranwal, a.tahun_rkpd, a.id_program_rpjmd, 
            ...
            FROM trx_rkpd_final_program_pd AS x 
            ...
            FROM trx_rkpd_final_program_pd AS x 
            INNER JOIN trx_rkpd_final_kegiatan_pd AS y ON x.id_program_pd = y.id_program_pd
            WHERE x.id_rkpd_rancangan = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            ...
            FROM trx_rkpd_final_program_pd AS x 
            INNER JOIN trx_rkpd_final_kegiatan_pd AS y ON x.id_program_pd = y.id_program_pd
            INNER JOIN trx_rkpd_final_pelaksana_pd AS z ON y.id_kegiatan_pd = z.id_kegiatan_pd
            INNER JOIN trx_rkpd_final_aktivitas_pd AS r ON z.id_pelaksana_pd = r.id_pelaksana_pd
            WHERE x.id_rkpd_rancangan = b.id_pelaksana_rkpd AND x.id_unit = b.id_unit AND x.tahun_forum = a.tahun_rkpd
            ...
            INNER JOIN trx_rkpd_final_urusan AS d ON a.id_rkpd_rancangan = d.id_rkpd_rancangan
            INNER JOIN trx_rkpd_final_pelaksana AS b ON d.id_urusan_rkpd = b.id_urusan_rkpd, 
          ...
          return DataTables::of($getRenja)
            ...
            ->addColumn('details_url', function($getRenja) {
                    return url('rkpd/getChildBidang/'.$getRenja->id_unit.'/'. $getRenja->id_rkpd_rancangan);
                })
            ->make(true);
    }
    ...
    public function getChildBidang($id_unit,$id_rkpd_rancangan)
    {
        $getRenja = DB::SELECT('SELECT DISTINCT '.Session::get('tahun').' AS tahun_forum, b.id_rkpd_rancangan,d.kd_bidang,d.nm_bidang,c.id_unit,
                ...
                FROM trx_rkpd_final a
                INNER JOIN trx_rkpd_final_urusan b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
                INNER JOIN trx_rkpd_final_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
    ...
    {
      $getProgramRenja=DB::select('SELECT (@id:=@id+1) as urut, a.* FROM (SELECT a.id_program_pd, a.id_forum_program, a.id_rkpd_rancangan, a.tahun_forum, a.no_urut, a.id_unit, b.nm_unit, a.id_renja_program, 
            ...
            COALESCE((SELECT COUNT(x.id_kegiatan_pd) AS jml_kegiatan
                        FROM trx_rkpd_final_kegiatan_pd AS x
            ...
            FROM trx_rkpd_final_program_pd a
            INNER JOIN ref_unit b ON a.id_unit = b.id_unit 
            INNER JOIN trx_rkpd_final_pelaksana c ON a.id_rkpd_rancangan = c.id_pelaksana_rkpd
            ...
            WHERE a.id_unit ='.$id_unit.' AND a.id_rkpd_rancangan = '.$id_pelaksana_rkpd.' AND a.tahun_forum='.Session::get('tahun'));

      return DataTables::of($getProgramRenja)
        ->addColumn('details_url', function($getProgramRenja) {
                    return url('rkpd/getIndikatorRenja/'.$getProgramRenja->id_program_pd);
                })
    ...
    public function getIndikatorRenja($id_rkpd)
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_renja, a.no_urut, a.id_program_pd, a.id_program_renstra,
                ...
                FROM trx_rkpd_final_prog_indikator_pd AS a  
                INNER JOIN trx_rkpd_final_program_pd AS b ON a.id_program_pd=b.id_program_pd
    ...
    public function getKegiatanRenja($id_program)
    {
      $getKegiatanRenja=DB::select('SELECT (@id:=@id+1) as urut, a.* FROM (SELECT a.id_kegiatan_pd, a.id_program_pd, a.id_unit, 
            ...
            WHEN 1 THEN "green"
            END AS warna,
            COALESCE((SELECT SUM(x.pagu_forum) FROM trx_rkpd_final_kegiatan_pd x WHERE x.id_kegiatan_pd=a.id_kegiatan_pd 
                GROUP BY x.id_kegiatan_pd),0) AS jml_pagu,
            COALESCE((SELECT COUNT(x.id_aktivitas_forum) as jml_aktivitas
            FROM trx_rkpd_final_aktivitas_pd x 
            INNER JOIN trx_rkpd_final_pelaksana_pd y ON x.id_pelaksana_pd = y.id_pelaksana_pd
            ...
            INNER JOIN trx_rkpd_final_program_pd AS x ON a.id_program_pd=x.id_program_pd
      ...
      ->addColumn('details_url', function($getKegiatanRenja) {
                    return url('rkpd/getIndikatorKegiatan/'.$getKegiatanRenja->id_kegiatan_pd);
                })
    ...
    public function getIndikatorKegiatan($id_rkpd)
    {
      $indikatorProg=DB::select('SELECT (@id:=@id+1) as urut,a.tahun_renja, a.no_urut, a.id_kegiatan_pd,
                ...
                FROM trx_rkpd_final_keg_indikator_pd AS a 
                INNER JOIN trx_rkpd_final_kegiatan_pd AS b ON a.id_kegiatan_pd=b.id_kegiatan_pd 
                INNER JOIN trx_rkpd_final_program_pd AS c ON b.id_program_pd=c.id_program_pd 
...
public function getAktivitas($id_forum_skpd)
{
   $getAktivitas=DB::SELECT('SELECT (@id:=@id+1) as urut, a.id_aktivitas_pd, a.id_pelaksana_pd, a.tahun_forum, a.no_urut, 
            ...
            FROM trx_rkpd_final_aktivitas_pd a
            ...
            LEFT OUTER JOIN (SELECT a.tahun_forum, a.id_aktivitas_pd,
                ...
                FROM trx_rkpd_final_lokasi_pd AS a
                INNER JOIN trx_rkpd_final_aktivitas_pd AS b ON a.id_aktivitas_pd = b.id_aktivitas_pd
            ...
            LEFT OUTER JOIN (SELECT a.tahun_forum, a.id_aktivitas_pd, Sum(a.jml_belanja_forum) as jml_belanja
                FROM trx_rkpd_final_belanja_pd AS a
...
public function getPelaksanaAktivitas($id_aktivitas){
   $getPelaksana=DB::SELECT('SELECT (@id:=@id+1) as urut,a.* FROM (SELECT a.id_pelaksana_pd, a.tahun_forum, a.no_urut, 
            ...
            FROM trx_rkpd_final_aktivitas_pd AS X
            INNER JOIN trx_rkpd_final_lokasi_pd AS Y ON X.id_aktivitas_pd = Y.id_aktivitas_pd
            ...
            FROM trx_rkpd_final_belanja_pd AS X
            INNER JOIN trx_rkpd_final_aktivitas_pd AS Y ON X.id_aktivitas_pd = Y.id_aktivitas_pd
            ...
            CASE a.status_data
                ...
                WHEN 1 THEN "green"
            END AS warna 
            FROM trx_rkpd_final_pelaksana_pd a
            INNER JOIN trx_rkpd_final_kegiatan_pd f ON a.id_kegiatan_pd = f.id_kegiatan_pd
...
public function getLokasiAktivitas($id_aktivitas_pd)
{
   $LokAktiv=DB::SELECT('SELECT (@id:=@id+1) as no_urut,a.* FROM (SELECT a.tahun_forum, a.no_urut, a.id_aktivitas_pd, a.id_lokasi_pd,
            ...
            a.uraian_lokasi, a.lat, a.lang, a.status_data, a.sumber_data, a.status_pelaksanaan, a.ket_lokasi, 
                        ...
                        END AS warna,
                        CASE a.sumber_data
                            WHEN 0 THEN "Forum Perangkat Daerah"
                            WHEN 1 THEN "Rancangan RKPD"
            ...
            FROM trx_rkpd_final_lokasi_pd a
            INNER JOIN ref_lokasi b on a.id_lokasi = b.id_lokasi
            INNER JOIN trx_rkpd_final_aktivitas_pd c ON a.id_aktivitas_pd = c.id_aktivitas_pd) a,
   ...
   $getBelanja=DB::SELECT('SELECT (@id:=@id+1) as no_urut,a.* FROM (SELECT a.tahun_forum,a.no_urut as urut,a.id_belanja_pd,a.id_aktivitas_pd, p.status_data AS status_aktivitas,
        ...
        h.uraian_satuan as satuan_2_forum, e.kd_rekening, e.nm_rekening, f.nm_aktivitas_asb,
                    ...
                    CASE a.status_data
                        ...
                        WHEN 1 THEN "green"
                    END AS warna
        FROM trx_rkpd_final_belanja_pd a
        ...
        INNER JOIN trx_rkpd_final_aktivitas_pd p on a.id_aktivitas_pd = p.id_aktivitas_pd) a,
        (SELECT @id:=0) x WHERE a.id_aktivitas_pd='.$id_aktivitas_pd);
...
public function getLokasiCopy($id_unit){
   $getBelanja=DB::SELECT('SELECT (@id:=@id+1) as urut, e.*, c.id_unit
        FROM trx_rkpd_final_aktivitas_pd e
        INNER JOIN trx_rkpd_final_pelaksana_pd d ON e.id_pelaksana_pd = d.id_pelaksana_pd
        INNER JOIN trx_rkpd_final_kegiatan_pd a ON d.id_kegiatan_pd = a.id_kegiatan_pd
        INNER JOIN trx_rkpd_final_program_pd b ON a.id_program_pd = b.id_program_pd
        INNER JOIN trx_rkpd_final_pelaksana c ON b.id_rkpd_rancangan = c.id_pelaksana_rkpd,
        (SELECT @id:=0) x
...
public function getBelanjaCopy(Request $req){
    ...
    if($cekProgramRkpd != NULL && $cekProgramRkpd[0]->status_rkpd == 0 && $cekProgramRkpd[0]->status_program == 0 && $cekProgramRkpd[0]->status_kegiatan == 0 && $cekProgramRkpd[0]->status_aktivitas == 0) {
        $getBelanja=DB::INSERT('INSERT INTO trx_rkpd_final_belanja_pd (tahun_forum, no_urut, id_aktivitas_pd, id_belanja_forum, id_zona_ssh, id_belanja_renja, sumber_belanja, id_aktivitas_asb, id_item_ssh, id_rekening_ssh, uraian_belanja, volume_1, id_satuan_1, volume_2, id_satuan_2, harga_satuan, jml_belanja, volume_1_forum, id_satuan_1_forum, volume_2_forum, id_satuan_2_forum, harga_satuan_forum, jml_belanja_forum, status_data, sumber_data) 
        SELECT tahun_forum, no_urut, '.$req->id_aktivitas_new.', id_belanja_forum, id_zona_ssh, id_belanja_renja, sumber_belanja, id_aktivitas_asb, id_item_ssh, id_rekening_ssh, uraian_belanja, volume_1, id_satuan_1, volume_2, id_satuan_2, harga_satuan, jml_belanja, volume_1_forum, id_satuan_1_forum, volume_2_forum, id_satuan_2_forum, harga_satuan_forum, jml_belanja_forum, 0, 4 FROM trx_rkpd_final_belanja_pd where id_aktivitas_pd ='.$req->id_aktivitas_pd);
...
public function getHitungASB(Request $req){
    ...
    if($cekProgramRkpd != NULL && $cekProgramRkpd[0]->status_rkpd == 0 && $cekProgramRkpd[0]->status_program == 0 && $cekProgramRkpd[0]->status_kegiatan == 0 && $cekProgramRkpd[0]->status_aktivitas == 0) {
        $getHitung=DB::INSERT('INSERT INTO trx_rkpd_final_belanja_pd (tahun_forum, no_urut, id_aktivitas_pd, id_belanja_forum, id_zona_ssh, id_belanja_renja, sumber_belanja, id_aktivitas_asb, 
...
public function unloadASB(Request $req){
    ...
    if($cekProgramRkpd != NULL && $cekProgramRkpd[0]->status_rkpd == 0 && $cekProgramRkpd[0]->status_program == 0 && $cekProgramRkpd[0]->status_kegiatan == 0 && $cekProgramRkpd[0]->status_aktivitas == 0) {
        ...
        $getHitung=DB::DELETE('DELETE FROM trx_rkpd_final_belanja_pd 
...
public function getBidang($id_unit,$id_ranwal){
        $urusan=DB::select('SELECT a.tahun_rkpd, a.id_rkpd_rancangan, a.id_bidang, d.nm_bidang, b.id_unit
            ...
            INNER JOIN trx_rkpd_final_pelaksana b ON a.id_urusan_rkpd = b.id_urusan_rkpd
...
// $cekProgramRkpd = DB::SELECT('SELECT a.status_data FROM trx_rkpd_final a
//     INNER JOIN trx_rkpd_final_urusan b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
//     INNER JOIN trx_rkpd_final_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
//     WHERE c.id_pelaksana_rkpd='.$req->id_rkpd_rancangan);

$cek = DB::SELECT('SELECT c.hak_akses, a.status_data FROM trx_rkpd_final a
    INNER JOIN trx_rkpd_final_urusan b ON a.id_rkpd_rancangan = b.id_rkpd_rancangan
    INNER JOIN trx_rkpd_final_pelaksana c ON b.id_urusan_rkpd = c.id_urusan_rkpd
    WHERE c.id_pelaksana_rkpd='.$req->id_pelaksana_rkpd);

        $data = new TrxRkpdFinalProgramPd();
...
$data = TrxRkpdFinalProgramPd::find($req->id_program_pd);
        $data->jenis_belanja= $req->jenis_belanja;
        $data->no_urut= $req->no_urut;
        $data->id_unit= $req->id_unit;
...
public function hapusProgRenja(Request $req){
    TrxRkpdFinalProgramPd::where('id_program_pd',$req->id_program_pd)->delete();
...
public function postProgRenja(Request $req)
    {
        ...
        $cekRkpd = DB::SELECT('SELECT a.status_data AS status_rkpd FROM trx_rkpd_final_pelaksana AS a
            INNER JOIN trx_rkpd_final_program_pd AS b ON a.id_pelaksana_rkpd = b.id_rkpd_rancangan
        ...
        $cek = DB::SELECT('SELECT a.id_program_pd, a.pagu_forum, (COALESCE(a.pagu_forum,0) - COALESCE(b.pagu_kegiatan,0)) AS selisih  
            FROM trx_rkpd_final_program_pd a
            LEFT OUTER JOIN (SELECT id_program_pd, COALESCE(SUM(pagu_forum),0) AS pagu_kegiatan FROM trx_rkpd_final_kegiatan_pd
...
$cek = DB::SELECT('SELECT a.hak_akses FROM trx_rkpd_final_pelaksana a
        INNER JOIN trx_rkpd_final_program_pd b ON a.id_pelaksana_rkpd = b.id_rkpd_rancangan
        WHERE b.id_program_pd = '.$req->id_program_pd.' GROUP BY b.id_program_pd,a.hak_akses');

        $data = new TrxRkpdFinalKegiatanPd();
...
$cek = DB::SELECT('SELECT a.hak_akses FROM trx_rkpd_final_pelaksana a
        ...
        $data = TrxRkpdFinalKegiatanPd::find($req->id_kegiatan_pd);
...
public function postKegRenja(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT c.id_kegiatan_pd, c.status_data, c.pagu_forum, COALESCE(b.jml_pagu_aktivitas,0) as jml_pagu_aktivitas, 
            ...
            FROM (SELECT a.tahun_forum, b.id_kegiatan_pd,
                ...
                FROM trx_rkpd_final_aktivitas_pd a 
                INNER JOIN trx_rkpd_final_pelaksana_pd b ON a.id_pelaksana_pd = b.id_pelaksana_pd
                WHERE a.status_data = 1
                GROUP BY a.tahun_forum, b.id_kegiatan_pd, a.status_data) b
            INNER JOIN trx_rkpd_final_kegiatan_pd c ON b.id_kegiatan_pd = c.id_kegiatan_pd
...
public function hapusKegRenja(Request $req){
    $result = TrxRkpdFinalKegiatanPd::where('id_kegiatan_pd',$req->id_kegiatan_pd)->delete ();
...
public function addAktivitas(Request $req)
    {
            $data = new TrxRkpdFinalAktivitasPd;
            ...
            FROM trx_rkpd_final_pelaksana_pd b
            INNER JOIN trx_rkpd_final_kegiatan_pd c ON b.id_kegiatan_pd = c.id_kegiatan_pd
            INNER JOIN trx_rkpd_final_program_pd d ON c.id_program_pd = d.id_program_pd
...
public function editAktivitas(Request $req)
    {
        $data = TrxRkpdFinalAktivitasPd::find($req->id_aktivitas_pd);
            $data->no_urut= $req->no_urut;
            $data->sumber_aktivitas= $req->sumber_aktivitas;
            $data->id_aktivitas_asb= $req->id_aktivitas_asb;
            $data->id_aktivitas_renja= $req->id_aktivitas_renja;
...
public function postAktivitas(Request $req)
    {
        ...
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan
            ...
            INNER JOIN trx_rkpd_final_kegiatan_pd a ON d.id_kegiatan_pd = a.id_kegiatan_pd
            INNER JOIN trx_rkpd_final_program_pd b ON a.id_program_pd = b.id_program_pd
            INNER JOIN trx_rkpd_final_pelaksana c ON b.id_pelaksana_rkpd = c.id_pelaksana_rkpd
            WHERE e.id_aktivitas_pd='.$req->id_aktivitas_pd);
        ...
        $cek = DB::SELECT('SELECT a.tahun_forum,a.id_aktivitas_pd, SUM(a.pagu_aktivitas_forum) as pagu_aktivitas, COALESCE(sum(e.jml_belanja),0) as pagu_belanja, 
            ...
            FROM trx_rkpd_final_aktivitas_pd a
            LEFT OUTER JOIN (SELECT id_aktivitas_pd, COUNT(*) AS jml_lokasi FROM trx_rkpd_final_lokasi_pd
            ...
            FROM trx_rkpd_final_belanja_pd AS a GROUP BY a.id_aktivitas_pd) e ON a.id_aktivitas_pd = e.id_aktivitas_pd
...
public function hapusAktivitas(Request $req)
      {
        ...
        if($cek[0]->status_data==0){
            TrxRkpdFinalAktivitasPd::where('id_aktivitas_pd',$req->id_aktivitas_pd)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
        } else {
...
public function addPelaksana(Request $req)
    {
            ...
            $data = new TrxRkpdFinalPelaksanaPd;
            $data->tahun_forum= Session::get('tahun');
            $data->no_urut= $req->no_urut;
        ...
        $cek = DB::SELECT('SELECT c.status_data AS status_kegiatan, d.status_data AS status_program,
            ...
            FROM trx_rkpd_final_kegiatan_pd c
            INNER JOIN trx_rkpd_final_program_pd d ON c.id_program_pd = d.id_program_pd
            WHERE c.id_kegiatan_pd='.$req->id_kegiatan_pd);
...
public function editPelaksana(Request $req)
    {
            ...
            $data = TrxRkpdFinalPelaksanaPd::find($req->id_pelaksana_pd);
            $data->no_urut= $req->no_urut;
            $data->id_sub_unit= $req->id_sub_unit;
            $data->id_lokasi= $req->id_lokasi;
            $data->ket_pelaksana= $req->ket_pelaksana;
...
public function hapusPelaksana(Request $req)
      {
        ...
        if($cek[0]->status_data==0){
            TrxRkpdFinalPelaksanaPd::where('id_pelaksana_pd',$req->id_pelaksana_pd)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
        } else {
            return response ()->json (['pesan'=>'Data Kegiatan telah diposting, Data tidak dapat dihapus','status_pesan'=>'0']);
        }
...
public function editLokasi(Request $req)
    {
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan, e.status_data AS status_aktivitas
            ...
            $data = TrxRkpdFinalLokasiPd::find($req->id_lokasi_pd);
            $data->no_urut = $req->no_urut ;
            $data->jenis_lokasi = $req->jenis_lokasi ;
            $data->id_lokasi = $req->id_lokasi ;
            $data->id_lokasi_teknis = $req->id_lokasi_teknis ;            
...
public function hapusLokasi(Request $req)
      {
        TrxRkpdFinalLokasiPd::where('id_lokasi_pd',$req->id_lokasi_pd)->delete ();
        return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
      }
...
public function addUsulan(Request $req)
    {
        try{
            $data = new TrxRkpdFinalKegiatanPdUsulan;
...
public function editUsulan(Request $req)
    {
        try{
            $data = TrxRkpdFinalKegiatanPdUsulan::find($req->id_sumber_usulan);
            $data->sumber_usulan= $req->sumber_usulan;
            $data->id_lokasi_forum= $req->id_lokasi_forum;
            $data->id_ref_usulan= $req->id_ref_usulan;
            $data->volume_1_usulan= $req->volume_1_usulan;
...
public function hapusUsulan(Request $req)
      {
        TrxRkpdFinalKegiatanPdUsulan::where('id_sumber_usulan',$req->id_sumber_usulan)->delete ();
        return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
      }
    ...
    public function addBelanja(Request $req)
    {
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan, e.status_data AS status_aktivitas
            ...
            $data = new TrxRkpdFinalBelanjaPd;
            $data->tahun_forum= Session::get('tahun');
            $data->no_urut= $req->no_urut;
            $data->id_aktivitas_pd= $req->id_aktivitas_pd;
...
public function editBelanja(Request $req)
    {
        $cekProgramRkpd = DB::SELECT('SELECT e.id_aktivitas_pd, c.hak_akses, c.status_data AS status_rkpd, b.status_data AS status_program, c.status_data AS status_kegiatan, e.status_data AS status_aktivitas
            ...
            $data = TrxRkpdFinalBelanjaPd::find($req->id_belanja_pd);
            $data->no_urut= $req->no_urut;
            $data->id_zona_ssh= $req->id_zona_ssh;
            $data->id_aktivitas_asb= $req->id_aktivitas_asb;
            $data->id_item_ssh= $req->id_item_ssh;
...
public function hapusBelanja(Request $req)
      {
        TrxRkpdFinalBelanjaPd::where('id_belanja_pd',$req->id_belanja_pd)->delete ();
        return response ()->json (['pesan'=>'Data Berhasil Dihapus'] );
      }

public function addIndikatorProg(Request $req)
    {
            $data = new TrxRkpdFinalProgIndikatorPd;
        ...
        $cek = DB::SELECT('SELECT * from trx_rkpd_final_program_pd WHERE id_program_pd='.$req->id_program_pd);
        if($cek[0]->status_data == 0){            
            try{
                $data->save (['timestamps' => false]);
                return response ()->json (['pesan'=>'Data Berhasil Disimpan','status_pesan'=>'1']);
...
public function editIndikatorProg(Request $req)
    {
            $data = TrxRkpdFinalProgIndikatorPd::find($req->id_indikator_program);
            $data->no_urut = $req->no_urut ;
            // $data->id_program_pd = $req->id_program_pd ;
            $data->kd_indikator = $req->kd_indikator ;
            $data->uraian_indikator_program =  $req->uraian_indikator_program ;
...
public function postIndikatorProg(Request $req)
    {
        ...
        $cek = DB::SELECT('SELECT a.status_data from trx_rkpd_final_program_pd a
            INNER JOIN trx_rkpd_final_prog_indikator_pd b ON a.id_program_pd = b.id_program_pd
...
public function delIndikatorProg(Request $req)
    {
        ...
        if($cek[0]->status_data == 0){            
            TrxRkpdFinalProgIndikatorPd::where('id_indikator_program',$req->id_indikator_program)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus','status_pesan'=>'1'] );
        } else {
            return response ()->json (['pesan'=>'Data Gagal Status Program telah diposting','status_pesan'=>'0']);
        }
...
public function addIndikatorKeg(Request $req)
    {
            $data = new TrxRkpdFinalKegIndikatorPd;
            $data->tahun_renja = $req->tahun_renja;
            $data->no_urut = $req->no_urut ;
            $data->id_kegiatan_pd = $req->id_kegiatan_pd ;
            $data->id_perubahan = 0 ;
        ...
        $cek = DB::SELECT('SELECT * FROM trx_rkpd_final_kegiatan_pd WHERE id_kegiatan_pd = '.$req->id_kegiatan_pd);
        if($cek[0]->status_data == 0){
            try{
                $data->save (['timestamps' => false]);
                return response ()->json (['pesan'=>'Data Berhasil Disimpan','status_pesan'=>'1']);
...
public function editIndikatorKeg(Request $req)
    {
        $cek = DB::SELECT('SELECT * FROM trx_rkpd_final_kegiatan_pd WHERE id_kegiatan_pd = '.$req->id_kegiatan_pd);
            ...
            $data = TrxRkpdFinalKegIndikatorPd::find($req->id_indikator_kegiatan);
            $data->no_urut = $req->no_urut ;
            $data->kd_indikator = $req->kd_indikator ;
            $data->uraian_indikator_kegiatan =  $req->uraian_indikator_kegiatan ;
            $data->tolok_ukur_indikator = $req->tolok_ukur_indikator ;
...
public function postIndikatorKeg(Request $req)
    {
        $cek = DB::SELECT('SELECT a.status_data FROM trx_rkpd_final_kegiatan_pd a
...
public function delIndikatorKeg(Request $req)
    {
        ...
        if($cek[0]->status_data == 0){
            TrxRkpdFinalKegIndikatorPd::where('id_indikator_kegiatan',$req->id_indikator_kegiatan)->delete ();
            return response ()->json (['pesan'=>'Data Berhasil Dihapus','status_pesan'=>'1'] );
        } else {
            return response ()->json (['pesan'=>'Data Gagal Status Kegiatan telah diposting','status_pesan'=>'0']);
        }
